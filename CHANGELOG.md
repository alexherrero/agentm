...
- **Prompt-submit recall returns nothing when the local embedding model is installed** — found while writing the above, and the reason the test docstring says so out loud. `embed.py` defaults to local mode; loading `BAAI/bge-large-en-v1.5` costs ~3.8s, and the 300ms deadline is spent by the time `query()` reaches either half. The vec results are discarded by the post-embed deadline check and the lexical fallback self-cancels on no-time-left, so recall yields zero entries against a vault that plainly contains a match — 4.39s and `Loaded 0`, where the same vault with the package absent gives 0.06s and the right entry. The hook itself is fine: it fires, exits 0, and reports the overrun honestly. What it contradicts is `hook.md`'s "return the partial results gathered so far," which is a recall-engine question and a design call about whether a 300ms budget belongs in front of a multi-second model load. Left for its own change rather than folded in here.
