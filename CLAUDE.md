...
- The harness hooks register themselves in `<prefix>/settings.json` as part of a normal `install.sh` / `install.ps1` run — there is no `--hooks` flag any more. The per-project PostToolUse verification hook that flag used to wire up is being re-homed machine-wide; see `PLAN-user-scope-hook-gaps`.
