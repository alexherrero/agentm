...
# Every test here gets its own engine state directory. The routing check that
# `FaithfulDerivedConformance` inherits registers and unregisters a slug
# through `repo_registry`, which has kept the registry there since the
# memory-root trims (agentm-vault plan 05) whatever backend it is handed, so a
# hand run rewrote the machine's `repos.json`.
