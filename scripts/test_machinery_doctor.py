...
# `ClaudeDesktopRowTests` lived here. The Claude Desktop surface — its stdio
# bridge, the `claude-desktop-entry` row and the how-to — retired in
# agentm-vault plan 16: the reason it was built (a chat surface with no route to
# the machine) is answered by `agent/inbox/` over Drive instead. The row's own
# hard-won property, that a doctor row must never reproduce a config file it
# does not own, is kept by `ProjectJsonPointerTests` and `JobConfigTests`, which
# read the operator's files and report answers about them rather than contents.
#
# `TheDoctorsRowCountFell` below is what stops the removal from being a silent
# one.


class TheDoctorsRowCountFell(unittest.TestCase):
    def test_the_desktop_row_is_gone_and_nothing_else_is(self):
        # Exactly one row, and only that row. A removal that took a neighbour
        # with it would look identical in a summary line.
        rows = {c.name for c in md.run_inventory(md.repo_root())}
        self.assertNotIn("claude-desktop-entry", rows)
        for still_there in ("install-head", "install-binary", "storage-rules",
                            "memory-hook-interpreter", "runner-cycle",
                            "crickets-sibling", "unattended-merge-gate"):
            self.assertIn(still_there, rows, f"{still_there} went with the desktop row")
        self.assertFalse(hasattr(md, "check_claude_desktop_entry"))
