...
Covers:
  ...
  - an entry left in a retired `memory/` home is neither listed nor resolved
"""
...
class DismissArchivesIntoOwnRootTests(_WatchlistReviewTestBase):
    def test_skill_watchlist_entry_archives_into_skill_watchlist_archive(self) -> None:
        path = self._write_entry(Path("Projects/agentm/_skill-watchlist"), "skill-src", "cool-skill")
...
class RetiredHomeIsNotASurfaceTests(_WatchlistReviewTestBase):
    def test_an_entry_left_in_a_retired_memory_home_is_neither_listed_nor_resolved(self) -> None:
        self._write_entry(Path("memory/_skill-watchlist"), "skill-src", "left-behind")

        self.assertEqual(wr.list_watchlist_entries(self.vault), [])
        resolved = wr._entry_path_from_slugs(self.vault, "skill-src", "left-behind")
        self.assertEqual(
            resolved,
            self.vault / "Projects" / "agentm" / "_skill-watchlist" / "skill-src" / "left-behind.md",
        )
