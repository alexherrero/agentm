...
gate "check-vendored-parity (lib + vault-lock + corpus-gate + storage-seam + hook-config)" bash scripts/check-vendored-parity.sh
...
gate "check-card-shape (class cards in the card's order: required fields, no retired field, no counter name; personal/ideas/ idea cards in their own shape)" "$PY" scripts/check-card-shape.py
gate "check-card-prompt (the inbox paste teaches the card's own shape)" "$PY" scripts/check-card-prompt.py
gate "check-memory-skill-table (the /memory table names every section the skill has)" "$PY" scripts/check-memory-skill-table.py
...
gate "verify-dreaming (a twin found, no note changed, the pair on the needs-review map)" bash scripts/verify-dreaming.sh
gate "check-dreaming-parity (the dreaming binary reproduces the recorded Python pass)" bash scripts/check-dreaming-parity.sh
gate "verify-declare-a-type (contract edit -> coverage falls -> the batch queue offers it -> climbs, real binary)" bash scripts/verify-declare-a-type.sh
