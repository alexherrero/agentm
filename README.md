...
> **Latest:** v4.3.0 (2026-05-27) — **Global install + `--scope user`** (paired pair #12 with [crickets v2.1.0](https://github.com/alexherrero/crickets/releases/tag/v2.1.0)). The first install-model overhaul per ROADMAP-V4 #30: `--scope user` flag added to `install.sh` + `install.ps1`. When passed, customizations install once to `~/.claude/` (skills + hooks + agents + commands as symlinks for source-clone operators; SHA256-aware copies for release-install operators). Auto-stay-in-sync default-on: source-clone edits propagate live via symlinks; release-install operators get SessionStart upstream-version-check + `~/.local/bin/agentm-update` launcher. 3 new cross-repo Python helpers at `lib/install/python/` (byte-identical with crickets). New `repo_registry.py` vault-backed primitive. crickets-sibling auto-detect bundled. Default scope stays `project` for v4.3.0 backward compat; flips to `user` in a future release once real-use validates. Per ROADMAP-V4 item #30.  
...
Currently shipping **v4.3.0** — global install + `--scope user`. The first install-model overhaul: customizations install once to `~/.claude/` (`--scope user`) instead of per-project. Auto-stay-in-sync default-on for source-clone operators (live symlinks) + release-install operators (SessionStart upstream-version-check + `agentm-update` launcher). Three new cross-repo Python helpers at `lib/install/python/` byte-identical with crickets. New vault-backed `repo_registry.py` primitive at `<vault>/_meta/repos.json`. crickets-sibling auto-detect bundled. Default scope stays `project` for backward compat; flips to `user` in a future release. Paired with [crickets v2.1.0](https://github.com/alexherrero/crickets/releases/tag/v2.1.0). See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
