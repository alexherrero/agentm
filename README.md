...
> **Latest:** v4.9.0 (2026-05-29) — **Agent Memory vault cleanup audit** (V4 #33; single-repo MINOR). A read-only lint over the MemoryVault: `harness/skills/memory/scripts/vault_lint.py` runs 9 checks over agent-shaped entries (frontmatter schema, kebab-case, field order, dates, unfilled `a | b | c` placeholders, schema drift, `[[wikilink]]` resolution, supersede integrity) and an `--audit` mode that writes a grouped operator-review report to `<vault>/_meta/vault-lint-<date>.md`. It **never touches the vault** — surfaces a suggested fix per finding; the operator applies (A3, locked DC-1; auto-repair → V5-5). Wikilinks resolve against the whole Obsidian vault root, not just `AgentMemory/`. Live dogfood drove three calibrations (131→39 findings, no false-positive floods); a pre-release adversarial review caught + fixed a contained supersede false-negative. `save.py`'s group regex was widened for deep groups the vault had outgrown. +26 tests (380 → 406). Single-repo release; crickets unaffected. Per ROADMAP-V4 item #33.  
...
Currently shipping **v4.9.0** — Agent Memory vault cleanup audit (V4 #33; single-repo MINOR). A read-only lint over the MemoryVault + a one-time audit pass. `harness/skills/memory/scripts/vault_lint.py` runs 9 checks over agent-shaped entries (frontmatter schema, kebab-case, field order, dates, unfilled `a | b | c` placeholder values, schema drift, `[[wikilink]]` resolution, supersede integrity), and an `--audit` mode writes a grouped operator-review report to `<vault>/_meta/vault-lint-<date>.md`. It **never touches the vault** — surfaces a suggested fix per finding; the operator applies (A3, locked DC-1; auto-repair deferred to V5-5). It lints only agent-shaped entries (the `kind`+`status`+`created` trio), skipping the operator's free-form notes, and resolves wikilinks against the whole Obsidian vault root. Live dogfood drove three calibrations (131 → 39 findings, no false-positive floods); a pre-release adversarial review caught + fixed a contained supersede false-negative; `save.py`'s group regex was widened for deep groups the vault had outgrown. +26 tests (380 → 406 per OS workflow). Deferred follow-ups: auto-fix (→ V5-5); bespoke-shape lint for the idea-incubator + `Ideas.md`; a personal-notes link-discovery audit (→ #43). Next ROADMAP picks: V4 #38 wiki bundle / #40 crickets-plugins-consolidation / #34 vault aesthetic pass. Single-repo release; crickets unaffected. See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
