...
> **Latest:** v4.14.0 (2026-06-02) — **Decouple from crickets: agentm stands alone** (single-repo MINOR). crickets v3.0 retired its bespoke per-host installer in favor of native Claude Code / Antigravity plugins; this is agentm's side of the clean break. The harness is now a **self-contained standalone install** — `install.sh`/`install.ps1` no longer clone or bootstrap crickets, `sync-lib.sh` is local-only, and agentm owns its `lib/install/` outright (closing ADR 0006's lib-sync re-audit trigger). crickets stays the optional toolkit, installed separately via its native plugin path. Also folds in the **V4 verification battery** — `verify-v4.sh` (scratch-vault integration check for the auto-orchestration push surface) + `check-all.sh` (one-command local gate battery), wired into CI.  
> **Prior:** v4.13.1 (2026-06-01) — **Auto-orchestration fast-follows** (single-repo PATCH). Three hardening fixes on the v4.13.0 push-surface: atomic state writes (concurrency-safe shared state), the idle chain's staged adapt candidates now surface in the SessionStart briefing (closing the discover→adapt→evaluate loop), and a stale hook-doc fix. Builds on **v4.13.0** — the auto-orchestration push-surface (V4 #23; the V4 foundation is complete).  
