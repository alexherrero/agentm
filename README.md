...
> **Latest:** v4.2.0 (2026-05-27) — **Vec-index drift detection + workflow-uses-vault completion.** Closes two coupled gaps from the 2026-05-27 adversarial review + plan #20 task 7's scope adjustment. **(A)** When an operator edits a `.md` file directly in Obsidian, the `sqlite-vec` index drifts silently — opportunistic mtime-check at recall time + `vec_index.py full-sync` operator-invoked sweep + `memory-reflect-idle` hook integration keep the index honest. **(B)** Slash commands + phase specs now route through `harness_memory.py read-state / write-state / vault-state-path` dispatcher CLI subcommands instead of bare legacy `.harness/<file>` reads; cleanup pass against operator's three target repos (agentm + sherwood + dev-setup) removed legacy state files (vault-canonical post-V4 #26 + workflow now actually uses it). Pre-#37 vec-indexes auto-migrate transparently via ALTER TABLE ADD COLUMN. Single-repo release (Crickets unaffected at v2.0.0). Per ROADMAP-V4 item #37.  
...
Currently shipping **v4.2.0** — vec-index drift detection + workflow-uses-vault completion. The `sqlite-vec` index stays honest when an operator edits `.md` files directly in Obsidian via opportunistic mtime-check at recall time + an operator-invoked `vec_index.py full-sync` sweep + `memory-reflect-idle` hook integration. Slash commands + phase specs now route through `harness_memory.py read-state / write-state / vault-state-path` dispatcher CLI subcommands; legacy `<project>/.harness/<file>` state files cleaned up on operator's three target repos (agentm + sherwood + dev-setup). Pre-#37 vec-indexes auto-migrate transparently via ALTER TABLE ADD COLUMN; no operator action required. Crickets stays at v2.0.0 (no paired release). See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
