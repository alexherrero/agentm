...
> **Latest:** v4.5.0 (2026-05-27) — **Migration tooling + opt-out documentation** (V4 #30 plan 3 of 3 — **CLOSING**; single-repo). Closes the V4 #30 trio (plan 1: v4.3.0 + v2.1.0 paired; plan 2: v4.4.0 single-repo; plan 3: this release). New `lib/install/python/install_migrate.py` primitive + `scripts/migrate-to-user-scope.{sh,ps1}` operator CLI: detects per-project agentm installs at `<target>/.claude/{skills,hooks,agents,commands}/`, classifies each entry via SHA256 against source-clone canonical paths (4 classifications: `safe_to_migrate` / `already_symlinked` / `operator_edited` / `unrecognized`), and moves byte-identical content into user scope. Preview-first; reversible via `--rollback` (mirrors V4 #26 pattern); opt-in `--cleanup` after byte-identical verification; auto-registers the repo in `repo_registry` (`--no-register` opt-out). New `wiki/how-to/Use-Per-Project-Install.md` documents the legitimate `--scope project` cases (CI runners; shared dev hosts; multi-developer dotfiles per locked DC-10) + `wiki/reference/Migration-Tool.md` is the flag-by-flag reference. 4 defects caught + fixed pre-commit by adversarial-reviewer (cleanup shape-bias destroyed operator non-`.md` files; `.DS_Store` polluted dir-bundle SHA on macOS; force-apply rerun overwrote backup; rollback file-branch missing dest-clobber guard). +26 unit tests; 212 project total. `--scope user` default-flip explicitly DEFERRED to a separate v4.5.x or v4.6.x release per locked DC-1. Single-repo release; crickets stays at v2.1.0. Per ROADMAP-V4 item #30.  
...
Currently shipping **v4.5.0** — migration tooling + opt-out documentation (V4 #30 plan 3 of 3 — **CLOSING**). Closes the V4 #30 trio: plan 1 (v4.3.0 + v2.1.0 paired) shipped `--scope user` install + `repo_registry` + auto-stay-in-sync; plan 2 (v4.4.0 single-repo) shipped wiki I/O codification + cross-repo views; plan 3 (this release) ships the automated + reversible migration tooling for non-operator users + opt-out documentation. New `migrate-to-user-scope.{sh,ps1}` operator CLI: preview-by-default; classifies per-project install via SHA256 (4 classifications); apply / rollback / cleanup modes; auto-registers in `repo_registry` (default-on); refuses to run inside CI by default. New `wiki/how-to/Use-Per-Project-Install.md` documents legitimate `--scope project` cases (CI runners; shared dev hosts; multi-developer dotfiles per locked DC-10); new `wiki/reference/Migration-Tool.md` is the flag-by-flag reference. `--scope user` default-flip explicitly DEFERRED to a separate v4.5.x or v4.6.x release per DC-1 (smaller blast radius). Next ROADMAP picks: V4 #38 wiki bundle (first sub-item of opinionated capability bundles meta; pickup signal *"let's build the wiki bundle"*) OR V4 #35 (documenter vault-context resolution). Single-repo release; crickets unaffected at v2.1.0. See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
