...
> **Latest:** v4.6.0 (2026-05-28) — **Documenter vault-context resolution** (V4 #35; single-repo MINOR). The documenter-side closure of the V4 #26 state migration: the doc-touching customizations now read operator conventions + project decisions from the vault instead of re-deriving them at each doc edit. New `documenter` recall phase + `resolve_documenter_context(slug)` helper + `documenter-context` CLI (`--slug` / `--budget` / `--format text|json`; rc `0`/`1`/`2` = bundle / vault-unavailable / slug-not-registered) in `harness_memory.py`. Three primitives consume the bundle: the `documenter` sub-agent runs a pre-flight before scanning `wiki/`; the `wiki-author` skill surfaces it in its preview-before-write step; the `diataxis-author` skill routes its convention read through the resolver — all graceful-skipping to repo-local context when the vault is unreachable. **Folds in v4.5.2** — an installer-probe bugfix (`scripts/vault_probe.py`): the v4.5.1 first-run probe preferred the `.obsidian` marker over `_meta/repos.json`, picking the parent Obsidian app-vault over a nested MemoryVault subfolder; the probe now ranks the repos.json marker first and descends into a nested vault. Dogfooded by having the `documenter` sub-agent author ADR 0007's `## Amendment 2026-05-28` *through the resolver* — which surfaced + fixed the probe bug and a recall-budget truncation (documenter budget 4k→10k + project-first ordering so project decisions survive the cap). +37 tests (275 → 312 per OS workflow). Single-repo release; crickets ships the paired HLD update ([`5c49095`](https://github.com/alexherrero/crickets/commit/5c49095)) but stays at v2.1.0. Per ROADMAP-V4 item #35.  
...
Currently shipping **v4.6.0** — documenter vault-context resolution (V4 #35; single-repo MINOR). The documenter-side closure of the V4 #26 state migration: with per-project state at `<vault>/projects/<slug>/_harness/`, the doc-touching customizations now read operator conventions + project decisions from the vault rather than re-deriving them each invocation. New `documenter` recall pseudo-phase + `resolve_documenter_context(slug)` structured-bundle helper + `documenter-context` CLI (`--slug` / `--budget` / `--format text|json`; rc `0`/`1`/`2` = bundle / vault-unavailable / slug-not-registered) in `harness_memory.py`. The `documenter` sub-agent (pre-flight before scanning `wiki/`), the `wiki-author` skill (bundle surfaced in preview-before-write), and the `diataxis-author` skill (convention read via the resolver) all consume it, graceful-skipping to repo-local context on vault-unreachable. **Folds in v4.5.2** — `scripts/vault_probe.py` fixes the v4.5.1 first-run probe (it preferred the `.obsidian` marker over `_meta/repos.json`, picking a parent Obsidian app-vault over a nested MemoryVault subfolder); the probe now ranks repos.json first + descends into a nested vault. Dogfooded by having the `documenter` sub-agent author ADR 0007's `## Amendment 2026-05-28` through the resolver — which surfaced the probe bug + a recall-budget truncation (documenter budget 4k→10k + project-first ordering so project decisions survive). +37 tests (275 → 312 per OS workflow). Next ROADMAP picks: V4 #38 wiki bundle OR V4 #32 auto-detect bootstrap. Single-repo release; crickets ships the paired HLD update (`5c49095`) but stays at v2.1.0. See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
