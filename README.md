...
> **Latest:** v4.6.1 (2026-05-28) — **Installer hook-wiring repair + harness-context hook + doctor wiring probe** (V4 #39; single-repo PATCH). Fixes a v4.5.1/v4.6.0 regression: `install.sh --scope user` dropped hook dirs into `~/.claude/hooks/<name>/` but never merged their `settings-fragment-bash.json` into `~/.claude/settings.json` — so `settings.json` had no `hooks` block and **none of the 10 installed hooks fired on any device**. The user-scope install now merges each fragment + absolutizes the command to `bash <prefix>/hooks/<name>/<name>.sh` (`merge-settings-fragment.py` gained `--command`), and records each merged fragment's `{path, sha256}` in `.agentm-config.json` (additive field, no schema bump). New **`harness-context-session-start`** user-scope SessionStart hook injects the project's vault `PLAN.md`/`progress.md` paths at session boot (only when both exist; 500ms budget; never blocks). `/doctor` replaces its false-clean "absent hooks block is OK" with a real 7-row wiring truth table — hook-dirs-on-disk-but-not-wired now reports `[FAIL] … re-run install.sh`. +18 tests (312 → 330). **Operators must re-run `bash install.sh --scope user`** to pick up the fix (hooks stay dormant until then). Single-repo release; crickets stays at v2.1.0 (byte-identical `install_state.py` lib-sync, no release tag). Per ROADMAP-V4 item #39.  
...
Currently shipping **v4.6.1** — installer hook-wiring repair + harness-context hook + doctor wiring probe (V4 #39; single-repo PATCH). Fixes a v4.5.1/v4.6.0 regression where `install.sh --scope user` installed hook dirs into `~/.claude/hooks/<name>/` but never merged their `settings-fragment-bash.json` into `~/.claude/settings.json` — leaving no `hooks` block, so none of the 10 installed hooks fired. The user-scope install now merges each fragment with the command absolutized to `bash <prefix>/hooks/<name>/<name>.sh` (`merge-settings-fragment.py` gained `--command`) and records `{path, sha256}` per fragment in `.agentm-config.json` (additive; no schema bump). New `harness-context-session-start` user-scope SessionStart hook injects the project's vault `PLAN.md`/`progress.md` paths at session boot (degraded-graceful; never blocks). `/doctor` swaps its false-clean "absent hooks block is OK" for a 7-row wiring truth table — installed-but-unregistered hooks now `[FAIL] … re-run install.sh`. +18 tests (312 → 330 per OS workflow). **Operators re-run `bash install.sh --scope user`** to apply the fix. Next ROADMAP picks: V4 #38 wiki bundle / #40 crickets-plugins-consolidation / #32 auto-detect. Single-repo release; crickets stays at v2.1.0 (byte-identical `install_state.py` lib-sync, no release tag). See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
