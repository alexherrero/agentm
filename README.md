...
> **Latest:** v4.4.0 (2026-05-27) — **Wiki I/O codification + cross-repo views** (V4 #30 plan 2 of 3; single-repo). Builds on plan 1's `repo_registry` to ship the wiki I/O foundation that the upcoming V4 #38 wiki bundle (first sub-item of opinionated capability bundles meta) will build on. New `wiki-author` skill auto-fires on imperative phrases ("update the wiki", "document this in the wiki", "update <slug>'s wiki" for cross-repo) → dispatches the `documenter` sub-agent under a codified cross-repo write contract (registry membership + preview-before-write + per-repo `.diataxis-conventions.md` override). New `/recent-wiki-changes` slash command + `scripts/recent-wiki-changes.{sh,ps1}` for cross-repo visibility (7-day default; configurable via `AGENTM_WIKI_RECENT_DAYS` env). ADR 0004 gains Amendment 2026-05-27 codifying 3 wiki I/O conventions. Single-repo release; crickets stays at v2.1.0. Per ROADMAP-V4 item #30.  
...
Currently shipping **v4.4.0** — wiki I/O codification + cross-repo views (V4 #30 plan 2 of 3). New `wiki-author` skill auto-fires on imperative phrases ("update the wiki", "document this in the wiki", "update <slug>'s wiki") to dispatch the existing `documenter` sub-agent under a codified cross-repo write contract; new `/recent-wiki-changes` slash command walks `repo_registry.list_repos()` from plan 1 to surface recently-modified wiki pages across all registered repos. ADR 0004 Amendment 2026-05-27 codifies preview-before-write + per-repo `.diataxis-conventions.md` override + cross-repo target resolution. Foundation for the upcoming V4 #38 wiki bundle (first sub-item of opinionated capability bundles meta-item; pickup signal *"let's build the wiki bundle"*). Single-repo release; crickets unaffected at v2.1.0. See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
