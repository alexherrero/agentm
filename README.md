...
> **Latest:** v4.5.1 (2026-05-28) — **On-device agentm config + first-run vault detection** (V4 #30 follow-up; single-repo PATCH). Closes the V4 #30 promised-but-not-shipped first-run vault detection: `MEMORY_VAULT_PATH` had no source-of-truth file backing it on disk, so vault-aware scripts silently graceful-skipped when the env wasn't exported. After v4.5.1, `vault_path` lives in `~/.claude/.agentm-config.json` (the renamed + schema-v2-bumped install-state file) and the resolver consults it as fallback when env is unset (env still wins as override; backward-compat preserved). New `scripts/agentm_config.py` CLI (`--vault-path` / `--get` / `--list` / `--unset` ops) lets operators set the field without re-running the full installer. New `install.sh _agentm_vault_first_run_prompt` auto-detects Obsidian-vault candidates under `~/Library/CloudStorage/GoogleDrive-*/` on macOS (`-L` follows shortcut links + symlinks; `.shortcut-targets-by-id` reachable; 10s timeout). New `--force-vault-prompt` / `-ForceVaultPrompt` flag re-fires the prompt when vault_path is already set. 6 vault-aware shell scripts gained config-file fallback (previously env-only): `list-plans.{sh,ps1}`, `rename-vault-personal-projects.{sh,ps1}`, `migrate-harness-to-vault.sh`, `recent-wiki-changes.{sh,ps1}`. Doctor skill taught V4 #26 vault-state + V4 #30 user-scope reality. CI gap closed via `unittest discover` — coverage delta 214 → 275 tests per OS workflow. Single-repo release; crickets ships byte-identical `install_state.py` sync but stays at v2.1.0. Per ROADMAP-V4 item #30 follow-up.  
...
Currently shipping **v4.5.1** — on-device agentm config + first-run vault detection (V4 #30 follow-up; single-repo PATCH). Closes the V4 #30 promised-but-not-shipped first-run vault detection — `MEMORY_VAULT_PATH` now has an on-device source-of-truth at `~/.claude/.agentm-config.json::vault_path` (the renamed + schema-v2-bumped install-state file). Resolver ladder: `$MEMORY_VAULT_PATH` env (override) → `.agentm-config.json::vault_path` (new — primary) → None. New `scripts/agentm_config.py` operator CLI for set/get/list/unset of config fields; `install.sh --force-vault-prompt` flag + macOS-only candidate auto-detect under Google Drive (Drive shortcut links resolved via `-L` find + `.shortcut-targets-by-id` reachable; 10s timeout; prune `.Trash*` / `.tmp` / `.fseventsd` / `.Spotlight-V100`); 6 vault-aware shell scripts gained config-file fallback. Doctor skill taught V4 #26 vault-state + V4 #30 user-scope reality. CI auto-discover via `unittest discover` closes pre-v4.5.1 OS-matrix coverage gap — 214 → 275 tests per workflow. Backward-compat preserved: legacy `.agentm-install-state.json` filename auto-migrates on first interaction; env still wins as override. Next ROADMAP picks: V4 #35 (documenter vault-context resolution — already planned, `.harness/PLAN.deferred.20260528-v4-35-documenter-vault-context.md`) OR V4 #38 wiki bundle. Single-repo release; crickets ships byte-identical `install_state.py` sync but stays at v2.1.0. See [CHANGELOG.md](CHANGELOG.md) and the [latest release](https://github.com/alexherrero/agentm/releases/latest).
