...
6. **Dispatch crickets' `wiki-maintenance:documenter` sub-agent** (graceful-skip if crickets' `wiki-maintenance` plugin is absent) once `PLAN.md` is written to create `pending` Feature/Subsystem pages for tasks that affect user-visible behavior or architecture.
...
9. End with a ≤5-bullet summary to the user. Next command to run is `/work`.
