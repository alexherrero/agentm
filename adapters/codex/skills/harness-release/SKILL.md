...
7. **Offer next-release themes to the GitHub Project** (optional, per canonical spec §8). If this release's accumulated deferred items show a *recurring theme* (pattern across multiple sessions, not a single item), propose at most one project item via `gh project item-create` with title + body preview. Bar is higher than per-phase items. Silent-skip if `.harness/project.json` absent, `gh` unavailable, or no theme emerged. **No `gh` invocation without user confirmation.**
