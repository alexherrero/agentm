...
Start by reading `.harness/PLAN.md`, `.harness/progress.md`, and the project's `AGENTS.md`. Then work the unchecked tasks in sequence (or the one the user specified). Don't confirm each task upfront — run the safety pre-check before each task and only stop to ask when it fails or a clarification is needed.
