...
// log: a task's are the session's, and a nightly writer would race the session
// that owns them; a project's own tracker is generated by the dreaming binary's
// mocs job (task 176), and a second writer would race that.
//
...
// sessionFiles are never written by an enrichment pass, wherever they sit: the
// session's files, and the project tracker the mocs job generates.
