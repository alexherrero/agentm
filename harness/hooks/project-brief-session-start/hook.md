...
- **The brief** (at most 20 lines, each clipped to 160 characters): a header naming the project and task, the project tracker's State (two lines) and Next (one), the task tracker's status, State (four lines) and Next (three), the last three progress lines, the count of open follow-ups (`followups.md`, else `_harness/FOLLOWUPS.md`: unchecked boxes and table rows not marked ✅) and of `status: unfiled` cards carrying this `project:`, and the plan's path. The header asks the session to pass the project and task when it captures, which is how the in-session writer stamps them.
