...
- **Observability session brief (2026-07-17).** After the plan-state block, the hook runs `scripts/health/session_brief.py` (resolved as a sibling of `harness_memory.py`) and appends its single line: the latest digest headline + how long ago the last cycle ran, or a deadman `[agentm] ⚠ Observability — no digest in N days, ladder stalled` when the digest ladder has gone quiet. This is the autonomy design's [Delivery → Session-start line](../../../wiki/designs/agentm-autonomy.md) workhorse channel. It lives on **this** visible hook — not appended to the memory-recall hook, whose multi-KB always-load dump the host collapses into an unread `<persisted-output>` blob, so a briefing line there is emitted into context but never *seen*. The script self-resolves the vault + telemetry paths, honest-quiets when the ladder never ran, and anti-fatigues itself (a short cooldown + semantic signature) so `/clear`/resume in the same window doesn't repeat it. Own 2s budget, `|| true`-guarded — never blocks boot.
- **Leaves its plan block to the opening brief (agentm-vault plan 09).** While `project-brief-session-start` is registered in the user or project settings, this hook prints no plan block; that hook prints a tracker brief, or this block by running this hook with `AGENTM_PLAN_BLOCK_ONLY=1`, which prints the plan block and nothing after it. The observability line below still prints here.
