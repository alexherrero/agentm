...
### `Home.md` and the repo `README.md`

The wiki `Home.md` is the **canonical home page**, and the repo's root `README.md` mirrors it: the same opener, *Get started*, *Learn more*, and latest-release note carry across both. The README adds only the repo-local sections the wiki has no place for — **Contributing** and **License** (plus the shared banner / badges / "works with" line) — which is the whole of the difference between them. Two mechanical adjustments when mirroring Home → README: wiki-internal links (`Compatibility`, `Architecture`, …) become full `…/wiki/<Page>` URLs because the README renders on the repo page, not inside the wiki; and repo-local links (assets, `LICENSE`) stay relative. Home is the source of truth — when its opener, install, or release note changes, update the README's mirrored body in the same pass. A short HTML comment at the top of each README records this so a future editor doesn't let them drift.
