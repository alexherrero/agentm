...
- **Sub-agents** (required, harness-shipped): `adapt-evaluator, memory-idea-researcher` — the memory-engine sub-agents agentm keeps. Crickets-provided (graceful-skip if crickets is not paired, **never FAIL**): `adversarial-reviewer, adversarial-reviewer-cross, explorer` (code-review / developer-workflows — moved out of agentm in the V5 dev-loop slim) and `diataxis-evaluator, documenter, evaluator` (wiki — `documenter` retired from agentm in the seven-section convergence, canonical in crickets' `wiki` plugin).
