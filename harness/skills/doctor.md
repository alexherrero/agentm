...
7. **Machinery integrity (Consolidation follow-ups batch, machinery-integrity lane).** The prior checks all ask "is agentm's own harness distribution installed correctly"; this one asks the operator's own question — "how do we know all these structures are working consistently right now?" — over this repo's *own* dev-loop machinery (its Stop hook, its scheduled runner jobs, its cross-repo bridges), the exact class of thing that sat merged-but-never-installed for weeks in two separate confirmed incidents (the session-cost-capture hook in this repo; crickets' cross-review Gemini-fallback degrading silently). Run `python3 <agentm>/scripts/machinery_doctor.py` (only meaningful from inside an agentm dev checkout — skip with `[SKIP] not an agentm dev checkout` otherwise) and map each printed row by its status:
   ...
   Two rows read the **vault** rather than this repo: **`project-json-pointers:repo`** and **`project-json-pointers:vault`**. Each takes one `project.json` — the repo-local `.harness/` one, and the vault-resident `<memory-root>/desk/projects/<slug>/_harness/` one — and asks whether its path-valued keys (`items_source`, `env.MEMORY_VAULT_PATH`, `env.IDEAS_SURFACE_PATH`) still land inside the vault this machine resolves right now. `[FAIL]` covers two shapes: a path that is simply gone, and the nastier one that motivated the row — a path that still **exists** but sits outside the live vault, so reads succeed and quietly return a frozen tree. That happened on 2026-08-14, when both repos' configs still named the retired Google Drive root and served content up to 12 days stale without ever raising. `check-no-hardcoded-vault-path` cannot catch it: that gate prunes `.harness/` by name and only recognizes the retired cloud-drive mount literal, so a future move between two plain local roots would pass it untouched. Note `MEMORY_VAULT_PATH` and `items_source` are checked against `memory_root()` while `IDEAS_SURFACE_PATH` is checked against `vault_path()` — `Ideas.md` is the operator's own note one level above the memory tree, and collapsing the two surfaces would flag a correct install. Report-only; doctor never rewrites a config. Because `.harness/` is gitignored a linked worktree carries none, so discovery falls back to the main clone rather than emitting no row at all.
