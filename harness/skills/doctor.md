...
- **Phase commands**: agentm vendors **none**. `recent-wiki-changes` was the last one and retired to crickets' **wiki** plugin (2026-08-12) — report it `[OK] present` if crickets is paired, `[SKIP] not installed` if absent, **never FAIL**. The six phase-gated dev-loop commands (`bugfix, plan, release, review, setup, work`) moved to the crickets **development-lifecycle** plugin in the V5 dev-loop slim — report `[OK] present` if crickets is paired, `[SKIP] not installed` if absent, **never FAIL**. A bare agentm install has no dev-loop commands and that is the expected, healthy shape (DC-2: agentm is unaware of the dev loop, no pointer, no requirement).
- **Sub-agents** (required, harness-shipped): `adapt-evaluator, memory-idea-researcher` — the memory-engine sub-agents agentm keeps. Crickets-provided (graceful-skip if crickets is not paired, **never FAIL**): `adversarial-reviewer, adversarial-reviewer-cross, explorer` (code-review / development-lifecycle — moved out of agentm in the V5 dev-loop slim) and `diataxis-evaluator, documenter, evaluator` (wiki — `documenter` retired from agentm in the seven-section convergence, canonical in crickets' `wiki` plugin).
...
4. **State files (agentm-vault plan 15)**: A project's state root is its own directory. Resolve through the resolver, never by composing a path:
   - **List the plans** — invoke `python3 <agentm>/scripts/harness_memory.py list-plans --project-root <project>`. On a synced backend (the vault) it prints each open task's `tasks/<name>/plan.md` in the project's vault directory; for a project with no vault it prints the repo-local `.harness/PLAN.md` and each `PLAN-<name>.md`, skipping GDrive `*(conflicted copy*` files. Report the set, e.g. `state files [OK] vault-resident — 2 open tasks: tasks/042-build-the-brief, tasks/043-ship-the-brief`.
   - **Dangling active-plan marker** — if `<project>/.harness/active-plan` exists, confirm `python3 <agentm>/scripts/harness_memory.py resolve-active-plan --project-root <project>` exits 0. Exit 2 (the marker names no task or plan) is **`[WARN] .harness/active-plan -> <name> is dangling`** — never FAIL. This mirrors the session-start hook's non-fatal surfacing; `resolve_active_plan` is the loud-error enforcer at bind time, `doctor` is only the reporter. A bare call with no marker exits 4 on a project that keeps its plans in tasks; that is the expected shape, not a finding.
   - Report which mode resolved (e.g. `state files [OK] vault-resident — <vault>/projects/<slug>/tasks/` or `state files [OK] repo-local .harness/`).
   - An empty listing reads `[OK] no open plan`: a project whose tasks are all done is healthy. FAIL only when `list-plans` itself exits non-zero. A repo-local `.harness/` empty of plan files beside a healthy vault resolution is the expected shape.
...
7. **Machinery integrity (Consolidation follow-ups batch, machinery-integrity lane).** The prior checks all ask "is agentm's own harness distribution installed correctly"; this one asks the operator's own question — "how do we know all these structures are working consistently right now?" — over this repo's *own* dev-loop machinery (its Stop hook, its scheduled runner jobs, its cross-repo bridges), the exact class of thing that sat merged-but-never-installed for weeks in two separate confirmed incidents (the session-cost-capture hook in this repo; crickets' cross-review Gemini-fallback degrading silently). Run `python3 <agentm>/scripts/machinery_doctor.py` (only meaningful from inside an agentm dev checkout — skip with `[SKIP] not an agentm dev checkout` otherwise) and map each printed row by its status:
   ...
   One row reads the runner's own last cycle rather than a manifest: **`runner-cycle`** (`~/.cache/agentm/runner/last-cycle.json`, left by every `agentm-runner run`). `[OK]` names how many manifests loaded and ran; `[FAIL]` names the manifests the cycle refused — a half-quoted command, a `tier: T1` — which since filing-v2 remainders task 1 stop nothing else (the other jobs ran; the session brief carries the same line); `[UNVERIFIED]` means no cycle has run on this machine yet. A job whose *own* manifest was refused also FAILs on its own row with the reason.

   One row reads the **user-scope** config rather than this repo's: **`unattended-merge-gate`** (V8-proving item 19). When the `n1-overnight` unattended-dispatch job is registered in `.harness/jobs/`, it reads `~/.claude/settings.json` and checks that `Bash(gh pr merge:*)` resolves to `allow` — not `ask`/`deny` — so an unattended merge-on-green run doesn't stall at the final `gh pr merge`. `[OK]` when allowed (or when no such job is registered — the CI case, since `.harness/` is gitignored, so this never flags there); `[WARN]` with a copy-pasteable remedy (`scripts/enable-unattended-merge.sh`) when the gate would block. It is **report-only** — doctor never writes the global file (agentm's install.sh doesn't touch the global permission surface at all; provisioning is the operator's dev-setup dotfiles' job, which owns that `permissions` block). The `deny > ask > allow` precedence it assumes is the design's own re-audit trigger — see `wiki/designs/agentm-autonomy.md` (amendment log, 2026-07-15).

   Two rows read the **vault** rather than this repo: **`project-json-pointers:repo`** and **`project-json-pointers:vault`**. Each takes one `project.json` — the repo-local `.harness/` one, and the vault-resident one in the project directory's `desk/` — and asks whether its path-valued keys (`items_source`, `env.MEMORY_ROOT`) still land inside the vault this machine resolves right now. `[FAIL]` covers two shapes: a path that is simply gone, and the nastier one that motivated the row — a path that still **exists** but sits outside the live vault, so reads succeed and quietly return a frozen tree. That happened on 2026-08-14, when both repos' configs still named the retired Google Drive root and served content up to 12 days stale without ever raising. `check-no-hardcoded-vault-path` cannot catch it: that gate prunes `.harness/` by name and only recognizes the retired cloud-drive mount literal, so a future move between two plain local roots would pass it untouched. Note `MEMORY_ROOT` (and its deprecated alias) is checked against `memory_root()` while `items_source` is checked against `vault_path()` — the project space sits beside the memory tree since filing-v2 2b, and collapsing the two surfaces would flag a correct install. `IDEAS_SURFACE_PATH` is no longer checked: nothing reads it since `Ideas.md` became a file the dreaming binary generates (agentm-vault part 13). Report-only; doctor never rewrites a config. Because `.harness/` is gitignored a linked worktree carries none, so discovery falls back to the main clone rather than emitting no row at all.

   One row reads the **projects space** for a directory that should not exist: **`harness-dirs`** (agentm-vault plan 15). A project's state root is its own directory — tasks under `tasks/`, machine files under `desk/` — and the retired per-project state directory must not come back. `[FAIL]` names any such directory at any depth under `projects/`, and any project for which the plan resolver, asked bare or by each task, still answers a path inside one; `[OK]` counts the projects and the answers checked; `[UNVERIFIED]` means no vault resolves on this machine. The code half of the same guard is `check-no-harness-paths`, which fails on the literal in tracked code.
...
**Graceful-skip if not installed.** `explorer` moved to the crickets development-lifecycle plugin in the V5 dev-loop slim. If the sub-agent isn't present in any host's agent paths, report **skip** with reason: *"explorer sub-agent not found — install the crickets development-lifecycle plugin to enable this probe."* Never FAIL on its absence — a bare agentm install legitimately has no `explorer` (DC-2).
...
doctor: <adapter> — <PASS|FAIL>
  ...
  live probes (--live):
    explorer          [SKIP] crickets development-lifecycle not installed — probe needs the explorer sub-agent
