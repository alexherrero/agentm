...
**On request only.** The phase-close trigger that staged a marker per
session at `post-work` and `post-release` retired in agentm-vault plan 04: it
left 52 markers nobody ever composed, and the design has since ruled that
crystallize is never a session's act — it is a weekly dreaming phase reading
closed work for recurrence (plan 11), and a synthesis written on request. This
module is the on-request half: `crystallize_exploration(vault_path, slug,
digest)` and the `write` / `read` CLI below.
