...
Public surface:
    ...
    confirm(vault_path, run_id, index, revert_log, *, ttl_days=30, now=None) -> str
        ...
        index. Returns the revert-log entry id.

    expire_stale(vault_path, run_id, *, ttl_days=30, now=None) -> list[int]
        Marks every still-pending proposal past its TTL as expired (a
        pure state transition — expired proposals are never applied by
        this call or any other path) and returns their indices.
...
def expire_stale(vault_path: Path, run_id: str, *, ttl_days: float = DEFAULT_TTL_DAYS, now: float | None = None) -> list:
    ...
    (that raises `ExpiredProposalError` instead)."""
    now = now if now is not None else time.time()
    manifest = _load_manifest(vault_path, run_id)
    state = _load_state(vault_path, run_id)
    state.setdefault("_manifest_staged_at", manifest["staged_at"])
