...
#### Since filing v2's write path: the review queue is a reading, not a directory

Nothing writes to `_inbox/` any more (2026-09-04). A reflect candidate below HIGH confidence is filed at its class directory flagged `filing_confidence: low`; a capture lands `status: unfiled`; a probable duplicate carries `review_flags` and `related`. The review surface is the generated MOC at `<memory-root>/memory/mocs/needs-review.md` — one line per note with a context phrase saying why it is there — regenerated by the weekly dream cycle and on demand:

```
python3 ~/Antigravity/agentm/harness/skills/memory/scripts/needs_review.py --vault <memory-root> --write
```

An entry clears when the note is re-judged: enrichment raises it to `active` at high confidence, the operator edits the stamp, or a later note supersedes it. The scorecard's "needs review" line carries the same count. The triage engine below still runs against a legacy `_inbox/` while one exists and is otherwise a no-op; re-pointing its promote / merge / expire dispositions at the flagged population is a recorded follow-up.
...
**The dreaming binary** (`agentmdream`, built beside `agentmd` by `install.sh`) is the second Go binary the design names: one pass and exit, under a dual gate — enough time since the last pass (`-every`, 168h) *and* activity since (captures in the index, genuine recalls in the recall history) — behind a lock a second start is refused by (exit 3), with every mutation journaled to `<engine state dir>/dreaming/journal.jsonl` before it is made and resumed from that journal after a crash without loss or double application. Report-only by default (`agentmdream run` decides and prints; `-apply` writes; `-force` skips the gate); `agentmdream status` and `agentmdream journal` read the state. Its first job is the lifecycle policy above, ported byte for byte and writing the same lifecycle journal; the Python layer keeps running beside it until the parity gate flips the takeover. The runner triggers it through `templates/jobs/dreaming.yaml`.

**Its jobs, batch 1** (`daemon/internal/dreaming`): after the lifecycle lane, the pass collapses content-identical copies (`copies` — the port of the Python suffix-backlog drain: every `status: active` memory bucketed by its live body fingerprint, each family collapsed into its earliest note, the copies marked `status: superseded` + `supersedes: <canonical>`, never deleted, at most 25 families a pass), re-files what write time got wrong (`refile` — a memory whose `type:` the contract routes to another class moves there under the same basename, a taken basename is reported, a `near-duplicate` flag whose twin no longer exists is cleared), and promotes what recurs (`promote` — the port of `consolidate.py`: a target three or more distinct episodic notes link becomes `memory/crystallized/consolidated-<slug>.md` with `kind: crystallized`, `lifecycle_tier: durable`, and its sources in `consolidated_from` and `derived_from`; an entry that exists is never overwritten). A crystallized note without a non-empty `consolidated_from` fails `check-vault-frontmatter`. Parity with the Python layer is asserted against a recording, never recomputed: `scripts/fixtures/dreaming-parity/` holds a scratch corpus and the `expected.json` that `scripts/health/record_dreaming_parity.py` recorded from the Python producers with the clock pinned; the Go tests reproduce it and `test_dreaming_parity.py` fails if the Python layer drifts. Re-record only on purpose.
...
**The overlap and the flip.** The binary runs report-only beside the Python layer until the operator flips the takeover, and the flip is one change to two manifests: `-apply` on `agentmdream run` in `templates/jobs/dreaming.yaml` and `--skip-ported` on `dream.py` in `templates/jobs/dream.yaml`, so exactly one layer mutates each lane. Under `--skip-ported` the Python cycle steps aside from the three lanes the binary carries — the suffix-backlog drain (`copies`), the calendar rollups (`calendar`) and the lifecycle policy (`lifecycle`) — and its digest says so (`ported: …`); the stages the binary does not carry (lint repair, compression, the artifact shelf, inbox triage, the needs-review MOC, insights, the confirm-gated proposals) keep running. During the overlap `templates/jobs/dreaming-divergence.yaml` runs `scripts/health/dreaming_divergence.py --write` daily: both layers over a copy of the memory root, every decision they disagree on written verbatim to `<memory root>/diagnostics/dreaming/divergence-<date>.md` — counts and paths, no disposition; the window closes when the operator has dispositioned the divergences in writing. `scripts/check-dreaming-parity.sh` guards the recording in the battery and CI: the Go planners must reproduce `expected.json` and the Python producers must still produce it, so a drift in either layer is loud and a re-recording is a decision. Rollback is the reverse edit — the Python layer is still there.
