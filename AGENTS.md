...
It runs the unit suite (`scripts/test_*.py`) + every `check-*` gate (syntax · references · adapters · parity · vendored-parity (CONS-1 merge of the old lib-parity/vault-lock-parity scripts — lib + vault-lock + corpus-gate + storage-seam + hook-config modes) · multi-plan-naming · worktree-slug · no-auto-worktree · one-way-imports (CONS-1 merge of the old process-seam-import-direction/capability-resolver-one-way scripts — opinion-resolver + capability-resolver + process-seam + lc8-storage-vault + lc8-bridge rules) · storage-seam-no-path-leak · opinion-honesty · personas · governs-index · no-hardcoded-vault-path · vault-frontmatter (strict YAML parse over every note under the vault root, not only `agent/`, and never behind the recall wall) · payload-layout-free · payload-parity · no-pii · wiki · slop · kind-taxonomy (advisory, report-only)) + every `verify-*` integration check and the health-scorecard checks (`verify-v4` · `verify-orchestration-briefing` · `verify-hook-resolution` · `verify-state-routing` · `verify-reflection` · `verify-dreaming` · `verify-phases` · `verify-memory-roundtrip` · `health-score-determinism` · `validate-audit-coverage` · `run-ablation-baseline`, against throwaway scratch vaults), prints a PASS/FAIL table, and exits non-zero if any gate fails. CI additionally runs the heavier `smoke-install` + `gitleaks` on every push. As the project grows, add new checks to `check-all.sh` (and a CI step) so the battery stays the single source of truth for "is it green." Full reference: [wiki/reference/CI-Gates.md](wiki/reference/CI-Gates.md).
...
When a plan closes out (the last task goes `[x]`, `/release` runs its close-out step), the completed `PLAN.md` and the rest of that close-out's done artifacts move into an `archive/` subdirectory of the active working directory — `archive/PLAN.archive.YYYYMMDD-<slug>.md`, not a flat `PLAN.archive.YYYYMMDD-<slug>.md` sitting directly alongside the still-active files. Done work moves one level deeper into its own `archive/`, so the eyeline at the top level stays a small, current set of active files rather than accumulating every past plan flat alongside them. A plan kept as a task in the vault (`tasks/NNN-<slug>/`) never moves at close-out: its tracker says it is done, and the nightly job moves its finished records.
