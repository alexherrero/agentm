...
It runs the unit suite (`scripts/test_*.py`) + every `check-*` gate (syntax · references · adapters · parity · vendored-parity (CONS-1 merge of the old lib-parity/vault-lock-parity/workflow-parity scripts — lib + vault-lock + wiki-publish-transform + storage-seam + hook-config + workflow modes) · multi-plan-naming · worktree-slug · no-auto-worktree · one-way-imports (CONS-1 merge of the old process-seam-import-direction/capability-resolver-one-way scripts — opinion-resolver + capability-resolver + process-seam + lc8-storage-vault + lc8-bridge rules) · storage-seam-no-path-leak · opinion-honesty · personas · governs-index · no-hardcoded-vault-path · vault-frontmatter (strict YAML parse over every vault note, `_harness/` included) · no-pii · wiki · slop · kind-taxonomy (advisory, report-only)) + every `verify-*` integration check and the health-scorecard checks (`verify-v4` · `verify-orchestration-briefing` · `verify-hook-resolution` · `verify-state-routing` · `verify-vec-index` · `verify-reflection` · `verify-dreaming` · `verify-mcp-surface` · `verify-phases` · `verify-memory-roundtrip` · `health-score-determinism` · `validate-audit-coverage` · `run-ablation-baseline`, against throwaway scratch vaults), prints a PASS/FAIL table, and exits non-zero if any gate fails. CI additionally runs the heavier `smoke-install` + `gitleaks` on every push. As the project grows, add new checks to `check-all.sh` (and a CI step) so the battery stays the single source of truth for "is it green." Full reference: [wiki/reference/CI-Gates.md](wiki/reference/CI-Gates.md).
