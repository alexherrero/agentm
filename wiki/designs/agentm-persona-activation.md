...
- **Partly done at build:** extended `check-personas.py` for the four axes; wrote the adoption path (`persona_resolve.py` / `adopt`); taught `install.sh` to dispatch `personas/`. **Done (2026-07-06):** renamed the persona file `personas/rememberer.md` → `personas/brain.md` (frontmatter `name:` included), with its test fixture (`test_persona_resolve.py`) and the reference pages that named it (`persona-tier-schema.md`, `Completed-Features.md`) updated to match. **Still open:** adding the four new axes to the two existing manifests (`brain`, `team-coordinator`) — a separate code change, out of scope for the rename.
...
- **2026-07-06 — `rememberer.md` → `brain.md` rename closed.** The rename this build's entry (below) deliberately left out of scope is now done: `personas/rememberer.md` → `personas/brain.md`, with the test fixture and reference pages updated to match. The four-manifest-axes item stays open.
