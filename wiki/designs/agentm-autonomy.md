...
- **Session-start line — the workhorse.** The SessionStart hook already assembles a briefing at every session open, and it gains one line: the day's headline, the needs-your-eye count, and how long ago the last cycle ran. Its deadman variant is the point. When no digest has landed in several days, the line says so (*"no digest in 4 days — ladder stalled"*), so a stalled runner surfaces in your next session instead of staying dark for days. This is a surface you already see, and it needs no push infrastructure.
