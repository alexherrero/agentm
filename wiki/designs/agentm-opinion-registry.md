...
- **Invariants committed at the boundary from the start** — base always recoverable, supplement fully discardable, opinion changes revertable (operator ruling) — and both the supplement *writer* (the Experience loop, Stages 2-3 shipped 2026-07-25, [PR #380](https://github.com/alexherrero/agentm/pull/380)) and the *revert-log* primitive it unifies with (shipped 2026-07-07, V6-4) are now built. The registry only reads the supplement, so it is unaffected; the revertability invariant is enforceable now that the memory-system revert-log has landed.
...
- **2026-07-25 — Risks + Locked design calls reconciled to Experience's Stages 2-3 ship (v9.3.0 release sweep, [PR #380](https://github.com/alexherrero/agentm/pull/380) / [#381](https://github.com/alexherrero/agentm/pull/381)).** Three places in this design still called the accumulate loop's writer `[PENDING-IMPL]` after it shipped: the shared-snapshot risk's re-audit trigger, the "invariants committed at the boundary" locked call (which also called the revert-log primitive unbuilt — it shipped 2026-07-07, V6-4), and the rejected runtime-bridge alternative's reasoning. All three reconciled to as-built; no substance change to what was decided, only to what's now true. Left open on purpose: the cross-plugin consumer-side runtime-overlay wiring itself is still unbuilt — a separate task from Experience shipping the writer. *Re-audit trigger:* none — a tense reconciliation, not a decision.
- **2026-07-07 — intro + Objective brought current with the Wave-B ship (G12 AG close-out sweep).** The NOTE header and the Objective still read in the future tense ("This design specifies it"; "the one piece the opinions design leaves as `[PENDING-IMPL]`") even though the resolver shipped 2026-07-06 (the "built (AG Wave B leader 2/5)" entry below). Reconciled both to past tense — the compose-and-serve request-by-name path is built (`scripts/opinion_resolver.py`); the registry is no longer a spec awaiting implementation. No substance change; the cross-references to the model-effort tier scale and the Experience supplement (both still `[PENDING-IMPL]`) are unaffected. *Re-audit trigger:* none — a tense reconciliation, not a decision.
- **2026-07-06 — markdown-prose consumer grammar locked (AG Wave D follow-on).** Wave D's `PLAN-wave-d-opinion-wiring` build found only 2 of the 9 registered opinions had a real code binding with a runtime call site (`diagnostics`→*how-we-engineer*, `development-lifecycle` routed-dispatch→*efficient*, both shipped); the other 7 are static markdown skill/command files with no runtime hook, exactly the gap `check-opinion-honesty.py`'s own docstring flagged as deferred "until that grammar is locked." This closes it, superseding the "likely build-time interpolation" hypothesis the entry below queued as a follow-up. A capability/skill now declares via a dedicated `opinions:` frontmatter list; delivery splits on the opinion's own `implements:` pointer into **self-provider** (the caller already is the standard's home artifact — stays hardwired, a stub-tracks-plugin drift check replaces any runtime request) and **genuine cross-plugin reuse** (baked into the caller's `dist/` at `generate.py`-time from a committed, parity-gated snapshot — never a live cross-repo or vault read, so it survives crickets' standalone-install model and a vault-less, agentm-less CI).
...
- **2026-07-06 — first two consumer wirings land (AG Wave D, `PLAN-wave-d-opinion-wiring`); confirms the grammar gap this design already flagged.** `diagnostics` (`how-we-engineer`) and `development-lifecycle`'s routed-dispatch layer (`efficient`, replacing its `find_capability`-mediated read of token-audit's routing table) now call `opinion_resolve()` for real — both are code with a genuine render/read step to hook into. The other 7 originally-planned bindings (`code-review`, `research`, `maintenance`, `wiki`, `design` ×2, `development-lifecycle`'s own direct `done`/`how-we-engineer` bindings) stay hardwired: they're static markdown skill/command files with no runtime hook, and this design's own 07-06 entry above already named the underlying gap (a capability/skill `requires:`/`enhances:` reference "stays undetectable until that grammar is co-designed"). `privacy`'s `privacy-review` binding is deferred separately — the skill itself doesn't exist yet (picked up when `PLAN-wave-d-tokens-and-privacy` builds it); `token-audit` was dropped from the consumer set entirely — it's `efficient`'s one-way measurement provider, not a consumer, and was a modeling error in the original wiring survey, not a real gap. A follow-up design task was queued (`task_a2bed2b9`) to lock the markdown-prose consumer grammar — the entry above is that follow-up landing.
