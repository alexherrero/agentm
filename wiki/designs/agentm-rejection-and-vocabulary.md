...
| 2026-08-16 | **§5's arc-close gate is held, and the hold that was already on it had quietly expired.** Operator decision the same day the vocabulary thread closed. The gate buys one agent-layer run at ~$50.68 and ~3 hours against two clauses, and its queued plan has carried a hold since 2026-08-15 whose release condition read *"do not buy this run until `alias-pilot` has produced its deterministic retrieval-layer number."* That condition has been met — `alias-pilot` ran, and so did `alias-pilot-structural` and `path-signal`, all three refuted at 0 conversions — so the stated hold now reads as satisfied while the reason for it has only strengthened. **Thread B shipped no change at all, and Thread A's elicitation rung was reverted, so this run would re-measure a system byte-identical to the one that scored 0.6799.** The queued plan's own arithmetic, computed from the recorded replicates rather than estimated, already showed both clauses mis-priced: clause 2's ≥90% needs 58 of 64 while the eight targets are already hit 3.33 of 8 at the agent layer, leaving 4.67 questions of real headroom and a ceiling near 85.4%; clause 1's ≥0.725 clears only if a gold-blind engine matches the gold-informed oracle's full ceiling, which `pp07` measured false. **New release condition:** the successor retrieval-competition arc must land at least one rung that moves the deterministic retrieval-layer number, and both clauses are then re-priced against that new column before the run is bought. **What replaces this thread, and why it is not a fourth vocabulary rung:** the three nulls share one measured cause — the distinguishing term is present in the index and the target note is outranked — so the successor arc works ranking rather than lexicon, in five sections: chunk-level lexical indexing with its aggregation chosen from a pre-flight probe rather than from the literature default (this corpus has already measured max-over-chunks *causing* the length subsidy it would be meant to cure); a durable off-gold probe set followed by Vector-PRF on the dense arm; an embedder-swap probe; HyDE bounded to the deliberate path by the layering rule; and, only if the first four leave the oracle's eight unconverted, alias generation gated by an **outcome** filter rather than the relevance filter the SIGIR 2024 reproducibility study found harms recall-based metrics. Brief at `<vault>/Agent/desk/projects/agentm/_harness/BRIEF-retrieval-competition.md`. **One clause shape changes for every rung of that arc**, inherited from this one's last failure: a prediction is scored in two halves, positive and negative, reported separately, and a positive half of 0 of N refutes the rung regardless of the negative half — `path-signal` passed clause (c) at 6 of 8 on six correct predictions that nothing would happen. Re-audit triggers: any rung landing a deterministic gain re-opens the pricing of both clauses here, and a successor arc that also fails to move the number should re-price the gate against retirement rather than against a later run. | final |
