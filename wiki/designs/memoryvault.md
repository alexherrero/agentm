...
**Base-skill dependencies (all shipped via prior plans):** [#3 evaluator](https://github.com/alexherrero/crickets/wiki/crickets-development-lifecycle) (per-step review), [#4 kill-switch + steer](https://github.com/alexherrero/crickets/wiki/crickets-developer-safety) (long-running execution control), [#5 commit-on-stop](https://github.com/alexherrero/crickets/wiki/crickets-developer-safety) (crashed-session recovery), [#6 design skill](https://github.com/alexherrero/crickets/wiki/crickets-development-lifecycle) (this design doc is the first real dogfood of that skill).
...
- [#3 fresh-context evaluator](https://github.com/alexherrero/crickets/wiki/crickets-development-lifecycle) — consumed by reflection sidecar's optional per-entry grading.
- [#4 kill-switch + steer](https://github.com/alexherrero/crickets/wiki/crickets-developer-safety) — long-running execution control (reflection sidecar respects `.harness/STOP`).
- [#5 commit-on-stop](https://github.com/alexherrero/crickets/wiki/crickets-developer-safety) — crashed-session safety branch (orthogonal but complementary to MemoryVault crash recovery — commit-on-stop handles dirty git trees, MemoryVault crash markers handle missed reflection).
- [#6 design skill](https://github.com/alexherrero/crickets/wiki/crickets-development-lifecycle) — this design doc is the first real dogfood.

**External**:
- *(historical: v1 spec listed Anthropic API for embeddings; v0.9.2 dropped this — see [crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld). `sentence-transformers` below is now the only embedding path.)*
...
2. **Network surface**: the agent never makes the vault contents accessible via network. As of v0.9.2, MemoryVault makes **zero external network calls** during normal operation — embeddings are computed entirely on-device via local `sentence-transformers` (see [crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld)). The only network access is the one-time model download (~1.3GB BGE-large from HuggingFace Hub) on first invocation, after which the toolkit is fully offline-capable.
...
- **Local-embedding failure** (rare — sentence-transformers not installed, or PyTorch MPS regression): save still succeeds (file write is unconditional); embedding queue stays pending until deps are restored. UserPromptSubmit hook falls back to grep+frontmatter when sentence-transformers is unavailable. No external network dependency exists post-v0.9.2 ([crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld)) so there's no rate-limit / API-failure class of incident.
...
Vault contents are PRIVATE — assumed to contain PII. Storage = user's local + synced storage (user's account). Embeddings are computed entirely on-device via local `sentence-transformers` ([crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld)) — no external API calls. If sentence-transformers is unavailable (not installed, or PyTorch MPS issue), recall degrades to grep+frontmatter-only.
...
- **Smoke install tests** (existing `smoke-install-bash.sh` + `.ps1`): extended to verify `memory` skill + 4 hooks install correctly at the 2 host destinations (Claude Code + Antigravity; gemini-cli removed in v0.9.0 per [ROADMAP item #15](https://github.com/alexherrero/agentm/blob/main/.harness/ROADMAP.md) / [crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld)).
...
7. (L) Recall engine — sqlite-vec + grep+frontmatter merge + local embedding integration (BGE-large via sentence-transformers; see [crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld) for the v0.9.2 local-only refactor).
8. (M) Idea ledger — `Ideas.md` + `_idea-incubator/` two-tier capture + permeable boundary enforcement.
9. (S) Documentation pass — how-to + amendment log + cross-refs.
...
4. (S) Documentation pass — how-to update + amendment log.
...
4. **Re-audit triggers**: after 1 month of real use, re-audit the locked design calls. Capture-threshold + recall-quality + cross-device sync conflicts get re-evaluated; amendment log updated if any decisions flipped.

No feature flags; no phased rollout to user segments (single user). The escape hatches are: `memory.review_mode: silent` (cuts MEDIUM-tier prompts), `AGENT_TOOLKIT_EMBEDDING_MODEL` env var (swap default BGE-large for a smaller model on low-spec hosts — added in v0.9.2 per [crickets HLD](https://github.com/alexherrero/crickets/wiki/crickets-hld); still local-only — no API option), `memory.enabled: false` (kill switch — disables all hooks + auto-recall, vault becomes read-only).
...
**2026-05-15 — Initial design locked.** Four-group architectural session (A1-A4/B1-B3/C1-C4/D1-D3) settled 13 design calls covering write primitives, reflection sidecar, recall engine, idea ledger, seed pass, and discovery mining. Translated to 6 implementation parts (write-primitives, recall-loop, reflection-and-recovery, idea-ledger, seed-pass, discovery-mining), sequenced topologically.

**2026-05-16 — Host scope narrowed to `[claude-code, antigravity]`.** `memory` skill ships without gemini-cli in `supported_hosts`; fleet-wide sweep (2026-05-17) aligned all existing customizations to the same 2-host scope. Why not ship with 3 hosts: ROADMAP #15 was already removing gemini-cli — shipping then re-sweeping would be redundant churn. Re-audit: gemini-cli re-added to the toolkit.

**2026-05-20 — Embedding-mode collapsed to local-only (v0.9.2).** DC C2 superseded: dual-mode API + local → local `sentence-transformers` only. Default model upgraded `all-MiniLM-L6-v2` (384-d, MTEB 56.3) → `BAAI/bge-large-en-v1.5` (1024-d, MTEB 64.2); `EMBEDDING_DIM` 384→1024; `AGENT_TOOLKIT_EMBEDDING_MODEL` env var escape hatch (still local). Why not retain API path: primary operator is a Claude Ultra subscriber without a separate embedding API key; desktop-class hardware (M-series + 64GB-RAM) makes local quality the better default. Re-audit: host drops below M-series floor; a local model exceeds BGE-large at lower footprint; Anthropic ships a materially better API embedding path.

**2026-05-22 — Discovery + mining shipped (crickets v0.10.0, #7b).** Five sub-commands: `/memory index-skills`, `/memory reflect corpus`, `/memory discover-skills`, `/memory adapt-skills`, `/memory watchlist`. `adapt-evaluator` sub-agent (write-scoped to `_skill-watchlist/`) enforces adapt-don't-import; stdlib-only, no new deps. Why not fork-and-import: forking external skills would erode the "no external skill execution" boundary — adapt-don't-import is the enforced boundary. Re-audit: operator-confirmed 4-source whitelist grows stale; cadence or dry-run defaults need adjustment.

**2026-06-25 — Migrated to launched living design (AG Phase 3).** `status: final`→`launched`; AG graph keys added; Document History table collapsed to amendment log; `(former ADR XXXX)` chain-pointers stripped; dead TLS/API-key embedding residue removed; internal provenance removed from body.
