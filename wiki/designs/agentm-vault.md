...
This design; the pointer row and note in `agentm-filing-v2.md`; `Designs.md` and `_Sidebar.md` entries; the vault's `index.md` rewritten when `Filing.md` folds in; `reference/Memory-Daemon.md` for the probe's root; `reference/CI-Gates.md` for the new gates; the how-to for the vault layout once the casing lands; `Capture` reconciled by session 2; `Experience & Dreaming` pointed here by session 3 for the Python cycle's retirements and the crystallization trigger, its body reconciled in the landing group that retires the stages; `reference/Memory-Daemon.md` for the batch's flags and the tier wiring; the runner's how-to for `window:`.
...
The daily corpus scorecard gains: residue count (zero after the purge, and a line that stays), class populations, the card-shape gate's count of non-conforming notes, empty directories under `agent/`, and the health-pass cadence itself. The morning note carries the night's spend against its token line, the steps skipped and why, and the week's sank list; a night that ran past its window is a line in it.
