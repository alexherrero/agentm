...
|---|---|---|
| `agentmd` | index, retrieval, MCP surface, capture, enrichment, the probe | field stamps, the card shape, ranking by importance in permanent spaces, the probe's root; the tool's four capture fields, one meaning for `status`, enrichment owed by stamp, the eager trigger removed, the tally gate; the batch reads the tier table, the two-shape prompt, usage printed (session 3) |
| `agentmdream` | the nightly mutation pass, maps, rollups | map naming and pagination, empty-rollup skip, deep-once enrichment state; `-every 24h`, promote reads Captured and Candidates and writes semantic candidates, a report pass leaves the clock alone (session 3) |
...
├── projects/       <slug>/ charter.md tracker.md followups.md · decisions/ designs/ research/ · tasks/<verb-slug>/ archive/ desk/ · moc-<slug>.md
...
**One morning note.** `agent/diagnostics/morning/YYYY-MM-DD.md` with a `latest` mirror, generated after the last step of the night, and it is the body of the daily email. It replaces the dreaming scorecard and the observability digest email. Four sections, each omitted when empty: *what ran* — the binary's seven rows, enrichment as judged · filed · below the floor · sank · calls · tokens · model, the Python proposals by section, and which steps were skipped and why; *what needs you* — the needs-review sections with counts and the first five items each, the binary's archive candidates, and the week's sank list, which is the contract's "weekly digest of what quietly sank"; *the corpus in one line*, with a link to the day's corpus scorecard, whose parked daily job is revived; and *spend*, last night's tokens and the seven-day total. The observability ladder stops writing a digest that would say "$0 · 0 events", the same rule the calendar's empty reviews got, and its spend line joins the fourth section when it is not zero. The session brief line reads the morning note's first section. The daemon's queue-age alert keeps its three-day threshold; after the first batch it is measuring something real. *Why one note and not the seven surfaces:* on 2026-09-06 the scorecard named a fixture, the digest said nothing, and the brief counted markers nobody would ever compose; a page you read has to be one page.
...
**The project skeleton.** `charter.md`, `tracker.md`, `followups.md` and `roadmap.md` (only where a project has one) at the root, beside the generated `moc-<slug>.md`; then `decisions/`, `designs/`, `research/`, `drafts/` (only where a project drafts), `tasks/`, `archive/` and `desk/`. The charter is yours — what, why, where, what to read first — and the machinery never rewrites it; `_index.md` is renamed to `charter.md` because that is work's name and the two vaults converge. `desk/` holds only what you do not open: the board mirror, `features.json`, `corrections.md`, session markers, and briefs that have no task yet. *Why the tracker and the followups at the root and not in `desk/` as at work:* they are two of the three files you open, and they sit with the third. *Why no `plans/`:* a plan at home always belongs to a task; a directory is discovered, never conjured. *Re-audit* if a project accumulates plan documents that are not tasks — then `plans/` returns for that project.

**`_harness/` dissolves; nothing renames to `desk/`.** Every file moves once, by a table rather than a judgment. A `PLAN-<slug>.md` and its `progress-<slug>.md` become `tasks/<verb-slug>/plan.md` and `progress.md`, with a `tracker.md` written from the Status line and the Goal. A queued plan becomes a task whose tracker says `queued`, and `queued-plans/` goes. An archived plan becomes `archive/tasks/<slug>/` with a tracker at `done` and `closed:` from its archive date. Briefs, prompts and probes go into their task as `brief.md`, or into `desk/briefs/` when no task exists. `designs/` and the research bundles merge into the project's `designs/` and `research/`. `FOLLOWUPS.md` becomes `followups.md`. `ROADMAP-MASTER.md` becomes `roadmap.md`, with `ROADMAP.md`'s open table folded in by hand while you read the diff. The board and feature ledgers go to `desk/`, the eight handoff files to `archive/`. This series becomes `tasks/refine-the-vault/`, the first task in the new shape. The repo-local copies (`init.sh`, `verify.sh`, `known-migrations.md`, `.project-mode`) stay where the repo reads them. Task slugs are verb-first, derived from today's plan slugs and listed for your correction before the migration runs; the naming gate holds them after. Nothing is deleted, the structural-move tooling rewrites the links, and `agentmd embed` closes the move.
...
**Five statuses, and what each transition writes.** `queued` (shaped, no session has worked it), `active`, `parked` (stopped on purpose, the reason in State), `done`, `dropped` (withdrawn, the reason in Outcome); today's nine spellings map onto them in the migration. Open, at `/plan`: the directory, the plan, the tracker at `queued` with Objective from the Goal and Next as step one, a progress line. First step, at `/work`: `active`, State and Next rewritten, a progress line per step as today. Park and resume, on your word: the status, the reason, a line. Close, at the last step or at `/release`: `done` and `closed:`; the Outcome, which is the close-out recap the session already writes; the close-out progress line; the board row to Done; and the directory moved whole to `archive/tasks/<verb-slug>/`. Drop: the same with `dropped`. The project's tracker is rewritten at every task open and close — its State names the active tasks, its Next the next one — and whenever what the project is for changes. *Why close moves the directory:* agentm holds forty-six closed units against one open, and the folder you open should show the one; work's flatten into `drafts/` would collide with blog's real drafts and lose the directory that kept a task's artifacts together. This resolves session 1's forty-directory re-audit by construction. *Re-audit* if you find yourself opening `archive/tasks/` daily — a `recent` view in the map is the answer, not moving them back.

**A decision is a note with a kind; a handoff is a tracker.** A `decisions/` note carries `kind: decision` and one ruling, with its alternative not taken and its re-audit trigger. For agentm and crickets, whose decisions live in living designs' amendment rows, the note points at the row and does not restate it. A handoff is the tracker's State and Next plus the session trace's Outcome. The standalone handoff files go to `archive/`, and no new one is written. *Why:* a handoff file is stale the moment the next session runs and nothing retires it; a tracker is rewritten by construction.

**Every card and trace carries `project:` and `task:`.** The in-session writer, the Stop hook's miner and the trace stamp them from the binding the resolver already knows: the repo's `project.json` names the vault project, and `.harness/active-plan` names the task. `memory_capture` takes both as optional fields. Recall at session start gives a card whose `project:` matches the session's binding a modest boost; the multiplier is the agent's, on the dampen mechanism the ranker already has. The project tracker is itself a note, `kind: tracker`, ranked by `importance` like everything in the permanent space, so "where does agentm stand" finds it on any surface; session 6 decides how it reaches the pasted payload. *Why not infer the project from tags or path words:* zero notes carry the field and twenty-four carry a project tag; inference has already failed. *Re-audit* when a session works two projects at once — the field holds one value, and the second project goes in `related:`.

**Dreaming enriches a card beyond what the session had.** A session hook lays a card down from what was in front of it: title, summary, `why`, excerpt. The next deep pass makes it much richer from what the session did not have: the neighbours recall finds, the decisions, designs and research that bear on it, the analysis session 3 named. That content is written into the body under its own dated heading (`## Added by dreaming`), below what the session wrote, and never rewrites the captured text or the `why`. The frontmatter moves as session 3 already rules, your `importance` never overwritten. A card with `project:` is enriched with its project's records as neighbours first, which is the difference the field makes. Inside `projects/`, the pass reads everything — tracker, plan, progress, decisions, research — as input, and writes the same additive section and frontmatter into the charter, `decisions/`, `designs/` and `research/` notes. It never writes `tracker.md`, `plan.md` or `progress.md`: those are the session's, and a nightly writer would race the session that owns them. `personal/` keeps session 1's frontmatter-only rule. `moc-<slug>.md`, `projects/moc-tasks.md` and `projects/moc-projects.md` are generated nightly by the binary's mocs job, extended from `memory/mocs/` to the projects space; `Projects/index.md` retires into the last. The index keeps a project's `archive/` and ranks it the way it ranks the root `_archive/`, penalized rather than excluded, and the Go arm reads a bounded head of a progress log as the Python arm already does. *Why additive and never a rewrite:* the captured text is the evidence, and a rewrite that reads well is how residue survived the last purge. *Re-audit* if an added section on a project record is longer than the record — then the pass is writing a synthesis where a link would do.
...
**Flatten a closed task into `drafts/`, as work does.** Rejected; blog's `drafts/` holds drafts, and a task's artifacts stay together in their directory under `archive/tasks/`.
...
8. **Projects and tasks.** Three landings in order: agentm's resolver and writers accepting both layouts, with `project:` and `task:` on captures and traces and the mocs job over the projects space; the crickets commands writing and reading the tracker, the paired release; then the migration under quiesce — the `_harness/` table applied, seventy-eight trackers written, `charter.md` renamed, the daily-note wiring, embed backfill — and the `_harness` fallbacks retired one release later. The weekly crystallize phase registers with the night's jobs in group 9, after this group's trackers exist for it to read.
...
The daily corpus scorecard gains: residue count (zero after the purge, and a line that stays), class populations, the card-shape gate's count of non-conforming notes, empty directories under `agent/`, and the health-pass cadence itself. The corpus scorecard also lists tasks by status per project and the age of each project tracker's `updated` stamp, and the morning note lists every crystallized note the weekly phase wrote. The morning note carries the night's spend against its token line, the steps skipped and why, and the week's sank list; a night that ran past its window is a line in it.
