...
The state a persona acts on is **not** in the manifest — it comes from Memory beneath. The manifest field schema is finalized — the `tier:` / `opinions:` / `modes:` / `triggers:` activation axes shipped and were retrofitted onto the live manifests (Wave B/D); the contract + quick-reference live in [persona-tier](persona-tier.md) + `wiki/reference/persona-tier-schema.md`.
...
**Invoked.** The two paths from §Adoption are realized by the **activation plumbing** — how a manifest's stance + composition reach a *running* agent: injected context for an interactive session, a dispatched sub-agent (cold or warm), a scheduled `/loop`, or a host-run goal (the [goal contract](agentm-goal-contract)). **Automatic** adoption fires when a crickets workflow step, or a mid-conversation detection cue, matches a manifest trigger. *(The activation plumbing itself is **built** — `scripts/persona_resolve.py` + `scripts/persona_compile.py` ship the resolve → six-step `adopt()` → per-host launch-compile pipeline, and all 11 manifests carry the four activation axes (Wave B build-part 3 + Wave D). What stays **designed-not-built** is the automatic **detection** half: mid-conversation stance-swap has no trigger grammar, and wiring workflow-step adoption is a cross-repo change to crickets' phase commands — see [persona-activation](agentm-persona-activation.md). Today `brain` + the `team-coordinator` Planner seed run as live manifests; the other nine are authored but not yet auto-adopted.)*
...
- **Leans on** the [Opinions](agentm-opinions-and-gates.md) surfaces (the "Leans on" column) — *how* a persona retrieves an opinion is the request-by-name registry there (**built** — `opinion_resolver.py`, Wave B; binding it per-persona at activation rides the still-designed model-effort tier scale).
...
- **Manifests authored; auto-adoption is the residual.** All 11 roster personas now ship as `kind: persona` manifests — `brain` and the `team-coordinator` Planner seed run live, and the other nine (Architect, Designer, Tech-Lead, Engineer, Reviewer, Operator, Troubleshooter/SRE, Researcher, Maintainer) were authored in Wave D with their four activation axes. The **activation plumbing** (resolve → `adopt()` → launch-compile) and the **opinion request-by-name registry** are built. What stays *designed-not-built* is the **automatic-detection** half: the mid-conversation trigger grammar (no trigger grammar / confidence gate yet) and the cross-repo wiring of workflow-step adoption into crickets' phase commands ([persona-activation](agentm-persona-activation.md)).
- **Re-audit triggers:** flip the automatic-detection triggers + workflow-step wiring to as-built when they land ([persona-activation](agentm-persona-activation.md)); reconcile `Coordinator-Roles.md` + the crickets agent-defs at the role-retirement landing.
...
- **2026-07-07 — activation plumbing + full manifest roster + opinion registry flipped designed→as-built (G12 AG close-out sweep).** Four lines had gone stale against Wave B/D ships: §manifest called the field schema "partly designed" (the four activation axes shipped), §Invoked called the activation plumbing "the unbuilt core" (`persona_resolve.py` + `persona_compile.py` ship the resolve → `adopt()` → launch-compile pipeline, Wave B build-part 3), the Dependencies "Leans on" bullet called the opinion request-by-name registry "designed, not built" (it shipped Wave B), and Risks called the other nine personas "not yet authored as manifests" (Wave D task 2 authored all nine with their four activation axes). Reconciled all four to as-built and narrowed the genuine residual to the **automatic-detection** half — the mid-conversation trigger grammar (an open design problem: no trigger grammar / confidence gate yet) and the cross-repo workflow-step wiring into crickets' phase commands ([persona-activation](agentm-persona-activation.md), whose own Wave-D task 3 stopped-and-reported on exactly that cross-repo boundary). *Re-audit trigger:* flip the detection triggers + workflow-step wiring when they land.
...
- **2026-06-23 — roster reconciled to two capability renames (propagation).** The "Leans on" column updated for renames that landed elsewhere: `development-lifecycle` → `development-lifecycle` (the lifecycle merge) and `github-ci` → `maintenance` (the maintenance reframe). No model change. Maintainer leans on `maintenance` (its executing arm); Troubleshooter/SRE on `maintenance` + `diagnostics` (repair + diagnose).
