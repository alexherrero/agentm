...
A job can also carry a `window` — the hours of day it may start in, local time. A job due outside its window is not late; it waits for the window's next opening, and the lookback above counts from that opening rather than from the moment it fell due. This is how the nightly jobs stay inside `02:00-06:00` regardless of what hour the runner first happens to see them due:

- enrichment
- the dreaming binary
- the Python dreaming cycle
- the corpus scorecard
- the morning note
- the daily email that carries it
...
**2026-09-11 — the night's line is one number, two million tokens.** The operator's budget was written as a million tokens on the strong tier or two million on the cheap one, one night's money at two prices. It is now two million on either tier (`StrongTokenLine`, `CheapTokenLine`, `daemon/internal/enrich/usage.go:214-227`), so the night's allowance is a count of tokens and the tier decides only what they cost. `templates/jobs/enrich-nightly.yaml` declares `budget: tokens: 2000000` to match, and the morning note holds the night to the same number (`morning_note.py:87`). *Why not raise the strong line alone and leave cheap at two million:* that is what this is — the two constants stay separate so a later ruling can part them again, and the metering already reads a line per tier. *Re-audit trigger:* a cheap model is named in `daemon.cheap_model`, at which point equal token lines mean the cheap tier buys the same night for less money, and whether that is the intent needs the operator's word.

**2026-09-11 — the morning note and the email join the night (agentm-vault plan 04, task 6).** `morning-note.yaml` carries `window: "02:00-06:00"` and `order: 5`, and `observability-email-daily.yaml` gains the same window at `order: 6`, so six manifests now share the window in one order: enrichment, the binary, the Python cycle, the corpus scorecard, the note, the email. The note reads what the four steps before it wrote, and the email sends the note, so each has to start after the one before it. The morning note also reads the runner's per-job markers and `last-cycle.json` to say which steps did not run and why. The daily digest's template, which had no window, is deleted. *Why not leave the email on its own daily clock:* it would send whatever note existed when its interval came round, often the previous morning's. *Re-audit trigger:* an email that arrives without the night's note, which means the email started before the note finished.
