...
Nothing is excluded from Obsidian's index — the operator ruled (2026-08-18, at the filing-contract design's approval) that the whole vault is one graph, in the viewer as everywhere else. Obsidian remains a viewer with no authority over what's real (principle 2), and the daemon still checks link integrity itself rather than trusting the viewer. The graph-view-static concern this section once answered with an exclusion is answered instead by the nightly scorecard's rendered graph and by rank treatment in search — never by hiding files from the viewer.
...
- **2026-08-18 · the Obsidian exclusion is reversed.** The filing-contract design's approval ruled that nothing in the vault is excluded from Obsidian's index; the layout section's body is reconciled to that ruling in this same change. The quick-switcher/graph-static concern the exclusion answered is carried by the nightly scorecard's rendered memory graph and by search-side rank treatment instead. See `agentm-rescope-filing.md` (Status: final). *Re-audit trigger:* Obsidian becoming measurably unusable on the grown corpus — that reopens the question with data, and the answer still may not be an exclusion.

- **2026-08-12 · a metadata overlay was declined, and the frontmatter-edit convention it prompted is now written.** The work setup's Evolve-4 moved machine-written metadata (status, supersedes) into a committed `.agent_metadata.json` overlay so code never rewrites frontmatter. Its three hazards are real; its mechanism is rejected here, because under files-are-truth a note whose own frontmatter says `active` while an overlay says `deprecated` is a lying file — two sources of truth for one fact, the exact fork shape this vault hit three times in one week. Our answers stay single-source: **machine edits to existing frontmatter are line-surgical — edit the one line with anchored replacement, never round-trip the block through a YAML serializer** (the rehoming pass flipped 445 statuses this way under `check-vault-frontmatter`); lifecycle changes commit through the daemon with attribution, which makes a note's `git log` its provenance rather than churn; and rebuildability needs no overlay because status lives in the file and every index is a deletable cache. *Adopted from the same review:* write-time supersede enforcement (`supersedes` set by the filing pass when a new fact conflicts with an old one) joins the filing design, in frontmatter. *Re-audit trigger:* the first machine writer that needs to update more than a line at a time — that is the point to build a shared surgical-edit helper rather than let each writer improvise one.
