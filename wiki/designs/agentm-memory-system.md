...
- **It becomes navigable by an index over the markdown — built.** A device-local vector index makes the corpus semantically searchable, and the **SQLite metadata table + hybrid `--filter` recall (V6-11) shipped 2026-07-06** — a query can now narrow by `kind`/`project`/`status`/`tag` in the same SQL-joined-with-vector query. **Still designed-for:** a **knowledge-graph** (V6-2) extracting typed edges over the wikilinks (deterministic, no LLM) so relationships become a retrieval path — including **multi-hop traversal** — plus **chunking** and **RRF hybrid retrieval** (V6-3/10) to sharpen ranking as the corpus grows past the hundreds. Graph and index are layers *over* the pages — markdown stays the source of truth; the graph is for navigation and discovery.
...
- **V6-11 (the metadata table + hybrid filter) shipped 2026-07-06** — see the [memory index](agentm-memory-index) design. **The rest of V6 indexed-recall stays designed-for, not built:** the recall loop's ranking is still the pre-V6 form (semantic-weighted merge + grep fallback, now filterable by the V6-11 columns); the reserved `DerivedMaintenance` extension point is where a future V6 slice adds the **knowledge-graph layer** (typed edges over the wikilinks; multi-hop traversal), **RRF hybrid retrieval** (BM25 + vector + graph), **consolidation tiers**, and **chunking**. Graph and index stay layers over the markdown (the pages remain source of truth); the rest of that spec lives in the V6 plan + the seam design.
