...
- **V6-1 (per-note lifecycle tiering) shipped 2026-07-07** — see the Capture section above and the amendment log below. **The rest of the decay/archive lifecycle stays designed-for.** Heat curation + supersession-archive + V6-1 tiering ship; consolidation tiers, crystallization, self-healing lint, and the dreaming compaction pass are V6/V7, gated on the revert-log + the `derived-from` provenance edge and the staging inbox. Until those land, archival stays manual (`/memory evolve`) or operator-confirmed. The migration to the three-tier folder layout is operator-gated and updates the hardwired `personal/_always-load` constants in `recall.py` / `heat_policy.py` / `save.py`.
