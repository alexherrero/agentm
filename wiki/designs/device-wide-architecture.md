...
- **v1.0 — pending — V5-6 — Routing-plane de-vaulting: `resolve_project` / `_vault_projects_dir`, `repo_registry`, and `state_mode` route through the storage seam.** The third leg of the V5 de-vaulting arc (data plane V5-3 ✅, config plane V5-7 partial ✅, routing plane V5-6 this entry). After this release a fresh install with only the `device-local` backend can host a project, harness state, and the repo registry without needing a vault. On the `obsidian-vault` backend every mechanism resolves to identical bytes in identical locations — behavior-preserving by parallel-run proof. `state_mode: vault` aliases to `state_mode: backend` at read time; no operator migration. Gate extensions: `check-storage-seam-no-path-leak.sh` and `check-process-seam-import-direction.sh` extended to cover the routing layer; `storage_conformance.py` parameterized across both concrete backends. Full design: [Seam-De-Vaulting-V5-6](../explanation/Seam-De-Vaulting-V5-6); reference: [Storage-Seam § Routing layer](../reference/Storage-Seam#routing-layer-v5-6). _Commits: pending._
