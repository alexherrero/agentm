...
A memory arrives, and two things happen to it. The capture transaction writes the file, extracts everything a regex can extract, and returns — offline, synchronous, under a hundred milliseconds, never dropped. Then an enrichment pass runs immediately afterwards, out of band, and does the work that needs judgment: it improves the whole note — body distilled into real prose, type, tags, aliases and altitude assigned, slug corrected while nothing links to it yet — splits blobs into atomic memories, and decides whether the result is good enough to keep. If that pass fails or the model is unavailable, the note simply stays `unfiled` and the nightly dreaming run picks it up. The model is never on the critical path.
...
| `idea` | `memory/semantic/`, per [Filing v2](agentm-filing-v2) |
...
**A gap worth naming.** Re-typing never moves a file, but re-*classing* would, and nothing in the source system says what happens when enrichment puts a memory in the wrong class. The rule proposed here: dreaming may move a memory between classes exactly once, only while it has no inbound links, and only through the revert log. After that the class is fixed and a mistake is corrected by superseding rather than moving. The same while-unlinked rule covers the slug: enrichment may rename a freshly captured note once, before anything links to it; after the first inbound link the slug is an address, and addresses do not churn. *Re-audit trigger: the first month in which class corrections exceed a handful.*
...
| documents at the project root | creating one of the five front-page files is standing; creating any other file, or modifying or replacing an existing one, takes explicit alignment |

The root is the project's visible face. Since the operator's ruling of 2026-09-24 (see [AgentM Vault design § Projects and tasks](agentm-vault#projects-and-tasks-session-4-decided)) it holds exactly five files — `project.yaml`, `charter.md`, `blueprint.md`, `tracker.md`, and the generated `moc-<slug>.md` — and the working bulk — plans, research, running drafts — goes into subfolders so the face stays legible. The convention is a shape to maintain; the alignment rule is the door. Below the root the agent adds and maintains freely, and it changes the root only in agreement.
...
Alongside status, a decay score governs rank rather than existence — **once the corpus can carry one.** A memory holds full strength through six months of silence, ranks at half to a year, an eighth to three years, and a sixteenth to five — **and a sixteenth is a floor, not a waypoint.** The curve never reaches zero, because a memory that nobody has needed in four years is not worthless, only cold, and a floorless curve makes it unreachable rather than merely unlikely.

The curve is built and it is turned off, and both decisions are measured rather than cautious. This corpus is too young to age: 89% of notes are under a month old, the oldest in the memory layer is 93 days against a first band at 182, and exactly five notes of 15,824 cross any band at all. Scored against the frozen gold set with decay as the only variable, R@5 fell from 0.781 to 0.750 — two questions lost, none gained. So `daemon.decay_enabled` ships false, and turning it on is a measurement someone takes later rather than a default nobody revisits.

One of those two losses is the more interesting finding, and it is a precondition rather than a bug: **for a temporal question, age is the signal, not the noise.** *"When did I switch from Antigravity to Claude?"* is answered by a note written in February, and demoting it for being old is demoting it for being the answer. A curve that ranks by staleness and a question class that ranks by antiquity point in opposite directions, and nothing here reconciles them yet. Enabling decay means answering that first.
...
Some memories never decay. Failure incidents are exempt because the whole value of an incident record is being there on the one day, years later, when the same failure recurs. Decisions are exempt for the same reason. Crystallized memories — the distilled lessons dreaming promotes out of repeated observation — are exempt because a lesson that survived being learned three times is the durable kind.

As built, that works out to four routes rather than three named classes, because two of them are proxies for how the corpus actually expresses durability rather than fields anybody types. An explicit `lifecycle_tier: durable` is the direct statement. `kind: failure-incident` carries the incidents. Decisions are recognized by the `decisions/` directory they have always lived in, since no `kind: decision` exists here — the operator's records retired into living-design amendment logs. And a space the filing contract does not govern is exempt wholesale: decay models a *memory* going cold because nobody has needed it, and those are documents. A 2016 lesson is not less true for being ten years old.
...
It runs immediately after the transaction commits, and again inside dreaming for anything still `unfiled`. **Its product is the whole note.** Capture writes raw material; enrichment turns it into the memory — the body distilled into real prose, the title and slug corrected while nothing yet links to them, type and altitude assigned, tags and aliases filled, a summary when the entry is long enough to want one. No field is off-limits to the pass; the guards are what keep the improvement honest, field by field. Voice and prose matter here, because the model's writing becomes the corpus: the enrichment prompt carries the voice specification, and since the prompt's hash is part of the pass version, a voice change is a version bump that the meters can see. The raw capture survives regardless — in git at the capture commit, and reachable through `source` — so a rewrite is never a loss, and re-distilling from the original is always possible.

And it never pays twice. Three stamps make every unit of model work idempotent: the note's own `enriched_by` / `rules_hash` marks it done at a version, the coverage ledger holds the same for every corpus-walking stage, and the source registry watermarks every external unit ever mined. A note already enriched at the current version is skipped on a fingerprint compare; a source already mined is skipped on a registry lookup; and when a prompt, model or rule changes, exactly the affected population re-enters the queue and nothing else does. Tokens go to new material and to deliberate re-runs — never to forgetting what was already done.

A stamp covers work that finished, and a refusal is the work that did not. A card whose answer a post-gate rejected is left exactly as it stood, so no stamp records that it was ever asked about, and it returns to the queue every cycle at full price. So the refusal is recorded too, in its own file beside the run record, under the same key the fingerprint compare uses. It is carried at its own version — over the gates' wording rather than the pass's — so sharpening the judge re-queues the refused set without re-owing the deep pass to the corpus. Only a refusal that named a claim is kept: a judge that could not answer is a fact about the judge, and remembering it would blacklist every card a bad hour touched.
...
1. Self-probe — a note carrying the `probe:` marker is the daemon's own synthetic round-trip fixture, not a memory, and is refused before every other question. It runs first because it is the only gate asking what the note *is* rather than where it sits or what it costs: a probe passes all six below on the merits, which is how one came to be judged on 2026-09-17. What the rewrite dropped was the marker itself, and the marker is the probe's identity — retirement, the card gate and the classifier all read it — so a stripped card is one nothing ever retires.
2. Eligibility — `status: unfiled`, an observational class, never `personal/`, never `mocs/`. Path rules, no judgment.
3. Privacy scrub — the model sees the same scrubbed text every other stage sees.
4. Size ceiling — an oversized blob is split along its headers deterministically first, and the model judges fragments. No input exceeds the ceiling, because the dispatcher enforces it rather than the prompt requesting it.
5. Idempotency — a note whose `enriched_by` matches the current pass version, whose `rules_hash` is current, and whose body fingerprint is unchanged is skipped without a call.
6. Refusal — a note a post-gate rejected at this same fingerprint, under these same gates, is skipped without a call. It returns when its body moves, when the prompt moves, or when the gates' own version moves.
7. Budget — the cycle carries a call-and-token budget from configuration. When it runs out, the remainder defers to the next cycle under the coverage ledger's cursor, and the digest reports what deferred.
...
| correct title and slug | inbound-link check — zero links is the precondition | a linked note's slug never changes; a bad one is fixed by supersession |
...
| Date | Change | Status |
|---|---|---|
| 2026-09-25 | *Projects, tasks, and the door between them* follows the operator's ruling of 2026-09-24: a project's root holds five files — `project.yaml`, `charter.md`, `blueprint.md`, `tracker.md`, `moc-<slug>.md` — and the door answers any other new root file with Alignment ([PR #697](https://github.com/alexherrero/agentm/pull/697), task 176 step 4). This reverses the row's "as many as the project needs — no cap". *Why not keep the cap-free rule and tidy by hand:* root files had accumulated in every project where nothing said no, and the front page should read the same everywhere. The governing record is [AgentM Vault § Projects and tasks](agentm-vault#projects-and-tasks-session-4-decided). *Re-audit trigger:* the operator allowing the same sixth root file in several projects. | final |
| 2026-09-17 | A seventh pre-gate, and it runs first: **the daemon's own self-probe is not a memory, and the pass refuses it before it asks anything else.** The probe writes a synthetic note once a day, asks for it back sideways and leaves it behind as the artifact of the check. On the night of 2026-09-17 enrichment judged one, and the rewrite dropped the `probe:` marker along with `aliases`, leaving `status: unfiled` at `filing_confidence: low`. The call was the cheap half of that; the marker was the expensive half. It is the probe's identity rather than a value on it — the next run retires yesterday's card by reading it back off the file, `check-card-shape` exempts a probe from naming a transport by reading it, and the classifier excludes a probe from every measurement by reading it — so a stripped card is one nothing retires and the vault gains another every night. `enrich.SelfProbe` reads the marker through `note.Parse`, the corpus's own parser, rather than a second reader of the same key, and lives in the free set the cheap-tier audit draws its sample through, so the audit stops sampling the probe too. `probe` also joins `carriedFields` as a second guard, opposite `aliases`, which stays out: a probe's aliases are the round trip's own nonce and are spent the moment it finishes, while its marker is what every later run reads it by. Retirement is deliberately **not** loosened to compensate — the marker stays the only thing that authorizes deleting a file, because a cleanup falling back to the tag or the file name would delete on exactly the path-and-word evidence this design rejected. *Re-audit trigger: any writer that rewrites a note's frontmatter wholesale, which is the shape that dropped the marker here.* | final |
| 2026-09-11 | A sixth pre-gate: **a refusal is recorded, so the pass stops re-paying for answers it already has.** A card a post-gate rejects keeps no stamp, so eligibility offered it again the next night — nineteen cards did that in one run for about eight dollars, and the nightly job would have repeated it indefinitely. The refusal now lands in `enrich-refusals.jsonl` beside the run record, keyed by `Fingerprint.Key` itself rather than a second implementation of it, and carried at a `GatesVersion` kept out of `PassVersion` on purpose: sharpening the judge re-queues the refused set, where folding it into the pass version would re-owe the deep pass to all eight thousand cards. Not the card's frontmatter, because `PassDepth` reads a stamp's presence and a refusal beside one would have to be excluded by hand in every reader; not a ledger stage, because the ledger's rebuild recovers rows by walking the corpus and a refusal has nothing there to recover from. **And a confirmed cause of false refusals is fixed**: the faithfulness judge was shown the card's body with the frontmatter stripped, while the enricher was handed the whole card and told every claim must trace to it — so a proposal naming the card's own `source_id`, `lifecycle` or capture date was refused for asserting what the judge could not see. Three of the nineteen were exactly that. The judge now sees the frontmatter less the fields the pass writes itself, so an already-enriched card cannot ground a claim in its own previous answer. Re-audit when the refused set stops shrinking under a prompt change, which would mean the refusals are the prompt's rather than the cards'. | final |
...
| 2026-08-19 | Part 1 (`rules-and-contract`) built. Three scope decisions taken during the build and recorded here rather than left in commit messages. **(a) Two registers.** `memory_types` (the six) and `record_kinds` (infrastructure record shapes — briefs, telemetry, the `*-index` family, personas, session traces), with a note carrying `type` or `kind` and never both. This generalizes this document's own "generated pages carry no type at all" from `mocs/` to the whole non-memory population: a frontmatter census of the live vault found 43 distinct values where § Detailed Design enumerates 22, and forcing `brief` to become `reference` would put a digest of Tuesday in the same taxonomy as a convention that has held a year. **(b) Go owns the parser.** The design says "the daemon validates the block on every read"; the first build put the parser in Python and left `config.Types` a Go literal, which made "a type added to the rules exists everywhere at once" false in shipped code. `daemon/internal/rules` is now the only parser and the Python batch layer asks it over `agentmd rules --json`. Operator ruling in the same session: plan the wider Python retirement after all six parts land. **(c) `_inbox` deferred from the collapse** — 9,860 of 10,700 notes, excluded on operator ruling because its statuses are the lifecycle part's to define and enrichment rewrites those notes when it drains them. | final |
| 2026-08-19 | Sequenced into 6 plans via `/design sequence`, staged as `PLAN-<slug>.md` in the vault's `_harness/queued-plans/` — this repo's documented two-tier staging (`stage_plan.py` + `/plan --activate`) supersedes the skill's generic `<slug>.PLAN.md` path, and `check-multi-plan-naming` enforces it. Topological order: rules-and-contract → capture-and-indexes → search-and-lifecycle → enrichment → dreaming-ledger-and-sources → meters-and-scorecards. None activated; the build session activates the first. | final |
...
| 2026-08-27 | Part 6 built and closed, with two scope corrections recorded where they were found rather than folded away. **The meters' population was wrong**: they were measuring the whole corpus when the question is whether *enrichment* is converging, so they now read the filed memory corpus and the figures before the correction are not comparable to the ones after. **And the labelling process was inverted.** The design assumed label-then-measure; three rubrics failed their own pre-registered calibration that way — κ = 0.189, 0.349, 0.286 — and every disagreement ran agent-flagged / operator-kept. What replaced it is calibration-first adjudication: the agent states a grade and its reason, the operator agrees or disagrees, and the resulting standard is tested against held-out notes with the predictions and the bar written down beforehand. Reading those notes also surfaced two capture defects the rubrics had been unknowingly measuring — bodies sliced mid-word by a character-offset excerpt, and reference notes whose body was their own description. Both are fixed at capture and backfilled. | final |
| 2026-08-21 | Enrichment built and measured. Two live batches of thirty randomly-sampled unfiled notes ran against the real vault, and the homogenization risk this design has carried since the first revision now has a number instead of an argument: mean pairwise similarity fell from 0.6222 among the sources to 0.4929 among the rewrites, so the pass made the notes **more** distinct rather than less. Per-note fidelity averaged 0.7778 with nothing below the floor. Three deviations from the design as written, each on evidence. Faithfulness is checked **per note** rather than sampled, on operator ruling — affordable because the queue drain is deferred, and it caught a model editorializing on the first batch. Model-tier qualification is **out of scope** for this part; the pass takes a configured model name and the sampled-audit machinery stays its own mechanism. And the eight-part `altitude` dampening from part 3 is **held behind a flag**: capture writes `altitude: artifact` on every new note, so dampening only what carries the field penalized recency rather than artifact-ness until enrichment labels the corpus. | final |
