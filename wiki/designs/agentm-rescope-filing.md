...
| Date | Change | Status |
|---|---|---|
| 2026-08-18 | Seventh revision, and approval. The Obsidian exclusion is reversed on the operator's ruling: nothing in the vault is excluded from Obsidian's index — the whole vault is one graph in the viewer as everywhere else — and the scorecard's rendered graph becomes a convenience view rather than a replacement. The sibling `agentm-rescope-memory.md`, which carried the original exclusion, is amended to match under the living-design convention. Operator approved the design; Status: draft → final. | final |
