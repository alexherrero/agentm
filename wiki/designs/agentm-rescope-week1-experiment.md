...
- **2026-08-11 - the research stratum this design asked for now has its first real data, from a live ingest.** The operator handed over an article and asked for it distilled the way the capture doctrine describes; six atomic `reference` memories landed `active`, and the acceptance test was a set of concept-phrased questions asked in vocabulary the notes do not contain. **The split is clean and it is not a ranking result.** Every alias written at capture time returns its own note at rank 1 — "is soil power renewable or does it run out", "can soil power run a rice cooker", "do engineers take soil power seriously", each hitting the note it was written for. Questions phrased in unanticipated vocabulary return **nothing at all**: "does dirt electricity wear out", "how many soil cells to light a bulb", "is harvesting energy from mud legitimate". **Why the failure is empty rather than merely low:** FTS5 ANDs its terms, so a single out-of-vocabulary content word in a seven-word question empties the result set before ranking is consulted. No penalty tuning, column weight or over-fetch touches that; the note is not ranked badly, it is not a candidate. **What this establishes about aliases, which the week-3 retest left open:** written at ingest from the asker's own intent they are load-bearing and they work, which is consistent with that retest's finding that the *backfill* failed because a model reading a note paraphrases the note. The generator's source of question vocabulary was the variable all along. **What it does not yet establish:** whether this matters at corpus scale. Six notes and a dozen probes is a demonstration, not a measurement, and the honest next step is the same one this design has always named — a research-stratum block in the gold set, scored across replicates, before the sidecar question is reopened on evidence rather than on a vivid miss. *Re-audit:* that block existing. Until then the standing rule is unchanged and FTS5-only still holds.
