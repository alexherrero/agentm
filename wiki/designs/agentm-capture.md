...
governs: [scripts/memory_mcp_tools.py, harness/skills/memory/scripts/capture.py, harness/skills/memory/scripts/save.py, harness/skills/memory/scripts/ingest.py]
seeded: 2026-07-16
approved: 2026-07-17
---
...
The capture door provides the single security checkpoint between untrusted content and your agent's environment. The system treats all captured content purely as data. Text that reads like an instruction gets no authority at capture time. Articles the ingest sweep fetches from forwarded links cross this same boundary. The system records exactly where every piece of data came from. The system excludes the `_inbox/` folder from the agent's memory searches by default. You can include it with the `--include-inbox` flag. Captured content enters your active working memory only after the triage system promotes it. We reuse the MCP server's `_validate_path_segment` tool to ensure files go to the right place. We discuss the remaining risk of malicious content entering your active memory in the Risks section.
...
- Content promoted by the triage system eventually becomes visible to your agent. We protect you by tracking data sources. We hide staged items from searches. We run everything through the triage process. *Re-audit trigger:* any real injection incident.
