...
Two mechanisms carry candidates across this boundary, both landing on the same trust guarantee through different machinery. A `memory_capture` candidate — a thought, a link, or an idea typed at capture time — files directly to its class directory today, flagged `filing_confidence: low` (filing v2's write path). The inbox triage system that once staged it behind an explicit promote, merge, or expire decision is retired (agentm v9.20.0). A forwarded link or document the ingest sweep fetches on its own schedule crosses the same boundary through its own staging mechanism instead: the sweep patches the candidate in place, still recall-invisible, and only calls `ingest.ingest()` to promote it to permanent memory after the candidate survives one full sweep cycle — the sweep's own review window. Explicit, human-invoked `/memory ingest` and `memory_append` sit outside both: you named the source yourself, so they write directly to permanent memory at the same trust level as any other deliberate save.
...
- Content promoted out of staging eventually becomes visible to your agent. We protect you by tracking data sources. We hide staged items from searches. A forwarded link reaches permanent memory only through the ingest sweep's own review window, never straight from untrusted content. *Re-audit trigger:* any real injection incident; and whether a `memory_capture` candidate still gets an equivalent review window now that the inbox triage system that used to provide one is retired (agentm v9.20.0) — filing v2's write path files it directly at low confidence instead.
