...
We need a way to get a thought or an article into agentm's memory from your phone, your browser, or a chat. **This design creates a single front door for all captured content.** Everything lands in its memory class the moment it arrives, marked with how much is known about it, and the machinery on your machine judges it from there. We define the paths in, how each kind of capture is handled, the machinery that does the processing, and the security boundary everything crosses.

### Background

We need a way to capture information from any device. When this design was first written, captures were staged in an inbox that an automated triage pass promoted, merged or expired. Filing v2 (September 2026) retired the inbox: a capture files straight into its class directory and carries its own filing state, `status: unfiled` until something judges it. The body below describes the write path as it is; the history at the bottom records the change. What a capture holds the moment it lands, and who may write which field, is decided in [AgentM Vault](agentm-vault.md) § Capture.

Two transports already work. The claude.ai Capture project writes candidates into the vault from the phone or any chat through the Google Drive connector. The Obsidian Web Clipper writes candidates from desktop Chrome. Neither transport needs a credential on any device. Our privacy rules shape every choice here. Data moves only through local and first-party tools. All captured content is treated as untrusted until it crosses one audited doorway.

Building this takes little new effort. We reuse many existing components. We already have the tools to write files safely, split text into chunks, evaluate confidence, run scheduled tasks, and provide server capabilities. The new work consists of a capture interface, tracking where data came from, a command to process long articles, and a scheduled job that fully processes forwarded links. We leave automatic cross-linking across the entire vault to the auto-organization design (`agentm-auto-organization.md`). That design ships in the same arc. Newly ingested notes get connected soon after they land.
...
First are the ways in. Several doors deliver a candidate note into its memory class under `Agent/memory/`. These doors include your phone, your browser, and your desktop.
...
|---|---|
| `memory_capture` | MCP tool: files a candidate at its class directory, `status: unfiled` until something judges it |
...
| ingest sweep | a runner job: forwarded links and documents among the unfiled captures → the ingestion pipeline |
| the bridge guard | the one audited boundary all captured content crosses |

| Trigger | Path |
|---|---|
| the Capture project (phone / any chat) | Drive connector `create_file` → the vault |
| Obsidian Web Clipper (desktop Chrome) | clip → the vault |
| MCP call from any connected host | `memory_capture` → its class directory |
| CLI / in-session verb | `/memory capture` · `/memory ingest` |
| runner schedule | ingest sweep, cadence-bound (default hourly) |
| session hooks | reflection files only its HIGH candidates, as `unfiled`; MEDIUM and LOW go to the session trace |

Saves either succeed completely or fail cleanly. The system alerts you immediately if a capture fails. Your thoughts are never lost silently. Every pass that changes a note journals the change, and the vault is a git repository, so you can undo any of them.
...
| any file into a class directory (desktop) | works today | any app that writes the folder is a door; a note without a type files as the contract's default type, `unfiled`; zero build |
| `/memory capture` · `memory_capture` (in-session) | this design builds them | the explicit door for sessions and scripts |
| reflection's HIGH lane | shipped | the agent's own session learnings; MEDIUM and LOW stay in the session trace as candidates |
...
From your phone, the flow works today. You open the **Capture** project in the Claude app. You send `capture this: <link>`, a bare thought, or `idea: <thought>`. You can include a trailing instruction like "add to my ideas ledger". The project uses standing instructions. Claude reads the link and builds the candidate. The claude.ai Google Drive connector creates the file in the vault. The file rides Anthropic's servers to Drive. Drive syncs the file to your machine. The phone requires no harness. The process needs no credentials. Your machine can be asleep. Drive delivers the file when the machine wakes. The local machinery processes the file from there. You complete one initial setup step. You approve the connector's Create-file permission with "Always allow". A failed capture appears in the chat reply. You always know the exact outcome.

The Obsidian Web Clipper runs in your desktop browser. It saves a full article into the vault with one click. Clipper paths resolve relative to Obsidian's own vault root. This root sits one level above the engine's `vault_path()`. You prefix the clipper paths with `Agent/`. A class directory is an ordinary folder in the vault. Any app that writes a file there is a door. You can set Obsidian's new-note button to default there. You configure this path via Settings, then Files and links, then Default location for new notes. Obsidian on the phone cannot open a Google-Drive-synced vault. The Claude app provides the phone path.

#### What arrives — the candidate shape

A candidate carries the memory card of [AgentM Vault](agentm-vault.md). We record the transport in `source:`, the time in `created:`, and the device in `surface:`. A candidate also declares its `kind`. We use `capture` for links, articles, and thoughts. We use `idea` for entries headed to the ideas ledger. A candidate includes any `tags` you attached. It also holds an `instructions` field. The rules for instructions appear in the next section. A candidate written by a model that knows why it was kept also carries `summary`, `why` and `importance`; one written by anything else leaves `why` empty, which is what marks it `status: unfiled` for the nightly pass to judge.

The system writes these notes through the filing engine and `save_entry`, the same door every writer uses, under the vault mutex and the daily write cap. The new notes use our standard file naming scheme. The nightly enrichment batch judges the unfiled ones together.
...
- *A thought or note* stays one candidate. Filing v2's write path files it directly at its class directory, `status: unfiled` and `filing_confidence: low`, rather than staging it behind a separate promotion step — the inbox triage system that once promoted, merged, or expired it from a staging folder is retired (agentm v9.20.0). An unfiled note is not hidden from your agent's searches; it is demoted in ranking until something re-judges it.
...
- *An idea* (`kind: idea`) preserves your words verbatim. The system records the raw text exactly as submitted. The machinery folds idea candidates into the ideas ledger's real format. One mechanism serves both operator-typed ideas and the ideas reflection lists in the session trace. The ledger's own shape lives in its pending design. This design only delivers candidates to it.
...
The capture door provides the single security checkpoint between untrusted content and your agent's environment. The system treats all captured content purely as data. Text that reads like an instruction gets no authority at capture time. The system records exactly where every piece of data came from. An unfiled capture is visible to recall from the moment it lands, at reduced weight until something judges it; nothing is hidden. We reuse the MCP server's `_validate_path_segment` tool to ensure files go to the right place.

Two mechanisms carry candidates across this boundary, both landing on the same trust guarantee through different machinery. A `memory_capture` candidate — a thought, a link, or an idea typed at capture time — files directly to its class directory today, flagged `filing_confidence: low` (filing v2's write path). The inbox triage system that once staged it behind an explicit promote, merge, or expire decision is retired (agentm v9.20.0). A forwarded link or document the ingest sweep fetches on its own schedule is patched in place as `status: ingest_staged` and promoted through `ingest.ingest()` after one full sweep cycle. That status is walled from the Python recall arm and not from the Go ranker, so the two arms disagree on it; the reconciliation is filed in the follow-ups, and until it lands the sweep's window is a delay, not invisibility. Explicit, human-invoked `/memory ingest` and `memory_append` sit outside both: you named the source yourself, so they write directly to permanent memory at the same trust level as any other deliberate save.
...
A capture is an ordinary memory entry from the moment it lands. This design adds nothing to how the system shapes those entries: the card is [AgentM Vault](agentm-vault.md)'s, and the memory-system design (`wiki/designs/agentm-memory-system.md`) holds the older entry contract it inherits from. That contract requires one atomic note per fragment, frontmatter keys, `kind` as the folder, and dense wikilinks. The same design document details the vault layout, the three ownership tiers, and the archive convention. That convention keeps cold notes out of the way. The recognized `kind:` values live in the Kind-Taxonomy registry (`wiki/reference/Kind-Taxonomy-Registry.md`). Ingested reference notes land under `personal/domains/<topic>/` following the entry contract. Ideas land in the ledger. The ledger's shape lives in its own pending design (`wiki/designs/memoryvault/parts/idea-ledger.md`). Read those documents to understand where everything this design captures ends up.
...
We reuse our existing tools without changes: `save.py`, `vault_lock`, `chunking.py`, reflection's lanes, `dream_confirm`, the runner, and the MCP server. The claude.ai Google Drive connector carries phone and chat captures. Its traffic routes through Anthropic's servers. No credential lives on any device or sandbox. The Obsidian Web Clipper is operator-installed. Chrome blocks automated extension installs by design. We use the Dispatch and Remote Control preview tools as the in-session phone paths. We coordinate with the auto-organization design (`agentm-auto-organization.md`, same arc) to handle cross-linking your notes.

## Migrations

We add new fields, one new tool, and two new commands. Your existing notes retain their current format. The `memory_append` tool keeps working exactly as it does today. We add a second tool beside it. You can manually fix the group name on the two older domain notes. No systems depend on them.
...
- A `memory_capture` candidate is visible to recall from the moment it lands, at reduced weight while `unfiled`. There is no review window. The boundary is elsewhere: captured content never gains instruction authority (the `instructions` rule, mechanical), a fetched or mailed transport carries `trust: untrusted` through every pass, and recalled memory is data to the surface that reads it. *Re-audit trigger:* any real injection incident, or a surface that acts on recalled text as instructions.
- Capturing content easily from your phone might increase the number of items landing in your vault. Your weekly summary tracks these numbers. The auto-expire this risk once relied on (`inbox_triage.py`) retired in agentm v9.20.0 — see the Trust Boundary section and the amendment log below.
- A forwarded link can fail to fetch due to a paywall, a dead link, or your machine being offline. The candidate stays where it landed with the failure recorded. Your digest surfaces it. Nothing disappears silently.
...
We expand the write surface. We secure this by routing everything through one audited doorway. We reuse our tested path validation tools. A casual capture lands `unfiled`, demoted in recall until the nightly pass or a session judges it. We rely entirely on local and first-party tools to move your data safely.
...
We use our existing safe write tools to ensure file completeness. A bad capture lands as one unfiled note and touches nothing else. Every pass that changes a note journals the change, and the vault is a git repository, so you can undo any mistake.
...
You monitor your capture activity in the same places you monitor all memory activity. The daily corpus scorecard and the nightly digest show the unfiled count, the queue's age, and what the enrichment batch judged. You see live memory actions directly on your console. The capture→recall probe's result lives in the plan's evidence. The FRIDAY health family on the dashboard lights up at the end of the arc, per its locked acceptance shape.

### Logging Plan

Every note records the device and source you used to capture it. The enrichment batch journals what it decided for each note. The ingest sweep logs its progress through our standard task runner.

### Rollback Strategy

We add new files and one optional tool. You can delete a bad capture by hand, or let the second sub-floor verdict sink it to `dormant`. You can undo any pass's change through the vault's git history. If you remove the new tool, the system returns exactly to how it works today. There is no schema or state migration to unwind.

## Document History

| Date | Change | Status |
|---|---|---|
| 2026-09-06 | **Reconciled to the write path as it is, from session 2 of the vault-perfection series.** The staging premise this design was written around — an `_inbox/` folder, candidates hidden from recall, an automated triage system promoting, merging and expiring them — was replaced by filing v2 on 2026-09-04 and the retirement on 2026-09-06 left about twenty-six passages still narrating it. Every one is rewritten: a capture files at its class directory, `status: unfiled` until something judges it, visible to recall at reduced weight from the moment it lands; the review queue is a query, not a folder; the nightly enrichment batch is what judges the unfiled; reflection files HIGH candidates only and keeps MEDIUM and LOW in the session trace. The untrusted-content risk now states the posture that holds — there is no review window; the boundary is that captured content never gains instruction authority, that fetched and mailed transports carry `trust: untrusted`, and that recalled memory is data to the surface reading it — with a re-audit trigger on any injection incident. Two facts named rather than smoothed: the ingest sweep's `ingest_staged` status is walled in the Python recall arm and not in the Go ranker (filed in the follow-ups), and `filing_confidence` is read by no ranker, so nothing here implies it demotes. The candidate shape now points at [AgentM Vault](agentm-vault.md) for the card and its `why`. The stale `save_entry` bypass rationale (kebab-case folder names) is gone with the folder it explained. | launched |
| 2026-09-06 | **The inbox triage system retires; a `memory_capture` candidate stages nowhere now.** `inbox_triage.py` walked `memory/_inbox/`, a directory filing v2 had already emptied — nothing wrote to it any more, so its promote/merge/expire had proposed nothing since. Its three jobs now have owners elsewhere: the heat policy, the dreaming binary's `copies` job, and the lifecycle axis. The Trust Boundary section above is corrected: a `memory_capture` candidate files directly to its class directory today, flagged `filing_confidence: low` (filing v2's write path), rather than staging behind an inbox-triage promote/merge/expire decision, and it is demoted in ranking rather than hidden outright. The Dependencies list drops `inbox_triage` accordingly, and the two Risks entries touching it are corrected. What is genuinely lost: nothing now detects a fuzzy-similar, fingerprint-distinct pair and parks it for you — the copies job only takes content-identical families, and the dedup stage only proposes a merge. Recorded as a capability, not just an implementation, so rebuilding it stays a decision someone can take (agentm v9.20.0, [PR #575](https://github.com/alexherrero/agentm/pull/575)). Left deliberately untouched: the Objective, Overview, and Quality Attributes sections still narrate "the automated triage system" as ambient infrastructure in several places not tied to a single correctable claim — that's a wider reconciliation this entry flags rather than guesses at. | launched |
