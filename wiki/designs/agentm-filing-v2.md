...
> [!NOTE]
> **LAUNCHED** (2026-09-05; reconciled through 2026-09-06). This is the record of the build — six parts, v9.9.0 to v9.20.0. From 2026-09-06 the vault's shape is designed onward in [AgentM Vault](agentm-vault.md), the successor authored by the vault-perfection series; it inherits every decision here unchanged unless it says otherwise, and new rulings land there, not in this log.
...
zero disables it. The Go capture's `source:` held a provenance
reference while the contract's `source:` was the transport vocabulary — two
meanings of one field, left open by this part and closed in v9.19.0 (PRs
#570, #571 and #572): `source:` names the transport alone, a fetched page names its
address in `source_url:` and a mined unit its registry identity in
`source_id:`, readers resolve those first and still accept the legacy shape
for what nobody migrated, and the frontmatter gate refuses a value outside
the vocabulary. The corpus carried 578 transports, 124 bare URLs and 14
references; the migration moved the reference out and gave the fetched pages
the `untrusted` tier the contract had always specified and no reader had ever
been able to apply. And the labeled sample moved the next work upstream:
...
| 2026-09-06 | One meaning for `source:` (v9.19.0, PRs #570, #571, #572), closing DD§4's own open question. The field names the transport alone; a fetched page's address goes in `source_url:` and a mined unit's identity in `source_id:`; readers resolve those first and still accept the legacy shape, so the source registry is never blind to the population it covers; the gate refuses a value outside the vocabulary; the capture tool's schema and the CLI advertise all three fields, so a caller can discover them. The corpus migrated — 124 fetched pages and 14 references — and the fetched pages earned the trust tier the contract had always given them. Why not renaming the contract's field, or a reading-side rule: the first migrates 578 notes to accommodate 138, the second leaves the field overloaded so no gate can ever refuse a wrong value. Re-audit when a writer needs a transport the contract does not name, or a reference neither field can hold. Same day: the vault-perfection series opens and authors the successor, [AgentM Vault](agentm-vault.md); this log closes here and later rulings are recorded there. | launched |
