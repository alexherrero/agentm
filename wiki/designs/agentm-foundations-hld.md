...
- `src/development-lifecycle/commands/{work,bugfix,release,review}.md` (P4, P5)
...
**2026-08-26 — what per-project install was carrying, re-homed: registration is machine-wide, discovery happens at fire time.** Three hooks the retired `--scope project` path was the sole delivery mechanism for now install once for the machine and resolve the project when they fire: `verify-dispatch` (PostToolUse) runs the **edited file's own** project's `.harness/verify.sh`; `compaction-marker` (PreCompact) appends a marker to the active plan's progress log; `compaction-reanchor` (SessionStart, matcher `compact`) tells a resuming session its conversation was discarded rather than paused. *The pattern this establishes:* a per-project **behaviour** does not require a per-project **installation** — it requires a machine-wide registration plus an honest answer to "which project is this?" at the moment the hook runs. The honest answer is never the cwd: the predecessor hardcoded a cwd-relative `.harness/verify.sh` and worked only because hook and project were installed into the same directory. `verify-dispatch` resolves from the edited file's path; the compaction pair reads the event's `cwd` (DC-6) and asks the process seam. *Why not extend `install.sh` to register them:* it needs no extension — the existing hook-bundle mechanism (`harness/hooks/<name>/` + fragments, merged by `_agentm_merge_user_hook_fragments`, which rewrites each command to its absolute installed path) already does exactly this, so all three are additive and the installer was untouched. *Why the compaction marker writes through `write-state` rather than appending:* the resolved log may live in the vault, where a direct append bypasses `vault_lock.atomic_write` and can race the daemon. *A guard deliberately not built:* enforcing `supported_hosts` at install time was investigated and rejected — the machine-wide install has no Antigravity primitives destination, so filtering an antigravity-only primitive out of `~/.claude/` would deliver it nowhere rather than correctly; the missing piece is a delivery path, not a filter. *Re-audit triggers:* a fourth hook needing per-project behaviour that cannot resolve its project from either an edited file or the event cwd; an Antigravity primitives delivery path arriving, which makes the host filter meaningful for the first time; `supported_hosts` acquiring a reader in agentm (today it has none — only two writes in `persona_compile.py`).

**2026-08-24 — the per-project install is retired; AgentM installs machine-wide and nowhere else.** `install.sh --scope project` — the historical default, which copied a `.claude/` / `.harness/` / `.agents/` tree into each target repo — is deleted along with the code implementing it, the tests exercising it, the templates it delivered, the CI gates guarding those templates, and the docs describing it. `install.sh` drops 1336 → 764 lines and `install.ps1` 704 → 244. There is one install location, `$AGENTM_INSTALL_PREFIX` (default `~/.claude/`), and one command: re-running the installer *is* the refresh, so `--update` retires with it. `--scope`, `--update` and `--hooks` **fail** rather than being ignored, each naming its replacement — a stale caller passing one is announcing an expectation the installer no longer meets, and a silent no-op would let that ride until something downstream broke. *Why not keep per-project as a supported exception:* it was the documented default while being the path almost nobody wanted, so it carried the maintenance weight of a first-class mode — two delivery mechanisms with different on-disk shapes, a migration tool, a parity surface — to serve cases (team dotfiles, pinned config) that a machine-wide install plus per-project *state* already covers. *Why not a deprecation window:* nothing outside this machine consumes the installer, so a shim would have been ceremony with no beneficiary. *What this amendment reconciles in the body above:* the installer-boundary invariant still holds, but its enforcement moved — the copy-time `ensure_boundary_src` guard lived inside the `cp_managed` / `cp_user` helpers that existed only to serve per-project copying, so it went with them; the boundary is now asserted by the smoke suites' leak checks, which fail if this repo's own tooling or `wiki/` appears in a scratch install. *Re-audit triggers:* a genuine multi-tenant or CI-runner case where one machine must host two different AgentM versions simultaneously (the one scenario per-project scope actually solved); a host that stops supporting a user-level customization directory.

**2026-06-30 — Decisions section retired: taxonomy reduced from seven sections to six (machine-wide).** Once the ADR model retired and decisions moved into living-design amendment logs, the `decisions/` section held no surviving artifact class — so it is removed from the taxonomy, not kept as an empty slot. The body above now reads six sections (How-to · Reference · Architecture · Designs · Explanation · Operational; Architecture + Operational conditional). The shared scaffolder, the convention spec (`harness/documentation.md`), `check-wiki.py`'s folder map, and the project templates drop `decisions/` in the same pass; agentm + crickets delete their (already-empty) `wiki/decisions/` tombstones. *Why not keep an empty Decisions slot:* a permanently-empty always-present section is documentation drift that misleads every reader and re-scaffolds itself — the crickets [conventions](https://github.com/alexherrero/crickets/wiki/crickets-conventions) ADR-0020 re-audit trigger ("a universally-empty section — drop it") names exactly this case. *Why not a transient redirect stub:* the AG design-doc resolved (2026-06-19) the stub is only a revert-anchor, never the end-state; this finishes that migration tail. *Re-audit trigger:* a new durable decision-artifact class with no living-design home (re-add a section); a host that reintroduces the ADR model as the operator's default.
...
**The documentation convention (shared, current truth).** Both repos keep a narrative wiki at `wiki/`, organized by the **six-section taxonomy** — How-to · Reference · Architecture · Designs · Explanation · Operational (four always-present; Architecture conditional on a `wiki/architecture.yml` manifest, Operational conditional on non-public visibility; Tutorials fold into How-to). **The taxonomy is authoritative in the crickets [conventions](https://github.com/alexherrero/crickets/wiki/crickets-conventions) `documentation` domain — that is the single source for any conflict;** agentm follows it, and its `scripts/check-wiki.py` enforces the same six-folder map. There is **no `decisions/` section:** the agentm/crickets ADR model is retired and the section retired with it — a new decision amends the relevant living design, not a new record (this fold is itself an instance; the taxonomy dropped from seven sections to six in the 2026-06-30 pass below). Four agentm invariants carry forward: phase-boundary `documenter` writes (never inline — the rule against defensive documentation); the **installer boundary** (`install.sh` / `install.ps1` copy only from `harness/` + `adapters/` + `templates/`, never this repo's own `wiki/` / `scripts/` / CI, asserted by the smoke suites' leak checks — the copy-time `ensure_boundary_src` guard retired with the per-project copy helpers it lived in; see the 2026-08-24 amendment); the dumb-mirror GitHub-Wiki sync (the repo is source of truth); and deterministic structural lint over LLM judgment (`check-wiki.py`, [P3](#principles)). agentm's duplicate four-mode `diataxis-author` copy was retired toward crickets' canonical `wiki` plugin (graceful-skip when absent); the skill internals live in the crickets [wiki](https://github.com/alexherrero/crickets/wiki/crickets-wiki) design.

The two decisions this convention folded in (preserved with decision + why-not + re-audit; the "ADR" labels are historical — these are living-design entries, not records to amend):

- **Documentation convention (2026-04-01; was ADR 0002).** A narrative wiki at `wiki/` with `CamelCase-With-Dashes.md` globally-unique filenames and the four invariants above. *Why not docs-alongside-code:* the implementer biases toward confirming the plan, not describing what shipped, and it bloats implementer context. *Why not a single `DOCS.md` / auto-generated-from-code / YAML-status-frontmatter:* doesn't scale / humans can't freely edit / overhead nobody maintains. *Key assumption:* target projects want a wiki-shaped narrative; unused sections may collapse to a `README.md` pointer, but the frame stays for cross-project navigability.
- **Diátaxis spec → seven-section convergence (2026-04-21, amended 2026-06-11; was ADR 0004).** Replaced the audience-based four-subdir split with an intent-based Diátaxis split (tutorial / how-to / reference / explanation) + the deterministic `check-wiki.py` lint + mode-aware `documenter` writes; then converged 2026-06-11 onto the seven-section frame (since reduced to six — see the 2026-06-30 entry below) (matching crickets' standardized taxonomy that agentm's gate already enforced — the duplicate four-mode `diataxis-author` copy retired toward crickets in the same pass). *Why not the audience model:* every page ended up simultaneously reference + how-to + explanation, optimizing for none; intent (learn / do / look-up / understand) is how readers and agents actually navigate. *Why not LLM-judged quality:* structural lints catch ≥80% of authoring mistakes deterministically; the rest stays a human call. *Re-audit trigger:* a model bump that changes how agents consume mode-separated docs; a sixth documentation surface asserting a different taxonomy (reconcile to the crickets `documentation` domain, which is authoritative).

**2026-06-24 — folded ADR 0010 (MemoryVault internal taxonomy) into this design (AG ADR-migration tail, move-and-retire).** The held ADR 0010 resolved (design-doc amendment 2026-06-24) and folds here alongside the documentation convention — agentm's two "what-goes-where" taxonomies (the wiki seven-section frame above; the MemoryVault folder frame below) now share this conventions root. *(Its brief named `seven-section-convergence.md`, the doc-convention design since subsumed into this root, so 0010 lands here too.)*

**0010 — MemoryVault internal taxonomy (2026-06-05; amended 2026-06-18, V5-3).** The vault has four top-level areas with fixed semantics: **`personal/`** (the operator's cross-cutting personal data — preferences, domains, inbox, and the always-injected `_always-load/` subset; *not* project-keyed), **`projects/<slug>/`** (per-project work memory — `_harness/`, `decisions/`, `conventions/`, `wiki-style/`, `_index.md`; the `projects/_global/` pseudo-project holds on-demand cross-project globals), **`_idea-incubator/`** (pre-project ideas/research), **`_meta/`** (vault infrastructure — embeddings, indexes, backups, manifests). The governing rule: **anything project-keyed — including `_global` — lives under the top-level `projects/` root; never nest `projects/` under `personal/`** (project-keyed and personal are distinct axes, scoped through the one read path `_vault_projects_dir()`). The cross-project "house voice" lives in `projects/_global/wiki-style/`, **not** `_always-load/` (which auto-injects into *every* session — the context tax the on-demand global voice was designed to remove). *Updated 2026-07-05 (PLAN-r3-voice-mechanism task 3):* the base-voice source `docs-prose-style.md` no longer seeds `_always-load/` either — it demoted to `projects/_global/wiki-style/` alongside the other genre refinements, and a new ~15-line `voice-kernel.md` entry (the sound-like-the-operator essence + top anti-slop tells + a library index) is the only voice content still always-loaded. See crickets [wiki](https://github.com/alexherrero/crickets/wiki/crickets-wiki)'s 2026-07-05 amendment for the full cascade. *Why not nest project-keyed stores under `personal/`:* it conflates two unrelated axes (is-private? / is-project-scoped?) into one path segment and forks the projects root in two, so a reader scanning a project's memory misses half of it. *Re-audit triggers:* a vault-root rename (the `personal-private/` → `personal/` rename already fired this in V5-3, agentm v5.3.0 `053217b`, applied in lockstep across the vault dir + `_ALWAYS_LOAD_REL` + all path refs); any new vault-rename migration; agentm V6 indexed-recall (when the global tier becomes vector-discovered, "lives under `projects/_global/`" becomes a provenance tag, not a read path). *(`resolve_documenter_context()` returns `None` post-V5-3; documenter context is now provided by the V5-9 MCP memory server — the taxonomy itself is unchanged.)*

**2026-06-24 — folded ADRs 0003 / 0005 / 0006 / 0008 into this design (AG Phase 4, move-and-retire).**

**0003 — GitHub ProjectsV2 ownership and linking (2026-04-21).** Two-call flow: create the issue, then `gh project item-add` to classify it. `@me` must be passed literally (not interpolatable by the CLI); the `repo` field is the audit anchor for cross-project tracing. Why not a single GQL call: GitHub's API has no one-shot create-and-classify; the two-call split is required, not convenience. *Re-audit trigger:* if GitHub adds one-shot create-and-classify to the GQL API.
