...
> [!NOTE]
> **LAUNCHED (lifted 2026-06-24, AG Phase 3; originally approved 2026-06-21) · locked 2026-06-28 (final AG design sweep).** child-design — the Experience pillar, parent [agentm HLD](agentm-hld.md); the store this lifecycle tends is the [Memory System design](agentm-memory-system.md). `status: launched` (lifted into tracked `wiki/designs/` 2026-06-24, AG Phase 3); crystallization is still a designed piece carrying a `[PENDING-IMPL]` marker. **Forward learning is now designed and built** (2026-07-07, `PLAN-wave-e-experience` task 1) — the approved-source pipeline exists, thin v1, opt-in, see its own section below. **The runner they schedule on shipped 2026-07-06** (see the [Runner design](agentm-runner.md)) — this pillar's remaining consumer is unbuilt on its own merits, not blocked on the runner's existence.
...
- **Forward learning is now designed and built (thin v1); dreaming is still designed, not built** — the approved-source pipeline (`forward_learning.py`, `PLAN-wave-e-experience` task 1) ships opt-in with no sources configured by default and a single deterministic screening pass (no semantic judge sub-agent yet); dreaming remains a sketch beyond its own unbuilt state. Dreaming stays marked `[PENDING-IMPL]` above. **The runner they schedule on is built (2026-07-06)** — no longer part of either gap.
...
- **Re-audit triggers:** specify the accumulate loop now that forward learning has landed; revisit forward learning's thin-v1 note if a Pass-2 semantic judge is built or real sources are configured; flip dreaming to as-built as it ships.

## References

- `harness/skills/memory/scripts/` — `heat_policy.py` (promote/demote/pin), `reflect.py` (transcript mining), `ideas_incubator.py`, `adapt_skills.py`, `watchlist_review.py` (generalized to review both `_skill-watchlist/` and `_watchlist/`, `PLAN-wave-e-experience` task 1), `forward_learning.py` (the approved-source find/screen/classify pipeline, thin v1, opt-in; tested by `scripts/test_forward_learning.py`)
