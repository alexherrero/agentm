...
- **Dreaming's manual path is now fully built** — the revert-log primitive (task 1), the thin manual `/dream` pipeline (task 2), and the confirm/expire flow that wires staged proposals through `revert_log` (`harness/skills/memory/scripts/dream_confirm.py`, task 3) all ship. An operator can run `/dream` by hand today, review the proposals, and confirm them into a fully revertible apply. The only remaining gap is the scheduled, unattended pass that runs this on the runner instead of by hand (task 4).
