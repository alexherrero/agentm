...
*Task 3 of 7 (the write-time linker itself) shipped 2026-07-18 — see the as-built note after the paragraph below for how its timing actually works. Tasks 4 and 5 (the weekly link-improvement sweep and the ingest-batch handoff) have since shipped too, each with its own as-built note. The backfill and the connectivity meter (the rest of this section) remain designed-for.*
