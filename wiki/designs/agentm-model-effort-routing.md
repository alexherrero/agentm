...
- **enforced through [development-lifecycle](https://github.com/alexherrero/crickets/wiki/crickets-development-lifecycle)** — its agent-defs carry the `model:`+`effort:` frontmatter the host applies; its phase commands carry the advisory tier nudge. The as-built precedent is that plugin's *phase-aware-model-routing* (its folded ADR 0026 re-audit trigger has fired — see that design's 2026-07-04 amendment).
...
- **The chart decays on model releases** — model-version strings + the Gemini column go stale; **`content-refresh` carries the chart as a named checklist item** (mechanical re-pin auto-applied; a new-model *tier* placement surfaces for review), triggered by the weekly **CI model-drift detector**, and a model bump is also a P8 re-audit trigger.
...
- **As-built precedent:** crickets `developer-workflows` *phase-aware-model-routing* — `agents/worker.md` (`model: claude-sonnet-4-6`, realigned to T1 at ruling 8 — was `claude-opus-4-8`), `researcher.md` / `tech-lead.md` (`claude-sonnet-4-6`); the phase-command advisory nudges; global `~/.claude/settings.json` `opusplan`
