...
- [The persona tier](persona-tier) — the third classification above the substrate/plugin binary: a standing concern that composes capabilities it does not own (rememberer = degenerate persona; Planner (TPM) = first real persona). Refines the V5 unbundling ([agentm-hld](agentm-hld)); folds the former ADR 0016 (its load-bearing calls).
