...
### 2026-09-03 — Vault-level spaces resolve through a second backend instance (filing-v2 part 2b)

**Decision:** A space that sits above the backend's root — today the vault-root `Projects/` sibling of the memory root, where every project's working tree and `_harness/` state now lives — is reached through a second instance of the same backend class, rooted one level up and derived from the backend's own root (`_root_projects_candidates` in `harness_memory.py`). The candidate is offered only when the backend reports `capabilities.sync`, the root is nested inside an Obsidian vault — `.obsidian/` at the parent and none at the root itself, the witness the pre-tag review added after showing that a flat vault's parent is the operator's home, where a `Projects/` directory is common — and the sibling directory already exists; nothing creates it. The same predicate (`_root_projects_dir`, vendored per skill script and pinned byte-identical by test) governs every Path-based walker and writer, and it also names the flat generation `<memory-root>/Projects` — matched by the directory's exact name, since on a case-insensitive disk `Projects/` answers for the V4-era `projects/` rung and every legacy project would read as root-space. `resolve_project` probes the root candidates first and reports `layout: "root"`; the desk and legacy rungs stay behind them as documented fallbacks. See §6.

**Why not a `..` Locator:** Locators are root-confined by construction (namespace operations only, no parent traversal), and that confinement is what keeps "swap the backend, the engine doesn't notice" true. A relative escape would also be joined onto the memory root by every downstream consumer that treats a key as vault-relative — on a case-insensitive disk that mis-roots writes onto the operator's own folders silently, the failure class the vault-path convention (see [CI-Gates](../reference/CI-Gates)) exists to prevent. A second instance keeps every key relative to *its* root.

**Why not the sibling root from global config:** the backend's own root is the one truth that survives a relocated vault, a test vault, or a foreign host; a configured absolute path re-introduces the hardcoded-path class `check-no-hardcoded-vault-path` forbids, and would let the Python and Go sides disagree about where the space is.

**Re-audit triggers:** (1) a backend with no filesystem parent (a remote store) — the candidate list must then come from the backend's own space table, not `root.parent`; (2) the daemon's `spaces.projects` (`Projects`, vault-root-relative) and the Python `_DEFAULT_SPACES["projects"]` (`../Projects`, memory-root-relative) naming different places — `test_memory_root.py` and the Go config tests pin each side, and the memory-root consistency gate allows exactly this one sibling; (3) filing-v2 part 3 retiring the last desk-era path — drop the `desk/projects` rung then, and fold `_vault_projects_dir` into the candidate list; (4) a vault presented without `.obsidian/` (a non-Obsidian front end) — the witness would then have to come from the config's `memory_root` key instead of the marker.
