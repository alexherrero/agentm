...
- `.agent/`, `.agents/`, `.codex/`, `.gemini/` — adapter trees for the other supported tools.
...
- **State vs. managed files are separated.** `.harness/` and `wiki/` are yours; `.claude/`, `.agent/`, `.agents/`, `.codex/`, `.gemini/` are managed and refreshed on `--update`.
