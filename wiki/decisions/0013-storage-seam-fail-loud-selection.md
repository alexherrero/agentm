...
<!-- V5-7 all 3 tasks shipped: task 1 — vault_path() reads plugins.obsidian-vault.vault_path first; task 2 — --vault-path writes the plugin-namespaced key + storage.backend=vault; task 3 — choose_protocol lost its vault_root parameter; the implicit "else an existing vault_path" rule is removed. Resolution chain is now 2-step: explicit storage.backend wins; else device-local. -->
The selection resolver (`scripts/backend_selection.py`) maps the on-device install config to a concrete, registered backend via the chain: an explicit `storage.backend` config value wins (always required for vault — set automatically by `--vault-path`); else `device-local`. **V5-7 task 3 (shipped):** the implicit third-precedence rule that would select `vault` when an existing `vault_path` was detected without `storage.backend` has been removed; `choose_protocol` no longer accepts a `vault_root` parameter. Vault selection is now always explicit. When the configured backend **can't be produced**, selection raises `StorageSelectionError` and **never** falls back silently. Three refusal cases:
