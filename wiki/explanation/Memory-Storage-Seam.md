...
> [!NOTE]
> **All 5 parts complete (V5-1, shipped in v5.0.0).** Part 1 shipped the *abstract* contract: the verbs as an abstract `StorageBackend`, the `Locator`/`Info`/`Capabilities` types, the `conflict_strategy` slot, the named-backend registry, the three-tier source/derived taxonomy, and the gate that keeps a `Path` from crossing it. Part 2 added the **first concrete backend** — [device-local](Storage-Seam#the-devicelocalbackend), plain markdown under `~/.agentm/memory/`. Part 3 added the **backend-agnostic conformance suite** — the deterministic gate that holds every backend to one objective contract (the verb behaviors, the byte-identical LF-exact round-trip, the `list`-on-absent pin, and the gated derived-layer rebuildability invariant); it is **not** a new `check-*` gate — it rides the existing cross-OS `[T]` unit-test step, which is what makes the LF-exact case real on the Windows runner. Part 4 added the **second concrete backend** — [the vault wrap](Storage-Seam#the-vaultbackend), today's synced Obsidian/GDrive vault reached through the seam, with `write` composing the full V5-0 `vault_mutex` + content-hash CAS + `atomic_write` stack and a never-orphan invariant proving the wrap moves no data. Part 5 — the final part — shipped **backend selection + the fail-loud guard** (plus the `Capabilities`-read and a `doctor` storage preview): a `storage.backend` config key and a resolver (`scripts/backend_selection.py`) that maps config → a registered backend, refusing loudly with `StorageSelectionError` when the named backend's plugin is uninstalled (and never demoting to device-local). The fail-loud-never-demote decision is recorded in [ADR 0013](0013-storage-seam-fail-loud-selection.md). **Two things remain beyond V5-1.** First, **the engine is still not wired to the seam**: `recall`/`reflect`/`save`/`evolve` and the five hooks are byte-unchanged, still reading state the way they do today — routing the engine's public API through the selected backend is the **engine cutover**, a separate, later step. Part 5 resolves *which* backend a config would use and refuses a misconfiguration, but the live read/write path does not yet route through the seam. Second, **no derived index or abstract promotion ships yet** — the index lands in V6. Everything below about the *index* describes the contract V6 will conform to — not behavior that exists in `main`; everything about the *engine cutover* describes a step beyond this plan entirely.
...
The device-local floor is the *new* default; the [vault backend](Storage-Seam#the-vaultbackend) is the *existing* state given a new door. Today the engine reaches its synced Obsidian/GDrive vault by touching the filesystem directly. The vault backend re-presents that same vault — the same files, at the same paths — through the seam verbs instead. The design statement here is one of **conservation, not conversion**: wrapping the vault must not move, reshape, or migrate a single byte of an operator's live memory. It is a second way *in* to the existing tree, so that part 5's selection can *resolve* either backend by name — and the later engine cutover can point the engine at the resolved one — without the operator's data ever having been touched.
...
Like device-local, the vault backend *exists and works* but the live engine does not yet use it. Part 5 lets selection *resolve* it by name (an existing `vault_path` selects it); pointing the public API at the chosen backend — the **engine cutover** — is a separate, later step beyond V5-1.

## What this seam does *not* touch

The seam sits strictly *below* the engine's frozen public API. `recall` / `reflect` / `save` / `evolve` and the five memory hooks (the DC-7 surface) are byte-unchanged by this work; the storage seam never widens that surface. It is an *internal* refactor boundary — how the engine reaches its bytes — not a new public capability. A process calling the engine sees no difference; only the engine's own state access is rerouted through the verbs (and that rerouting — the **engine cutover** — is a separate, later step **beyond V5-1**, not part 5).

Part 2 does **not** change that, and neither does part 5. Shipping a concrete backend means the backend *exists and works* — you can construct a `DeviceLocalBackend` and call its verbs — but it does not mean the live engine *uses* it. Nothing in `recall`/`reflect`/`save`/`evolve` or the hooks now routes through `device-local`; they still access state exactly as before. **Part 5 narrows to selection + the fail-loud guard** (plus the `Capabilities`-read and the `doctor` preview): it lets the kernel *resolve which* backend a config would use — `device-local` on a fresh install, `vault` when a `vault_path` exists, a named plugin when `storage.backend` is set — and refuse loudly when the named backend's plugin is uninstalled. But "a fresh engine routes its public API through the selected backend" — the **engine cutover** — is a *separate, later step beyond V5-1*, not the part-5 outcome. After part 5, selection can name and refuse a backend; the engine still has not been pointed at it. That last wiring is deferred precisely so the selection-and-refusal contract can be proven on its own before the live read/write path moves.
