...
> [!NOTE]
> **Parts 1–4 of 5 complete.** Part 1 shipped the *abstract* contract: the verbs as an abstract `StorageBackend`, the `Locator`/`Info`/`Capabilities` types, the `conflict_strategy` slot, the named-backend registry, the three-tier source/derived taxonomy, and the gate that keeps a `Path` from crossing it. Part 2 added the **first concrete backend** — [device-local](Storage-Seam#the-devicelocalbackend), plain markdown under `~/.agentm/memory/`. Part 3 added the **backend-agnostic conformance suite** — the deterministic gate that holds every backend to one objective contract (the verb behaviors, the byte-identical LF-exact round-trip, the `list`-on-absent pin, and the gated derived-layer rebuildability invariant); it is **not** a new `check-*` gate — it rides the existing cross-OS `[T]` unit-test step, which is what makes the LF-exact case real on the Windows runner. Part 4 adds the **second concrete backend** — [the vault wrap](Storage-Seam#the-vaultbackend), today's synced Obsidian/GDrive vault reached through the seam, with `write` composing the full V5-0 `vault_mutex` + content-hash CAS + `atomic_write` stack and a never-orphan invariant proving the wrap moves no data. **No index or abstract promotion ships yet**, and **the engine is not yet wired to the seam**: `recall`/`reflect`/`save`/`evolve` and the five hooks are byte-unchanged, still reading state the way they do today. **Part 5 — the final part — ships backend selection + the fail-loud guard** (plus a `Capabilities`-read and a `doctor` storage preview): a `storage.backend` config key and a resolver that maps config → a registered backend, refusing loudly when the named backend's plugin is uninstalled. **Part 5 does *not* include the engine cutover.** Routing the engine's public API through the selected backend is a **separate, later step beyond V5-1** — selection resolves *which* backend and refuses a misconfiguration, but the live `recall`/`reflect`/`save`/`evolve` still does not route through the seam after part 5. The actual derived index lands in V6. Everything below about *selection* and the *index* describes the contract those parts will conform to — not behavior that exists in `main`; everything about the *engine cutover* describes a step beyond this plan entirely.
