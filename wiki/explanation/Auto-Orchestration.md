...
- **At the phase boundaries**, a finished `/work` session is meant to get reflected and a finished `/release` to refresh the skill surfaces, without you running `reflect` or `index-skills` by hand. The entry point is `phase_dispatch()` in [`scripts/harness_memory.py`](https://github.com/alexherrero/agentm/blob/main/scripts/harness_memory.py) (V5-5 / LC-3); the valid phases are locked in the `_BRIDGE_PHASES` frozenset in that same module. **This one is not live** — the bridge works, but no plugin currently calls it (see the Developer plugin note below). Session reflection still happens on every session through the Stop hook, so the visible behavior is close enough that the gap went unnoticed.
...
- **Developer plugin** — the intended phase-boundary trigger, calling in through the `phase_dispatch()` bridge described [above](#the-gap-it-closes) rather than re-implementing the orchestration logic. **Nothing calls it today.** The bridge is built and tested, but the plugin that used to call it (`crickets/developer-workflows`) was retired, and its replacement (`crickets/development-lifecycle`) does not — a grep for `phase_dispatch` across the installed crickets tree returns nothing. So `post-work` reflection and `post-release` skill-refresh do not fire automatically at phase boundaries right now; the Stop hook still reflects every session, which is why this has gone unnoticed. Re-wiring a caller is a crickets-side change.
