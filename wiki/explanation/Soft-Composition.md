...
The two fields stay separate because the resolver behind them is a simple checker, not a dependency solver. All it does is answer one question: is this other capability installed? That single limit is what splits the two kinds of dependency. Something a plugin genuinely can't run without has to be a hard `requires:`, because only the host that installs the plugins can guarantee it's there. Everything the resolver handles is optional by nature — which is exactly what `enhances:` means: nice to have, and when it's absent the plugin just falls back to working on its own. It's tempting to let the resolver chase the must-haves too and fold the two fields into one, but then it would have to fail the install when something required is missing, and that's the host's job, not the resolver's. So the resolver stays a plain yes-or-no check, and the two fields stay apart. The rest of this page fills in each half.
