...
> [!NOTE]
> **V5-6** (2026-06-18): the non-local value renamed `"vault"` → `"backend"`. Existing `state_mode: vault` entries alias transparently at read time — no operator action, no file rewrite. `"local"` is unchanged. See [Memory-storage seam](memory-storage-seam).
