...
> [!NOTE]
> **Status:** implemented — declared by [`.harness/PLAN.md`](https://github.com/alexherrero/agentm/blob/main/.harness/PLAN.md) "V5-10 part 1 — Multi-plan state (agentm substrate slice)". The substrate shipped — the round-trip lock, `resolve_active_plan`, the `check-multi-plan-naming` gate, and the named-plan-aware session-start hooks + `doctor` are in `main`. The crickets behavioral half shipped (the `developer-workflows` `/work`·`/plan`·`/review` `--name <slug>` flag → scoped `PLAN-<slug>.md` / `progress-<slug>.md`, plan "Multi-plan writers", 2026-06-12), and the last dependency — the `.harness/active-plan` marker **writer** — shipped in crickets developer-workflows v3.7.0 (2026-06-13): `spawn_worker.py` writes `<worktree>/.harness/active-plan` (bare slug + newline), read back by `resolve_active_plan`.
...
> [!NOTE]
> **Contract lock landed; feature fully implemented.** The round-trip half above is test-locked as of "V5-10 part 1" task 1, `resolve_active_plan` (the session→plan binder, [below](#binding-a-session-to-its-plan--resolve_active_plan)) is test-locked as of task 2, the `check-multi-plan-naming` gate shipped as of task 4, and the **named-plan-aware session-start hooks + `doctor`** shipped as of task 5 ([trace below](#what-has-shipped-so-far)). The crickets behavioral half shipped (the `--name <slug>` flag across `/work`·`/plan`·`/review`, 2026-06-12), and the last remaining piece of this substrate — the `.harness/active-plan` marker *writer* (component (2)'s worktree-spawn helper) — shipped in crickets developer-workflows v3.7.0 (2026-06-13).
...
All dependencies are now shipped: the `.harness/active-plan` marker **writer** (component (2)'s worktree-spawn helper, crickets developer-workflows `spawn_worker.py`, v3.7.0, 2026-06-13) and the crickets behavioral half (`/work`·`/plan`·`/review` `--name <slug>`, the `progress-<name>.md` append writer, **2026-06-12**). See [The agentm/crickets seam](#the-agentmcrickets-seam).
