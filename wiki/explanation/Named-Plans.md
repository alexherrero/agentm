...
> [!NOTE]
> **Status: implemented** (2026-06-13, updated 2026-09-15). The multi-plan-state substrate — the round-trip lock, `resolve_active_plan`, the `check-multi-plan-naming` gate — ships in `main`; crickets' development-lifecycle plugin writes the `.harness/active-plan` marker this page's reader consumes. See Build history below for the full rollout, including the task-layout plan, the tracker, and the projects-migration resolver half.
...
A task's plan takes a third shape (agentm-vault plan 09): `tasks/<slug>/plan.md`, with its `progress.md` and `tracker.md` beside it in the same directory, all three addressed by absolute path rather than a bare filename under `_harness/`. It resolves only on a synced backend, since a device-local `.harness/` has no project directory to hold a `tasks/` tree. Within a slug, a task at `tasks/<slug>/plan.md` is tried first, and the flat `PLAN-<slug>.md` pair is the fallback — a second place a slug can resolve, alongside the flat naming contract above.
