...
Why every phase command can file deferred-work items to a GitHub Project — and why the integration is opt-in, preview-and-ask at every write, and a silent no-op when unconfigured. The ownership mechanics are recorded in [ADR 0003](0003-ProjectsV2-Ownership-And-Linking).
...
Opt-in happens once, at `/setup`: the operator is offered a project, and on yes the agent runs the two-step ProjectsV2 dance — `gh project create` then `gh project link`. The two steps are load-bearing, because ProjectsV2 has no repo-owned form: to make a project appear under the repo you must link a user- or org-owned project to it. The rationale is [ADR 0003](0003-ProjectsV2-Ownership-And-Linking). On success, `.harness/project.json` records `{owner, number, url, repo}`; the *absence* of that file is the signal every downstream phase uses to skip silently.
...
- [ADR 0003 — ProjectsV2 ownership and linking](0003-ProjectsV2-Ownership-And-Linking) — why the create-then-link dance.
