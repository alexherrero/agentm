...
> [!NOTE]
> **All 5 parts complete** (V5-1, v5.0.0). The seam, both concrete backends (device-local, vault), and backend selection with a fail-loud guard all ship. No derived index or abstract promotion yet — that's V6; everything below about the *index* describes V6's target contract, not current `main` behavior. See Build history below for the part-by-part rollout.
...
Two answers lead this page, because a storage-agnostic engine has to settle two things: what it reads *through*, and what it reads *by default*. The first answer is the interface below — a small set of verbs that lets the backing swap without the engine noticing. The second is the default that interface points at: **plain markdown files on the user's own disk**. That default is an intentional choice. A memory engine should work the moment it's installed — no vault to mount, no Drive to authenticate, no database to provision — so the floor has to be the simplest thing that already satisfies "storage-agnostic," and bare `.md` files under `~/.agentm/memory/` are exactly that. Anything heavier — a synced vault, a database, a remote service — is a job a *plugin* takes on, never a cost the engine imposes on a fresh install. (The deep-dive on that floor, and the write-safety it does and doesn't need, is [The first concrete backend](#the-first-concrete-backend-the-bare-markdown-floor) below.)

The tempting answer is "the engine already knows it's files — let it keep calling `pathlib` and `open()`." That bakes one filesystem layout into every read and write the engine makes. Swapping the backing — to a device-local default, to a future bytes or remote channel — would then mean editing the engine itself, everywhere it touched a path. The seam is the alternative. It is a small interface (`scripts/storage_seam.py`) of the verbs a storage layer actually needs:
...
The vocabulary mirrors [fsspec](https://filesystem-spec.readthedocs.io/)'s method names and its named-protocol registry pattern — a well-trodden public shape for "one interface, many filesystems" — but the seam imports neither fsspec nor any database; bare markdown is the floor, and the dependency here is just a borrowed convention. When a backend needs an operation the verbs don't expose, it can widen the contract openly instead of reaching past it quietly.
...
Three small types travel with the verbs, each kept lean by design:
...
`DerivedMaintenance` reserves two operation *names* — `reindex` (rebuild a derived tier from source) and `changed_since` (the incremental feed of source locators newer than a watermark) — but ships no implementation. The class is abstract; there is intentionally no concrete subclass, and a scope guard test asserts both that fact and that the module imports no database or index library. The index itself is V6.

Reserving the names without the bodies is a sequencing call. The V6 index is a large piece of work, and the real risk isn't a missing reindex function — it's that the index lands and the engine has no shaped place to call it from, so it bolts on awkwardly. Naming `reindex` and `changed_since` now gives V6 an affordance the contract already anticipated: an incremental feed keyed on `mtime`, and a full rebuild that reads from source. The shape gets decided while it is cheap to decide, in an abstract class with no callers. And the class is deliberately un-instantiable, so "named, not built" is enforced by structure instead of a comment.
...
The registry's key design choice is what it does on a *miss*. Resolving an unregistered name does **not** raise — `get` returns `None`. That follows the same distinction the seam draws everywhere else: **absence is not corruption.** A missing file degrades to `FileNotFoundError`; only a malformed key (a `..` escape) raises `InvalidLocatorError`. The registry extends that stance to *naming*: an unregistered protocol simply reads as absent. The registry's job is to *report* absence, leaving the decision about what it means to the caller.

Deciding what it means is part 5's job, and keeping that decision *there* is the point. Selection reads the `None` and fails loud: if the configured backend doesn't exist, that is a fatal misconfiguration the operator must see. Folding the raise into `get` would scatter that policy across every lookup; leaving `get` honest about absence concentrates the fail-loud in one place. The split is absence here, fail-loud there.
...
The contract above is an empty stage until something stands on it. The [device-local backend](Storage-Seam#the-devicelocalbackend) is the first thing that does — and the choice of *what* the first backend should be is itself a design statement.

It is **plain markdown under `~/.agentm/memory/`**: a user-owned directory of `.md` files, with no service, no daemon, and no embedded database. This is the floor the kernel commits to. The V5 unbundling's whole premise is that a memory engine should be storage-agnostic and should default to something a fresh install can use with zero setup — no vault to mount, no Drive to authenticate, no database to provision. Bare files on the user's own disk are the simplest thing that satisfies that, and the simplest thing is what the kernel should ship. Anything heavier — a SQLite or vector store, a synced vault, a remote service — is something a *plugin* may offer, never the default the engine assumes. (A database on a synced path is, specifically, a corruption pattern the [three-tier taxonomy](#the-three-tiers-and-why-the-index-never-syncs) already encodes as never-sync; making the default a database would bake that hazard into the floor.)
...
What the vault backend adds over device-local is the write-safety the synced, multi-writer reality demands, built by composing existing primitives. `write` serializes fleet-local agentm sessions on the one per-vault advisory mutex, then lands through a content-hash-CAS-guarded atomic write that catches a *non-mutex* writer (a GDrive sync, another device) slipping in between the pre-write read and the rename. This is the full V5-0 [vault write protocol](Vault-Write-Protocol) the abstract contract always specified a filesystem backend would compose — assembled here from the `vault_lock` primitives directly, so the seam never reaches up into the engine module it sits below.

The conflict posture is the one place the vocabulary invites a misreading worth heading off. The vault declares `conflict_strategy = "whole-file"` and `conflict_files = True` — but `"whole-file"` is a *name*, not a *merger*. agentm ships no automated whole-file (or any) merge. When Drive sync diverges, DriveFS materializes a "(conflicted copy)" sibling rather than losing a write; the existing `detect_conflict_files` + `conflict-merger` SessionStart machinery *surfaces* each conflict/base pair to the operator, and resolution stays human judgment. The string `"whole-file"` exists for part-5 selection to *read* when it decides how to treat a divergence: it declares the reconciliation unit, it does not perform one. A line-level automatic merge would be a future CRDT strategy that has not shipped.
...
Part 1 shipped the *abstract* contract: the verbs as an abstract `StorageBackend`, the `Locator`/`Info`/`Capabilities` types, the `conflict_strategy` slot, the named-backend registry, the three-tier source/derived taxonomy, and the gate that keeps a `Path` from crossing it. Part 2 added the **first concrete backend** — [device-local](Storage-Seam#the-devicelocalbackend), plain markdown under `~/.agentm/memory/`. Part 3 added the **backend-agnostic conformance suite** — the deterministic gate that holds every backend to one objective contract (the verb behaviors, the byte-identical LF-exact round-trip, the `list`-on-absent pin, and the gated derived-layer rebuildability invariant); it rides the existing cross-OS `[T]` unit-test step rather than a new `check-*` gate, which is what makes the LF-exact case real on the Windows runner. Part 4 added the **second concrete backend** — [the vault wrap](Storage-Seam#the-vaultbackend), today's synced Obsidian/GDrive vault reached through the seam, with `write` composing the full V5-0 `vault_mutex` + content-hash CAS + `atomic_write` stack and a never-orphan invariant proving the wrap moves no data. Part 5 shipped **backend selection + the fail-loud guard** (plus the `Capabilities`-read and a `doctor` storage preview): a `storage.backend` config key and a resolver (`scripts/backend_selection.py`) that maps config → a registered backend, refusing loudly with `StorageSelectionError` when the named backend's plugin is uninstalled, never demoting to device-local. The fail-loud-never-demote decision is in the [Memory-storage seam design](memory-storage-seam). The engine cutover shipped later, in V5-3 (v5.5.0); the [Memory-storage seam design § 3](memory-storage-seam) is the canonical account of what it changed.
