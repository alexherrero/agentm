...
| `audit(vault_path)` | `audit(vault_path: Path \| str, *, vault_root=None, spaces: dict \| None = None) -> dict` (`kind_registry.py:242`) | Read-only scan of a vault's `kind:` values. Returns `{"by_kind": {kind: count}, "malformed": [(path, raw_kind)], "unrecognized": [(path, raw_kind)], "total_files": int}`. Walks the whole vault root the memory root sits in (`corpus_root`, `kind_registry.py:91`; `corpus_notes`, `kind_registry.py:134`): the `vault_root` a caller passes, else the memory root's parent when an `.obsidian/` there witnesses a vault, else the memory root itself. It skips dot-directories, the contract's `recall_exempt_areas` (never entered, so a walled file is never opened), `_archive/` dirs and `PLAN.archive.*` files. Paths are keyed from the vault root (`agent/memory/…`, `projects/…`), and an optional `spaces` dict is filled with the notes read per top-level directory. The hand-listed roots it replaced (`_WALK_SUBDIRS`) had fallen behind the vault's moves; `graph_snapshot.py` keeps a hand-listed walk of its own, for the reason its `_walk_vault_paths` gives. `"malformed"` is a raw value failing `is_kebab()`; `"unrecognized"` is valid kebab-case but absent from `KNOWN_KINDS`. A note with no extractable `kind:` at all still counts toward `total_files` but appears in no other bucket. |
...
The `validate_vault(vault_path, *, scope_dirs=("personal", "projects"))` function (`frontmatter_validator.py:95`, signature `validate_vault(vault_path: Path | str, *, scope_dirs=_DEFAULT_SCOPE_DIRS) -> dict[str, list[str]]`) walks every `*.md` file under the vault's scope dirs. It returns `{rel_path: [violations]}` for notes with at least one violation. It omits clean notes entirely. It skips `_archive/` dirs and `PLAN.archive.*` files, the same two exclusions the `audit()` walk from `kind_registry.py` makes, though that walk covers the whole vault root rather than these scope dirs.

**DC-4 exclusion dirs.** The `_EXCLUDE_DIRS` constant in `frontmatter_validator.py` (a `frozenset`, matched per path segment: `_idea-incubator, _meta, _inbox, scratch, _archive, _opinions, _crystallize-staging, tasks`) mirrors the `_EXCLUDE_DIRS` constant in `vault_lint.py` exactly. This is the DC-4 convention. These subdirectories carry non-memory-entry content. This includes a project's session-owned plan state and staging areas. You never meant for these to satisfy the universal frontmatter contract. (Several of the named directories — `_meta`, `scratch`, `_crystallize-staging` — were themselves retired from the vault, `_meta` in two steps: filing-v2 part 2a retired everything under it except the seam-owned `repos.json`, and the memory-root trims (agentm-vault plan 05) moved that registry out too, to the engine state directory, retiring `_meta/` in full. The entries stay in the set so a leftover copy in an unmigrated vault never floods the walk. `_harness` sat in this set until agentm-vault plan 15 retired the vault's per-project `_harness/` directory outright and replaced the entry with `tasks` — a project's plan, progress and tracker now live under `tasks/<slug>/`, which carries no frontmatter by design just as the old `_harness/PLAN.md` didn't.) This exclusion prevents false positives. The first draft's walk lacked it. A real-vault run flooded about 1800 false `"no frontmatter block found"` violations. These targeted plain harness state files like `projects/<repo>/_harness/PLAN.md` and `progress.md`, since renamed `projects/<repo>/tasks/<slug>/plan.md` and `progress.md`. These files carry no frontmatter by design. You wrote `test_excludes_harness_meta_inbox_dream_staging_dirs` in `scripts/test_frontmatter_validator.py` as the regression test for that bug.
...
| Exclusion dirs | N/A | `_EXCLUDE_DIRS` (`frontmatter_validator.py`) mirrors `vault_lint.py`'s own `_EXCLUDE_DIRS` exactly (DC-4): `_idea-incubator, _meta, _inbox, scratch, _archive, _opinions, _crystallize-staging, tasks` (`_harness` retired from the set under agentm-vault plan 15) |
...
You wired a `check-kind-taxonomy` step (`scripts/check-kind-taxonomy.sh`) into `scripts/check-all.sh`. This runs the task-1 `audit()` through `check-vocabulary-membership.py --report`, over the corpus that gate resolves: `$MEMORY_ROOT` when exported, else the configured vault. It performs a graceful skip (exit 0) only when nothing resolves. Until 2026-09-20 it read `$MEMORY_ROOT` alone, and since `check-all.sh` has never exported either name, every battery run skipped. It acts like the existing `check-slop.py --report wiki` step. It is **report-only, never a failing gate**. You cannot use a hard PASS/FAIL on the real vault's 47-value mess yet. You must first make an operator judgment call on normalization. You parked this in [agentm issue #273](https://github.com/alexherrero/agentm/issues/273). The script prints violations as a report. The step's own exit code stays 0 regardless of findings. You confirmed this via subprocess-driven tests in `scripts/test_check_kind_taxonomy.py`. These tests verify an unset path, nonexistent path, clean vault, and a vault with real violations all exit 0. You can read the gate table entry in [CI Gates](CI-Gates).
