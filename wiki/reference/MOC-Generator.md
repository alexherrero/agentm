...
Browsing the vault by memory type is now the dreaming binary's job, not this script's. `daemon/internal/dreaming/mocs.go`'s `mocs` job writes a generated page per memory **type** (`type:` — the six values the filing contract registers, not the free-form `kind:` field this module's retired pages grouped by) under `memory/mocs/`, once a type holds at least `moc_min_members` live notes, and removes the page when the type falls below that; `moc-memory.md` lists every type, at or past the floor by its page's link and below it in full; `moc-root.md` is the generated entry point, listing every area's map. The same job maps the projects space (agentm-vault plan 09): `moc-<slug>.md` at each project's root, `Projects/moc-tasks.md` and `Projects/moc-projects.md`. Since task 176 step 6 the same job also (re)generates each project's own `tracker.md` from its task trackers — `PlanProjectTrackers` (`daemon/internal/dreaming/projecttrackers.go:199`, wired in right after the project maps at `daemon/internal/dreaming/run.go:275-282`); no session writes a project's tracker any more. See [Memory daemon reference § the dreaming binary](Memory-Daemon#the-dreaming-binary-agentmdream) for the tracker's shape. As of 2026-09-24 it also maps the two shared spaces (`daemon/internal/dreaming/spacemaps.go`): `resources/moc-resources.md` and `systems/moc-systems.md`, once either space holds a note — neither space exists in the vault yet. See [Memory daemon reference § the dreaming binary](Memory-Daemon#the-dreaming-binary-agentmdream) for the mechanism, and [CI gates reference](CI-Gates) for the gates that hold `mocs/` to that shape.
