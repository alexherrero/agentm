...
> [!IMPORTANT]
> The server documented on this page is **retired and removed**. Its launchd job
> (`com.agentm.memory-server`) was stopped when the Go daemon took over port
> 7821, and it had no live caller — nothing in `settings.json` or the recall
> hooks ever pointed at it. Its source, its plists and its own doctor left the
> tree on 2026-09-07 (agentm-vault design, landing group 11a). This page is kept
> for the tool shapes other pages still reference; the four tools below are not
...
The filing engine runs a corpus check during capture. A `deduplicated: true` result means the check matched an existing, live, not-yet-reviewed candidate and reinforced it instead of filing a new one. The engine refuses the match — and files a fresh candidate instead, under a name grown from the capture's own title and text if a different note already uses the target slug (agentm-vault § The card's collision rule: the next meaningful word, never a `~dup` mark, falling back to a hash suffix when no word frees the name) — when your capture carries a `source_url` or `instructions` value the matched candidate lacks, so a link resend's ingest-sweep trigger is never silently lost to a plain-text duplicate.
