...
|---|---|---|---|
| `required-field` | error | A required frontmatter field (every field except the optional ones — `fingerprint`, `supersedes`, `lifecycle_tier`, `heat_pin`) is missing. | Add the missing field in the locked order. |
...
| `field-order` | warn | The present frontmatter fields appear in the locked order (`kind, status, created, updated, tags, group, slug, fingerprint, always_load, supersedes, lifecycle_tier, heat_pin`). | Reorder frontmatter to the locked order. |
