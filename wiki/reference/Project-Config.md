...
| Where does the file resolve to? | Vault-resident post-V4 #26: the project directory's own `desk/project.json` (agentm-vault § Projects and tasks — `desk/` is the project's home for machine files since the projects migration, plan 15 retired the older `_harness/project.json` fallback rung). The writer routes through `harness_memory.write_machine_file` (`.project-mode`-aware — routes to the repo-local home `<repo>/.harness/project.json` for local-mode projects, or any project with no vault; distinct from the migrate-nagging legacy fallback). |
