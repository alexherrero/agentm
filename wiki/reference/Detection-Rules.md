...
|---|---|---|---|
| `R-harness` | `harness/phases/` dir present | — (sets `bypass` verdict) | This IS the harness source repo → detection is skipped. JSON reports `legacy_harness_present: true` when a `.harness/` state dir is also present, to offer migration. |
...
| `R-tests` | `tests/` or `spec/` dir, or a test-glob file (`*_test.py`, `test_*.py`, `*.test.*`, `*.spec.*`) | `evidence-tracker` hook | Enforces test-reading before `/work` task closeout. |
