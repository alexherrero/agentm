...
| `R-vault-content` | `_index.md` file, or `decisions/` dir **and** `conventions.md` (checked at `docs/conventions.md` only — `conventions.md` moved to `docs/` for a vault project under task 176 step 3, and a root copy no longer matches) | `memory` skill + the 4 memory hooks (`memory-recall-session-start`, `memory-recall-prompt-submit`, `memory-reflect-idle`, `memory-reflect-stop`) | Looks operator-personal → memory captures context. |
