...
| `--no-register` | `-NoRegister` | Skip the auto-registry step at end of `--apply`. By default the migration tool auto-calls `python3 scripts/repo_registry.py register <slug> --root <target>` so the repo shows up in [`/list-plans`](../how-to/Use-Auto-Context-In-Harness-Phases.md) + other cross-repo views. Pass `--no-register` for explicit opt-out (e.g. for ephemeral test targets). |
