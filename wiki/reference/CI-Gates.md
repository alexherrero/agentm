...
| verify-phases _(pending — lands in Hardening I plan task 7)_ | A full phase lifecycle (`/setup → /plan → /work → /release`) drives its deterministic seams — state read/write, `progress.md` appends, `features.json` updates, post-phase dispatch plumbing — end-to-end on a throwaway fixture project, run twice: once vault-resident, once repo-local. Linux/Mac only. | `scripts/verify-phases.sh` |
| verify-memory-roundtrip _(pending — lands in Hardening I plan task 7)_ | The memory engine round-trips on a throwaway fixture vault: `save` → `recall` surfaces it → `reflect` processes a synthetic transcript → `vec-index`/`embed` builds and reads back → `vault_lint` clean. Hermetic, stdlib embed path (no network). Linux/Mac only. | `scripts/verify-memory-roundtrip.sh` |
