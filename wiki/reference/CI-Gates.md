...
One command runs the deterministic battery — 21 gates: unit tests + every `check-*` gate + the four integration checks (`verify-v4`, `verify-orchestration-briefing`, `verify-phases`, `verify-memory-roundtrip`) — prints a PASS/FAIL table, and exits non-zero on any failure:
