...
| `fail` / `1` | Unregistered plugin; **or** `vault` selected with no resolvable `vault_path`; **or** the config file exists but is unparseable / names a non-string `storage.backend`. | The unregistered-plugin line reuses the **same** `_install_plugin_message` ([`#L293`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L293)) the [fail-loud guard](#the-fail-loud-guard) raises ([`#L196`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L196)), so the preview and the live refusal are **byte-identical**. |

Proven by `TestStoragePreview` (6 cases, [`scripts/test_backend_selection.py#L324`](https://github.com/alexherrero/agentm/blob/main/scripts/test_backend_selection.py#L324)). See the how-to [Choose a storage backend](Choose-A-Storage-Backend#what-okwarnfail-mean) for the operator-facing flow.
