...
| `choose_protocol(*, install_prefix=None) -> str` | Resolves the protocol *name* per the 3-step chain below, no instantiation ([`#L316`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L316)). **V5-7 task 3 (shipped):** `vault_root` parameter removed; implicit vault inference from a config-supplied `vault_path` removed. Vault is now selected only via explicit `storage.backend` config (step 1) or `$MEMORY_VAULT_PATH` env var (step 2). `vault_path()` reads `plugins.obsidian-vault.vault_path` first, then the legacy flat key (V5-7 task 1, shipped). |
