...
The storage seam is the contract between the memory engine and whatever holds its files ([`scripts/storage_seam.py`](https://github.com/alexherrero/agentm/blob/main/scripts/storage_seam.py)). The engine never touches the filesystem directly — it calls seven small verbs, and a backend answers them. Every verb speaks the seam's own `Locator` type, so no filesystem assumption ever reaches the engine. Two backends implement the contract today: the device-local store that ships as the fresh-install default, and the synced vault, wrapped behind the same verbs. A shared conformance suite holds both to one objective contract. This page is the lookup for the verbs, the value types, the registry, and both backends; for *why* the seam is shaped this way, see [Memory↔storage seam](Storage-Seam-Concepts).

> [!NOTE]
> **All five parts of V5-1 are shipped.** Part 1 laid down the abstract contract: `StorageBackend`, the `Locator`/`Info`/`Capabilities` types, the `conflict_strategy` slot, the [`BackendRegistry`](#the-backendregistry), and the [three-tier taxonomy](#the-tier-taxonomy). Part 2 added the [`DeviceLocalBackend`](#the-devicelocalbackend), the fresh-install default. Part 3 added the backend-agnostic conformance suite, which rides the existing cross-OS unit-test step rather than a new gate. Part 4 added the [`VaultBackend`](#the-vaultbackend), wrapping the synced vault write path behind the same seam. Part 5 added backend selection: the [`storage.backend` config key](#the-storagebackend-config-key) and [resolver](#the-selection-resolver), the [`Capabilities`](#the-capabilities-type)-read (capability-request matching itself shipped later, in V5-7, adding `select_backend(required=...)` and [`CapabilityMismatchError`](#capabilitymismatcherror)), the [fail-loud guard](#the-fail-loud-guard) that never silently demotes to `device-local`, and the read-only [`doctor` storage preview](#the-doctor-storage-check). Part 5 did not include the engine cutover — that landed later, in V5-3 (v5.5.0); see the [Memory-storage seam design § 3](memory-storage-seam) for what it changed. No index or abstract-promotion ships yet: `DerivedMaintenance` stays an abstract class with `reindex`/`changed_since` reserved as names only, and the real derived index lands in V6. Anything this page marks *(V6)* is a forward reference, not current behavior.
...
`Locator` is the seam's own handle — it names a place in a backend's namespace and does no I/O. It is not a `pathlib.Path`: it carries a normalized, backend-relative key and exposes only namespace operations. It is frozen and hashable, so a backend may use it as a dict key (the in-memory fixture does).
...
`Info` describes a locator — it is the frozen `dataclass` the `info` verb returns.
...
`Capabilities` describes what a backend can promise — a frozen `dataclass` each backend declares. It carries four booleans, all defaulting to the conservative floor (`False`). The contract (part 1) only *defines* them and both backends already declare their values — device-local (kernel-built-in) all-`False`, vault (the crickets `obsidian-vault` plugin — see [The `VaultBackend`](#the-vaultbackend)) `concurrent_writers`/`sync`/`conflict_files` `True`. Part 5 adds the **read side** (task 2, shipped): the [resolver](#the-selection-resolver)'s `select_backend` ([`backend_selection.py#L340`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L340)) returns the selected backend *instance*, whose `.capabilities` property the kernel reads directly to learn what it can promise — proven by `TestCapabilitiesRead` ([`scripts/test_backend_selection.py#L725`](https://github.com/alexherrero/agentm/blob/main/scripts/test_backend_selection.py#L725)), which reads all four booleans off both backends through the selection surface (the vault case via a synthetic plugin fixture) and pins the four-key shape as a superset (additive growth of new keys is allowed; a rename/removal breaks the pin). This is read-and-surface only — capability-request *matching* (a consumer declaring what it *needs* and selection honoring it) is **V5-7**. **V5-7 Part 1 (shipped):** `select_backend` now accepts an optional `required: Capabilities | None = None` keyword parameter; when provided, it runs a subset check after resolving the backend and raises [`CapabilityMismatchError`](#capabilitymismatcherror) (a `StorageSelectionError` subclass) if any `True` requirement is unmet. `required=None` (default) preserves prior behavior. A dataclass so the set can grow.
...
`StorageBackend` is the abstract interface a backend conforms to. A concrete backend registers under a protocol name (`device-local`, `vault`) via the [`BackendRegistry`](#the-backendregistry) and implements every verb. Concrete backends are out of scope for this part; the registry they will plug into ships here.
...
> [!NOTE]
> A *filesystem* backend composes the [vault write protocol](Vault-Write-Protocol) here (V5-0 `atomic_write` + content-hash CAS + `vault_mutex`), building on existing write-safety instead of reinventing it. The abstract contract only declares the shape; the composition lands with the concrete backends (parts 2 / 4), not in this ABC.
...
| `"whole-file"` | The whole file is the conflict unit — a *named* policy [part-5 selection](#the-backendregistry) reads, **not** an executor; agentm ships no automated whole-file merger, only the detect-and-notice machinery (`detect_conflict_files` + the `conflict-merger` SessionStart hook) that surfaces each conflict/base pair for operator-by-hand resolution. | The synced [`VaultBackend`](#the-vaultbackend) — now the crickets `obsidian-vault` plugin, not a kernel built-in (V5-3) — overrides the floor to this ([`obsidian-vault/scripts/storage_vault.py#L156`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py#L156), crickets repo). |
...
A process-wide default `BackendRegistry` instance ([`scripts/storage_seam.py#L393`](https://github.com/alexherrero/agentm/blob/main/scripts/storage_seam.py#L393)) the real backends register into. **Since V5-3 (v5.5.0) it holds exactly one persistent entry** — `device-local`, registered at import of [`storage_device_local`](#the-devicelocalbackend). The kernel's own `storage_vault.py` (the part-4 built-in) was deleted in [commit `d95468b`](https://github.com/alexherrero/agentm/commit/d95468b642773b2d9ac9b673f9acd695b5f2acef); the real `VaultBackend` now lives in the crickets `obsidian-vault` plugin (see [The `VaultBackend`](#the-vaultbackend)) and is **not** registered at kernel-import time at all — selection ([`_load_vault_plugin_backend`](#the-vaultbackend)) discovers the plugin's module, execs it (which self-registers `vault` as a side effect), reads off the class, and unconditionally pops the `vault` slot again in a `finally` — so the registry is back to holding only `device-local` immediately after any selection call, whether or not vault was chosen.

| Protocol | Registered class | Since |
|---|---|---|
| `device-local` | [`DeviceLocalBackend`](#the-devicelocalbackend) | part 2 — permanent, registered at kernel import |
| `vault` | [`VaultBackend`](#the-vaultbackend) (crickets `obsidian-vault` plugin) | part 4 originally; re-homed out of the kernel in V5-3 — transiently registered only for the duration of a `select_backend`/`storage_preview` call |

## Backend selection (part 5)

> [!NOTE]
> **All five backend-selection tasks are shipped.** Task 1 added the `storage.backend` config field and the [`choose_protocol`](#the-selection-resolver) / [`select_backend`](#the-selection-resolver) resolver. Task 2 added the [capabilities-read](#the-capabilities-type) — the kernel reads the selected backend's `.capabilities` through the returned instance; capability-request *matching* landed later, in V5-7, which added the `required` parameter to `select_backend` and a `--requires` flag on `doctor` (see [`The doctor storage check`](#the-doctor-storage-check)). Task 3 added the polished [fail-loud guard](#the-fail-loud-guard): a named [`StorageSelectionError`](#the-fail-loud-guard) that names the missing plugin, lists the registered alternatives, and never silently falls back to `device-local`. Task 4 added the read-only [`doctor` storage preview](#the-doctor-storage-check), rendering the byte-identical install-the-plugin message before any memory operation could refuse. The engine cutover was not part of this work — selection only resolves which backend to use and refuses a misconfigured one; the cutover that pointed the kernel at device-local shipped later, in V5-3 (v5.5.0). See the [Memory-storage seam design § 3](memory-storage-seam) for what it changed.

Part 5 adds the read side of the seam: a config key naming the chosen backend, and a resolver that turns that name into a registered backend instance via [`registry.get`](#the-backendregistry). The resolver module is intentionally not named `storage_*.py` and uses no seam-verb name (`choose_protocol` / `select_backend`, never `resolve`/`read`/`write`/…), so the [`check-storage-seam-no-path-leak`](CI-Gates) gate (scoped to `scripts/storage_*.py`) does not scan it even though it handles `Path`.
...
| `choose_protocol(*, install_prefix=None) -> str` | Resolves the protocol *name* per the 3-step chain below, no instantiation ([`#L316`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L316)). **V5-7 task 3 (shipped):** `vault_root` parameter removed; implicit vault inference from a config-supplied `vault_path` removed. Vault is now selected only via explicit `storage.backend` config (step 1) or `$MEMORY_ROOT` env var (step 2). `vault_path()` reads `plugins.obsidian-vault.vault_path` first, then the legacy flat key (V5-7 task 1, shipped). |
...
| How it instantiates | Two different paths, since V5-3 deleted the kernel vault built-in. For `device-local` and any explicitly-configured third-party backend: `storage_seam.registry.get(<protocol>)` → the registered class → instantiate ([`#L374`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L374)) — `device-local` with its optional root, a third party with the V5-1 minimal no-arg constructor. For `vault`: `registry.get` is bypassed entirely — `_load_vault_plugin_backend` ([`#L205`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L205)) discovers + execs the crickets `obsidian-vault` plugin's `storage_vault.py` and hands back its `VaultBackend` class, which is then constructed directly with the vault root resolved internally from `plugins.obsidian-vault.vault_path` (`plugin_cls(vault_root, lock_root=…)`, [`#L409`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L409)). See [The `VaultBackend`](#the-vaultbackend) for the discovery mechanism. |
...
As of **part-5 task 3** (and the silent-demotion hardening that followed), `select_backend` raises a dedicated [`StorageSelectionError`](#storageselectionerror) (no longer the task-1 placeholder `RuntimeError`) on any of **five** refusals — a fifth was added in V5-3 when the vault built-in was re-homed to a plugin. Three are raised by `select_backend` itself once a protocol is chosen; the other two are raised earlier by `_configured_backend` ([`backend_selection.py#L262`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L262)) — the reader of the explicit `storage.backend` value — which refuses rather than let a corrupt config silently drop the selection and demote to `device-local`:
...
| `vault` plugin absent or unloadable | `select_backend` — `vault` is selected and a `vault_path` resolves, but the crickets `obsidian-vault` plugin can't be discovered, or its `storage_vault.py` exists but doesn't export a usable `VaultBackend` class (`_load_vault_plugin_backend` returns `None` or raises) | `storage backend 'vault' is selected but the obsidian-vault plugin is not installed. Install it (crickets: \`claude plugin install obsidian-vault@crickets\`), or point $OBSIDIAN_VAULT_SCRIPTS at its scripts dir, or change storage.backend — refusing to demote to device-local, which could mis-write or orphan the vault.` — built by the `_install_obsidian_vault_message` helper, [`backend_selection.py#L140`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L140). |
...
> [!WARNING]
> **A module-identity gap could silently defeat this guard under direct-script invocation (R1.3 / agentmEngine#1, fixed).** `resolve_project()`'s `except (StorageBackendNotInstalledError, _bs.StorageSelectionError): raise` clause (the re-raise this guard depends on) is an `isinstance` check, and Python class identity is per *module object*, not per source file. When `harness_memory.py` is run directly as a script (`python3 harness_memory.py append-progress ...` — the standard CLI pattern used throughout this repo; `write-state` was the verb at the time this gap was found and is retired under agentm-vault plan 15), Python registers the running module as `__main__`, not `harness_memory`. `backend_selection.py`'s top-level `import harness_memory` then finds no `harness_memory` entry in `sys.modules` and imports a *second*, separate instance of the same file — whose `StorageBackendNotInstalledError` class is a different object than `__main__`'s class of the same name. The `except` clause's `isinstance` check no longer matches, the exception falls through to the broader `except Exception: backend = None`, and the guard silently demotes to `device-local` — reproducing the exact failure this section forbids, but only on the raising branch, only under direct-script invocation. The fix: the `if __name__ == "__main__":` block now runs `sys.modules.setdefault("harness_memory", sys.modules["__main__"])` before dispatch, so any later `import harness_memory` reuses the already-running instance instead of re-executing the file. Caught by [`verify-state-routing`](CI-Gates#what-each-gate-proves)'s never-demote check (case D) — the first fixture to exercise this raising branch under CLI invocation.

> [!NOTE]
> **Proven by a negative test.** `test_no_silent_device_local_fallback` (in the `TestFailLoud` class, [`scripts/test_backend_selection.py`](https://github.com/alexherrero/agentm/blob/main/scripts/test_backend_selection.py)) asserts the invariant that matters: on an unregistered backend the resolver **raises**, does **not** return a `DeviceLocalBackend`, and **never even constructs one** — the device-local root stays uncreated. The class also pins the unregistered-backend raise, that the install message lists the registered alternatives, and the `vault`-without-`vault_path` companion guard. The guard's *full* bite arrives once V5-3 deletes the built-in `vault` backend (until then `vault` is always registered); the refusal machinery itself is shipped now.
>
> The `_install_plugin_message` helper is deliberately factored out so the [`doctor` preview](#the-doctor-storage-check) (task 4, shipped — reuses it at [`backend_selection.py#L539`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L539)) renders a byte-identical message before any memory operation could refuse.

### The `doctor` storage check

`doctor` (the install-health skill) gains a **storage-backend preview** (task 4, shipped) — the operator's read-only way to see what selection *would* resolve before any memory operation could refuse. Invocable as `python3 scripts/backend_selection.py --doctor` (the `_doctor_main` CLI entry, [`backend_selection.py#L603`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L603)) and read by the `doctor` skill's structural check `4d`. It reports the selected backend, where the selection came from, whether its plugin is registered, and that the `device-local` root is writable — and it resolves all of this **without constructing a backend** (`storage_preview`, [`backend_selection.py#L471`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L471)), because construction would `mkdir` the device-local root.
...
| `warn` / `0` | `device-local` selected but its root is not writable (`_root_is_writable`, [`#L456`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L456)). | Preventive, never build-blocking. |
| `fail` / `1` | Unregistered plugin; **or** `vault` selected with no resolvable `vault_path`; **or** `vault` selected but the obsidian-vault plugin is absent/unloadable; **or** the config file exists but is unparseable / names a non-string `storage.backend`. When `--requires` is passed: any unsatisfied capability or unknown capability name. | The unregistered-plugin line reuses the **same** `_install_plugin_message` ([`#L539`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L539)) the [fail-loud guard](#the-fail-loud-guard) raises ([`#L383`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L383)), so the preview and the live refusal are **byte-identical**; the vault-plugin-absent line reuses `_install_obsidian_vault_message` the same way. |
...
A `RuntimeError` subclass ([`scripts/backend_selection.py`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py)) raised by the [selection resolver](#the-selection-resolver) when the configured backend **cannot be produced** — the loud half of the [registry's absence-vs-fail-loud split](#the-backendregistry). It lives on the resolver (above the seam, in `backend_selection.__all__`), not in `storage_seam`, because turning a registry *miss* into a *fatal* refusal is the resolver's decision, not the registry's. Raised in **five** cases (see the [fail-loud guard](#the-fail-loud-guard)) across two functions. `select_backend` raises it once a protocol is chosen: when `storage.backend` names a backend whose [`registry.get()`](#the-backendregistry) returns `None` (the plugin isn't installed), when `vault` is selected with no `vault_path` to seed it, or when `vault` is selected but the crickets `obsidian-vault` plugin is absent or unloadable (V5-3 added this third case when the vault built-in was re-homed to a plugin). `_configured_backend` — the reader of the explicit `storage.backend` value — raises it earlier: when the config file is present but unparseable (so a corrupt config cannot silently drop the selection and demote to `device-local`), or when `storage.backend` is present but not a non-empty string. Catching this loudly — instead of a silent `device-local` fall-back — is how the [`doctor` preview](#the-doctor-storage-check) (task 4, shipped) surfaces the install-the-plugin message before any memory operation can refuse (the V5-3 engine cutover, [Memory-storage seam design](memory-storage-seam), then pointed the kernel at device-local exclusively). The preview does not *raise* the error — it converts each `StorageSelectionError` case into a `"fail"` row carrying the same message ([`storage_preview`](#the-doctor-storage-check), [`backend_selection.py#L471`](https://github.com/alexherrero/agentm/blob/main/scripts/backend_selection.py#L471)).
...
> [!NOTE]
> **Engine cutover landed in V5-3 (v5.5.0).** `DeviceLocalBackend` works as a storage backend, and V5-3 completed the cutover the seam design deferred. The [Memory-storage seam design § 3](memory-storage-seam) is the canonical account of what it changed.

| Aspect | Detail |
|---|---|
| Protocol name | `device-local` (the module's `PROTOCOL` constant, [`storage_device_local.py#L54`](https://github.com/alexherrero/agentm/blob/main/scripts/storage_device_local.py#L54)). Registered at import — the class, not an instance. |
...
| `read(locator)` | `read_bytes().decode("utf-8")` — **byte-exact**, no newline translation, so content round-trips with the atomic writer. Missing path raises `FileNotFoundError` natively ([`#L130`](https://github.com/alexherrero/agentm/blob/main/scripts/storage_device_local.py#L130)). |
| `write(locator, content)` | Composes [`vault_lock.atomic_write`](Vault-Write-Protocol) (temp + fsync + rename) — **never** an open-and-truncate, so a crash leaves prior bytes intact. Parent dirs created if absent ([`#L136`](https://github.com/alexherrero/agentm/blob/main/scripts/storage_device_local.py#L136)). |
...
> [!WARNING]
> **Not a kernel module since V5-3.** The kernel's own `scripts/storage_vault.py` (and its test file) were **deleted for real** in [commit `d95468b`](https://github.com/alexherrero/agentm/commit/d95468b642773b2d9ac9b673f9acd695b5f2acef) (2026-06-21, "AG Phase 2, A2.3 part 1") — the V5-3 engine cutover had already stopped the kernel from *invoking* it directly, and this commit removed the file itself, closing out the scaffolding the earlier cutover left behind. The second **concrete** `StorageBackend` now lives entirely in the crickets `obsidian-vault` plugin, at [`src/obsidian-vault/scripts/storage_vault.py`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py) — today's Obsidian/GDrive vault, reached *through the seam*. Everything below describes that plugin file, not kernel source.

The plugin's `VaultBackend` implements the [seven verbs](#the-storagebackend-abc) against the resolved per-project vault root. Unlike `device-local` (registered permanently at kernel import), it is **discovered and loaded on demand**: `backend_selection._load_vault_plugin_backend` (see [How it instantiates](#the-selection-resolver)) locates the plugin's `scripts/` dir by convention, `exec`s `storage_vault.py` (which self-registers `vault` into the seam's default [`registry`](#registry-module-default) as an import side effect, same as the kernel built-in used to), reads off the `VaultBackend` class, and then **unconditionally pops the `vault` registry entry again** in a `finally` — so nothing about the plugin lingers in kernel process state between selection calls. It is the synced, multi-writer counterweight to the device-local floor: where device-local declares the all-`False` floor, the vault backend declares the positive profile and overrides the conflict floor. For *why* the vault sits behind a seam at all, see [Memory↔storage seam](Storage-Seam-Concepts#what-the-seam-is-for).

> [!NOTE]
> **Wrap, not rewrite, and now a move, not a wrap (V5-2 → V5-3).** V5-2 lifted the vault backend byte-faithfully out of the kernel into this plugin while both ran in parallel, proven identical by conformance + a byte-identical parallel-run on the operator's real vault. That green authorized **V5-3** to delete the kernel built-in for good, leaving the plugin copy as the sole vault backend — a **move**, not a fork: the plugin's docstring itself warns against letting it "absorb new vault behavior the built-in lacks while both are live." For the full cutover account, see the [Memory-storage seam design § 3](memory-storage-seam).

| Aspect | Detail |
|---|---|
| Protocol name | `vault` (the module's `PROTOCOL` constant, [`obsidian-vault/scripts/storage_vault.py#L98`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py#L98), crickets repo). Registered only transiently, for the duration of a selection call — see above. |
| Root | The resolved per-project vault path — a **required** ctor arg (the vault has no universal default, unlike device-local's `~/.agentm/memory/`). Created on construction (idempotent), so the root locator always resolves to a real directory ([`storage_vault.py#L113-125`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py#L113-L125), crickets repo). |
...
| `capabilities` | The synced, multi-writer profile — the positive contrast to device-local's floor: `concurrent_writers=True` (the mutex makes N writers safe), `conflict_files=True` (DriveFS surfaces "(conflicted copy)" siblings the engine must tolerate), `sync=True` (GDrive replicates the tree), `encryption=False` (no encryption at rest) ([`storage_vault.py#L141-153`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py#L141-L153), crickets repo). |
| `conflict_strategy` | Overrides the seam's `"none"` floor to `"whole-file"` ([`storage_vault.py#L155-169`](https://github.com/alexherrero/crickets/blob/main/src/obsidian-vault/scripts/storage_vault.py#L155-L169), crickets repo) — see the [strategy note below](#a-named-policy-not-an-auto-merger). |
...
### Conformance — now split kernel-side / plugin-side

V5-3's deletion of the kernel built-in moved vault conformance out of this repo. **Kernel-side**, `scripts/test_storage_conformance.py` no longer runs a `VaultConformance` case at all (that class — along with `VaultRunConformanceReport` — was deleted in the same commit that removed `storage_vault.py`); only `DeviceLocalConformance` runs the full universal battery here now. A `_make_scratch_vault_backend` helper still exists in that file, but only to back `RoutingConformanceReport`'s check that the V5-6 routing layer (`resolve_project` / `repo_registry`, below) works against *some* `VaultBackend`-shaped object — it constructs [`vault_backend_stub.VaultBackend`](https://github.com/alexherrero/agentm/blob/main/scripts/vault_backend_stub.py) (a standalone, same-contract stand-in for tests; it does **not** self-register into the seam registry), never the real plugin.

**Plugin-side**, the real vault conformance now runs in the crickets repo, against a checked-out agentm kernel (this is the `obsidian-vault-conformance` CI job — see [CI gates](CI-Gates)). `scripts/test_obsidian_vault_conformance.py` there carries `PluginVaultConformance` ([`#L188`](https://github.com/alexherrero/crickets/blob/main/scripts/test_obsidian_vault_conformance.py#L188)) — the same universal battery, over a scratch vault — and `PluginVaultRunConformanceReport` ([`#L207`](https://github.com/alexherrero/crickets/blob/main/scripts/test_obsidian_vault_conformance.py#L207)) for the per-backend pass report.

<a name="the-never-orphan-invariant"></a>**The never-orphan invariant itself is retired** — `test_storage_vault.py` (and the `VaultNeverOrphanInvariant` case it carried, which proved an old-way write and a seam write reached the same on-disk bytes) was deleted along with the kernel module; there is no longer an "old way" in this repo to compare against, since the seam *is* the only way now. What replaces it crickets-side is narrower but still load-bearing: `PluginVaultBehavioralContract` ([`#L237`](https://github.com/alexherrero/crickets/blob/main/scripts/test_obsidian_vault_conformance.py#L237)) closes the gap the byte-only conformance suite can't reach — it forces a foreign write into the CAS window (interposing on `Path.read_bytes`) and requires `write` to raise `ConcurrentModificationError` rather than silently clobber, and it pins `capabilities`/`conflict_strategy` to their literal vault values so a backend degraded toward device-local's floor can't pass either check.

### Module exports (`storage_vault`, now crickets-side)

The kernel ships no `storage_vault` module anymore. The crickets plugin file's `__all__` is `VaultBackend`, `PROTOCOL` — same shape the kernel built-in had. Importing it (which only ever happens inside `_load_vault_plugin_backend`'s `exec_module` call, never at kernel startup) registers `VaultBackend` under `vault` in the seam's default [`registry`](#registry-module-default), immediately popped again by the loader's `finally`. Like the other backend modules it ships no `python -m` entrypoint — it is consumed in-process. Kernel code that needs a same-contract `VaultBackend` for tests (not real plugin discovery) imports [`vault_backend_stub`](https://github.com/alexherrero/agentm/blob/main/scripts/vault_backend_stub.py) instead — `PROTOCOL`, `VaultBackend`; deliberately does **not** self-register, since a test double shouldn't compete with the real plugin for the registry slot.
...
> [!NOTE]
> **Status: complete** (V5-6, 2026-06-18). All five tasks shipped: `resolve_project` / `_vault_projects_dir` (task 1), `repo_registry` (task 2), `state_mode` vault→backend alias (task 3), gate extensions + conformance suite (task 4), ADR + docs (task 5). Decision: [Memory-storage seam](memory-storage-seam).
...
|---|---|---|---|
| `resolve_project` / `_vault_projects_dir` | `harness_memory.py:395,417` | **Shipped (task 1)** | `_vault_projects_dir(backend: StorageBackend) -> Locator`; `resolve_project` returns `{slug, project_locator, backend, project_root, layout}` — `project_locator` is a `Locator`, not a `Path`. `process_seam.py:113` uses `project_locator.key`. |
| `repo_registry` read/write | `repo_registry.py` | **Shipped (task 2); registry home moved (agentm-vault plan 05)** | `registry_path(vault_path) -> Path` replaced by `registry_locator(backend: StorageBackend) -> Locator` ([`#L116`](https://github.com/alexherrero/agentm/blob/main/scripts/repo_registry.py#L116)). All five public functions take `backend: StorageBackend`. `_mutate_registry` no longer holds `vault_mutex` — `VaultBackend.write()` handles it; CAS retry loop handles cross-device races. On `obsidian-vault` resolved to identical path at task-2 ship (LC-1, proven by LC-7 parallel-run test at [`test_harness_memory.py#L2608`](https://github.com/alexherrero/agentm/blob/main/scripts/test_harness_memory.py#L2608)) — the memory-root trims (plan 05) since moved the registry's home to the engine state directory (`registry_store()` picks the engine store whenever it holds the file, and the `obsidian-vault` backend's `_meta/repos.json` only while it is the sole copy), so the two backends no longer resolve to the same bytes at the same path. The one case that still reaches a backend, a legacy copy it holds alone, is what the routing conformance check below proves. |
| `state_mode` resolver | `harness_memory.py:296,669` | **Shipped (task 3)** | `"vault"` aliases to `"backend"` at read time in both `_read_config_state_mode` and `_read_project_mode`; `agentm_config` normalizes `"vault"` → `"backend"` at write time; CLI `--state-mode` help surfaces the deprecation note — no operator action required. |
...
- `check-process-seam-import-direction.sh`'s LC-8 back-edge scan — scans the same two files for any `import storage_vault` / `from storage_vault import` and fails loud (a routing mechanism may import the seam, never a capability plugin) — now lives as the `lc8-storage-vault` rule inside `scripts/check-one-way-imports.py` (CONS-1 merged three separate one-way-import scripts — this shell check plus `check-opinion-resolver-one-way.py` and `check-capability-resolver-one-way.py` — into one config-driven AST checker; `check-all.sh` runs it as `check-one-way-imports`). The rule's own name for this check remains `lc8-storage-vault`, so the concept it enforces is unchanged even though the old standalone script no longer exists.
- `storage_conformance.py` extended: `check_routing_repo_registry(make_backend)` proves that a registry copy held by a backend is read and written through that backend. Since the memory-root trims (agentm-vault plan 05) that legacy case is all of LC-4 left to prove. The check seeds `_meta/repos.json` on a fresh backend, inside an empty engine state directory of its own, and fails unless register, list and unregister read and write that copy and leave the engine state without a registry. Gated by `run_conformance(include_routing=False)` (default off, explicit opt-in); `RoutingConformanceReport` in `test_storage_conformance.py` runs `include_routing=True` on both backends, and `test_storage_conformance_negative.py` proves the check fails when the registry never reaches the backend.

For the intent and three-leg arc context, see the [Memory-storage seam design](memory-storage-seam).
