...
> [!NOTE]
> **Engine cutover landed in V5-3 (v5.5.0).** `VaultBackend` implements the seam contract and is still registered under `vault` for selection, but the kernel no longer invokes it directly — the vault backend is a plugin concern after V5-3. The never-orphan invariant proves the wrap still reaches the same on-disk bytes the old path did — the wrap is conserved, not removed. For what the cutover changed, see the [Memory-storage seam design § 3](memory-storage-seam), the canonical account.
