...
| `repo_registry` read/write | `repo_registry.py` | **Shipped (task 2); registry home moved (agentm-vault plan 05)** | `registry_path(vault_path) -> Path` replaced by `registry_locator(backend: StorageBackend) -> Locator` ([`#L77`](https://github.com/alexherrero/agentm/blob/main/scripts/repo_registry.py#L77)). All five public functions take `backend: StorageBackend`. `_mutate_registry` no longer holds `vault_mutex` — `VaultBackend.write()` handles it; CAS retry loop handles cross-device races. On `obsidian-vault` resolved to identical path at task-2 ship (LC-1, proven by LC-7 parallel-run test at [`test_harness_memory.py#L2608`](https://github.com/alexherrero/agentm/blob/main/scripts/test_harness_memory.py#L2608)) — the memory-root trims (plan 05) since moved the registry's home to the engine state directory (`registry_store()` picks the engine store whenever it holds the file, and the `obsidian-vault` backend's `_meta/repos.json` only while it is the sole copy), so the two backends no longer resolve to the same bytes at the same path; that later divergence is this seam's, not LC-1's, to re-prove. |
