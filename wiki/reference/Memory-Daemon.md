...
> [!NOTE]
> **Step 3 shipped, steps 4 and 5 pending** — `tasks/176-converge-the-vault-layout`. The move ran live on 2026-09-25 (vault commit `5e591f5371`): 25 files left the project roots — living documents to `projects/<slug>/docs/` (`followups.md`, `roadmap.md`, `conventions.md` in six projects, plus `version-summaries.md`, blog's six design/strategy/voice docs, dev-setup's `cloud-test-runbook.md`, and two `how-to-*.md` files), config and source lists to `projects/<slug>/desk/` (agentm's four lists; home's `home_config.py` and `test_model.py`) — with 100 path links rewritten across 57 notes and 0 new unresolved links. `door.Judge` does **not** enforce the lock yet: a project root can still take as many documents as it needs, so nothing stops a new file landing loose at the root again. The five-file lock itself is step 4.

The plan locks every project's root to exactly five files: `project.yaml`, `charter.md`, `blueprint.md`, `tracker.md`, and `moc-<slug>.md` (see [Vault project.yaml reference](Vault-Project-Yaml) for the new file's schema). Once step 4 lands, creating anything else directly under `projects/<slug>/` will answer Alignment — a session has to ask — while an edit to one of the five keeps today's Alignment, and subfolders (`docs/`, `desk/`, `decisions/`, `designs/`, `research/`) keep today's rules. Step 3 already moved the living documents that used to sit at a project root (`followups.md`, `roadmap.md`, and others) to `docs/`, and the config and source lists to `desk/`; `project.yaml`, `blueprint.md`, and (per the plan) a regenerated `tracker.md` are the remaining pieces steps 5–7 add, ahead of step 4 closing the door on the rest.
