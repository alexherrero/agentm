...
> [!NOTE]
> **Steps 2, 8 and 9 shipped** — `tasks/176-converge-the-vault-layout`. The door, the embed scope, the storage contract, and the maps job all know both spaces. Step 8 moved the first files into `resources/` live on 2026-09-25 (vault commit `7ac40ea5cd`): 233 reference cards to `topics/<topic>/` (file-watching, go-git, go-static-binaries, llama-cpp-embeddings, sqlite) and 101 watchlist items to `watchlist/<source>/`, plus a new `university/index.md`. `resources/` is a real directory now, with something for the door, the embed scope, and the dampening to act on. Step 9 filled `systems/` the same day (vault commit `6af892a888`): the six homelab notes moved from `memory/semantic/` to `homelab/components/` under their own names and became `kind: component` records, `homelab/system.md` is a new overview drawn from `homelab-domain` and `home-server`, and `agentm/system.md` and `crickets/system.md` are front doors to each repo's wiki.

The plan adds two shared spaces:

| Space | Holds | Dampened? |
|---|---|---|
| `resources/` | `topics/<topic>/` — 233 reference cards, moved from a project's `research/<topic>/reference/` on 2026-09-25 (step 8), dropping the `reference/` level. `watchlist/<source>/` — 101 items, moved from `projects/agentm/_watchlist/` the same day; the first forward-learning run afterward added 2 more. `university/` — opens with `index.md`, study guides after. | The watchlist — `resources/watchlist` joins `dampened_spaces` (`daemon/internal/rules/storage-rules.default.md:378`), narrowed on 2026-09-25 from all of `resources`; the dampening runs at the daemon's fixed strength (see [Space, altitude, and the two that are not penalties](#space-altitude-and-the-two-that-are-not-penalties) above) whatever the question, and with all of `resources` dampened, seven of the ten gold questions about a reference card lost it from the top five. The topic cards now rank as any note does: `agentmd search "fts5 columnsize"` returns `resources/topics/sqlite/columnsize-detail-config-options.md` first. |
| `systems/` | `homelab/` — a `system.md` overview written on 2026-09-25, plus `components/*.md`: the six homelab notes moved from `memory/semantic/` under their own names. A one-page front door per software system (`agentm/system.md`, `crickets/system.md`). | No |
...
`PlanSpaceMaps` skips a space outright when it isn't yet a directory — `isDirExact`, called from `spacemaps.go:170`. Step 8 gave `resources/` real files, so it no longer skips: the next nightly run has a `resources/moc-resources.md` to write. Step 9 did the same for `systems/`, so the next nightly run writes `systems/moc-systems.md` as well; `systems/` carries no dampening rule.

### The project root locks to five files

> [!NOTE]
> **Steps 3 through 6 shipped, step 7 pending** — `tasks/176-converge-the-vault-layout`. Step 3 moved the files off the project roots live on 2026-09-25 (vault commit `5e591f5371`): 25 files left the roots — living documents to `projects/<slug>/docs/` (`followups.md`, `roadmap.md`, `conventions.md` in six projects, plus `version-summaries.md`, blog's six design/strategy/voice docs, dev-setup's `cloud-test-runbook.md`, and two `how-to-*.md` files), config and source lists to `projects/<slug>/desk/` (agentm's four lists; home's `home_config.py` and `test_model.py`) — with 100 path links rewritten across 57 notes and 0 new unresolved links. Step 4 then closed the door. Step 5 gave every project a `project.yaml`. Step 6 gave the mocs job a generated `tracker.md` for every project — `PlanProjectTrackers` (`daemon/internal/dreaming/projecttrackers.go:199`, wired into the nightly run right after the project maps at `daemon/internal/dreaming/run.go:275-282`) renders it from the project's own task trackers, so no session writes one any more; see [the mocs job](#its-jobs-in-order) below for the shape. The first nightly pass over the shipped code has not run yet, so the file itself doesn't exist in the vault as of this writing. `blueprint.md` (step 7) is still to come.

The door locks every project's root to exactly five files: `project.yaml`, `charter.md`, `blueprint.md`, `tracker.md`, and `moc-<slug>.md` (see [Vault project.yaml reference](Vault-Project-Yaml) for the new file's schema). `door.RootFiles` names the first four (`daemon/internal/door/door.go:98`); `door.IsRootFile` adds the fifth, built from the project's own slug (`daemon/internal/door/door.go:102-112`). `Roots.Judge` reads both at the root: creating one of the five files is Standing — a project missing its `blueprint.md` is missing a page, and writing it is maintenance (`daemon/internal/door/door.go:184-189`) — creating any other new file at the root answers Alignment (`daemon/internal/door/door.go:191-196`), and changing an existing root document is Alignment as it always was (`daemon/internal/door/door.go:178-183`). Subfolders (`docs/`, `desk/`, `decisions/`, `designs/`, `research/`) are unchanged: everything below the root stays Standing. This reverses the door's earlier rule that a project could have as many root documents as it needed, with no cap.

Step 3 already moved the living documents that used to sit at a project root (`followups.md`, `roadmap.md`, and others) to `docs/`, and the config and source lists to `desk/`. Step 5 gave all twelve projects a `project.yaml` on 2026-09-25, held to its schema by `check-project-yaml`. Step 6 wired a generated `tracker.md` into the mocs job (see the callout above and [the mocs job](#its-jobs-in-order) below); `blueprint.md` (step 7) is still to come.
...
Never a tracker, a plan or a progress log — a task's are the session's, and a nightly writer would race the session that owns them; a project's own `tracker.md` is generated by the mocs job, which a second writer would race.
