...
> [!NOTE]
> **Steps 3, 4 and 5 shipped, steps 6 and 7 pending** — `tasks/176-converge-the-vault-layout`. Step 3 moved the files off the project roots live on 2026-09-25 (vault commit `5e591f5371`): 25 files left the roots — living documents to `projects/<slug>/docs/` (`followups.md`, `roadmap.md`, `conventions.md` in six projects, plus `version-summaries.md`, blog's six design/strategy/voice docs, dev-setup's `cloud-test-runbook.md`, and two `how-to-*.md` files), config and source lists to `projects/<slug>/desk/` (agentm's four lists; home's `home_config.py` and `test_model.py`) — with 100 path links rewritten across 57 notes and 0 new unresolved links. Step 4 then closed the door: `project.yaml`, `blueprint.md`, and a regenerated `tracker.md` are step 5's job and don't exist in the vault yet.
