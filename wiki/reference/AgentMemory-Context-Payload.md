...
| Your read/write posture | Surface-scoped (DC-2): chat surfaces read + query freely, never write — capture = suggest a paste-ready entry and name its home (`standards/` for a global rule, `Projects/<slug>/` for project context). Filesystem agents (Claude Code, Antigravity) may write directly. GDrive sync means you see last-synced state (flag, don't guess). |
