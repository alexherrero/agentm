...
| Folder map — what's where | `personal-private/_always-load/` (global conventions, read first), `projects/<slug>/` (`_index.md` / `decisions/` / `open-questions/` / `_harness/`), `external/<slug>/` (third-party projects under review/mentoring — same shape, kept out of `projects/`), `_idea-incubator/<slug>/`, `_inbox/`, `_meta/`. |
