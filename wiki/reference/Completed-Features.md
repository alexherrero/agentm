...
|---|---|---|---|
| 2026-07-09 | v5.14.0 — Autonomy observability + control plane; AG Wave E close-out; AA5 consolidation | **Autonomy:** device-local telemetry ledger + runner aggregator + static console + digest ladder into the vault + morning report + window-park, then a control plane on top (dispatch tier, board+handoff wiring, launch-time grade statement) closed out by a real N1 acceptance run. **AG Wave E:** V6 retrieval engine (typed-edge graph, RRF hybrid retrieval, chunking, time-weighting, consolidation), the dreaming arc (revert-log → `/dream` → confirm/expire → scheduled job), the experience pipeline (forward-learning, crystallization), and the scheduled-surfaces runner substrate. **AA5 consolidation:** persona + opinion-registry reconciliation to as-built, the efficiency health-axis lit, silent-dark verification-honesty closure, and a vector-inclusive re-measurement of the V6 retrieval stack that reverses the previously-shipped vector-less baseline. | 27 commits (`v5.13.0..HEAD`); solo harness release, no crickets pairing evidenced in this range — see the dated section below. Version number as instructed by the release session; **no `v5.14.0` tag existed at the time this entry was written** — confirm the tag lands before treating this row as final. |
...
| 2026-06-18 | [v5.5.0](https://github.com/alexherrero/agentm/releases/tag/v5.5.0) — V5-3: storage cutover: kernel → device-local | **V5-3:** vault backend deleted from `harness_memory.py` kernel; `harness_state_dir` / `read_state_file` / `write_state_file` always device-local; `phase_recall` always `""`; `resolve_documenter_context` always `None`; `vault_path()` raises `StorageBackendNotInstalledError` when `storage.backend=vault` + no vault (`$MEMORY_VAULT_PATH` stays graceful-skip). `personal-private/` → `personal/` group rename shipped in lockstep. ([ADR 0018](memory-storage-seam)) | 7 commits (`v5.4.0..v5.5.0`); 1170 unit tests 0 failures; 20/20 check-all.sh; solo harness MINOR |
| 2026-06-17 | [v5.4.0](https://github.com/alexherrero/agentm/releases/tag/v5.4.0) — V5-9 memory MCP server | **V5-9:** `scripts/memory_mcp_server.py` (singleton streamable-HTTP daemon at `127.0.0.1:7821`) + four MCP tools (`memory_search` / `memory_recall` / `memory_append` / `memory_forget`) + bearer-token + Origin-validation security + `scripts/memory_mcp_stdio_shim.py` (Claude Desktop stdio shim) + `scripts/memory_mcp_doctor.py` (4 health checks) + `install.sh --mcp-server` + launchd plist + host configs for Claude Code / Cursor / Goose / Claude Desktop. ([ADR 0017](agentm-memory-system)) | 11 commits (`v5.3.0..v5.4.0`); contract tests + 13 doctor unit tests; solo harness MINOR |
...
| 2026-05-12 | [v2.0.0](https://github.com/alexherrero/agentm/releases/tag/v2.0.0) — `crickets` repo split: `dependabot-fixer` + `ship-release` moved out | **BREAKING**: two skills migrated to [`crickets`](https://github.com/alexherrero/crickets); new shared `lib/install/` byte-identical across repos; PII + lib-parity CI gates added | 9 commits (`v1.0.0..v2.0.0`); new [ADR 0006](agentm-foundations-hld); paired with [crickets v0.5.0](https://github.com/alexherrero/crickets/releases/tag/v0.5.0); harness shared-skills narrow 4→2; cross-platform byte-identity work surfaced + fixed four real Mac/Windows bugs |
| 2026-05-11 | [v1.0.0](https://github.com/alexherrero/agentm/releases/tag/v1.0.0) — Three-adapter scope; Codex dropped; 1.0.0 commitment | **BREAKING**: Codex adapter removed; true-sync `--update` semantics; firm-semver 1.0.0 floor | 13 commits (`v0.9.0..v1.0.0`); new [ADR 0005](agentm-foundations-hld); ~1300 lines net removed; first major version; parity invariant simplified to three adapters |
| 2026-04-23 | [v0.9.0](https://github.com/alexherrero/agentm/releases/tag/v0.9.0) — Diátaxis documentation spec + `/doctor` skill | Diátaxis rollout (ADR 0004, 7-task plan); `migrate-to-diataxis` skill; mode-aware `documenter` writes; `/doctor` skill for post-install verification | 10 commits (`v0.8.7..v0.9.0`); new [ADR 0004](agentm-foundations-hld); two new shared skills; `scripts/check-wiki.py` shipped + flipped to `--strict`; wiki dogfood reshaped with `git mv` for blame |
| 2026-04-21 | [v0.8.7](https://github.com/alexherrero/agentm/releases/tag/v0.8.7) — GitHub Projects wiring + documenter end-to-end dogfood | `feat-gh-projects-integration` (pending — gated on offer-cycle observation); `feat-documenter-subagent` (this sweep is the dogfood) | 4 commits (`801dbd7..HEAD`), 23 files; new [ADR 0003](agentm-foundations-hld), new [Feature page](GitHub-Projects-Integration) |

## 2026-07-09 — v5.14.0: Autonomy observability + control plane; AG Wave E close-out; AA5 consolidation

**Commit range:** `v5.13.0..HEAD` (27 commits on `main`). Solo harness release — no crickets pairing evidenced in this commit range. **Version note:** no `v5.14.0` tag existed at the time this entry was written; the number was supplied by the release session as the intended next tag. Confirm the tag actually lands before treating this as historical fact rather than a release-in-progress.

### What shipped

**The Autonomy arc's observability ledger + console** (`da23e7e`, `7963a7c`). A device-local telemetry event log (`~/.agentm/telemetry/events-YYYYMM.jsonl`) with a scheduled runner aggregator (`scripts/runner/aggregator.py`) folding it into a small SQLite rollup keyed by plan, task, model, and window. On top of the rollup: a static HTML console (`scripts/health/observability_console.py`), a digest ladder into the vault's `_inbox/` (`scripts/health/inbox_digest.py`), a window-park artifact for when the subscription rate limit runs out mid-run (`scripts/health/window_park.py`), and a morning report naming what a run did, spent, and why it ended (`scripts/health/morning_report.py`).

**The Autonomy control plane** (`8f5eb9e`, `7d0c808`, `7d96526`). Resolved the fleet-substrate decision left open by the capability audit — Agent View (`claude --bg` / `claude agents`), not Agent Teams, re-verified live against Claude Code 2.1.193. Shipped `scripts/control_plane/`: `dispatch.py` (spawns + estimates), `board_sync.py` (wires the dispatch tier into the GitHub Projects board and decides the board tracking model — repurposing the board's `Track` field for dispatch tier rather than adding a by-agent axis, since no agent-identity concept exists in the dispatch substrate to track by), `grade.py` (the launch-time authorization grade statement), `handoff.py`, and `n1_run.py`. The N1 acceptance demo — originally escalated rather than faked when a sandboxed attempt hit an auth gap — was run for real afterward by the operator: two real Agent View background sessions dispatched, `$0.4607` spend recorded, matched independently against the raw event log.

**The observability residue trio** (`ec2ebe5`). Attribution tags on ledger events, `morning_report.py --out` persistence, and digest dogfood — the follow-up cleanup after the console shipped.

**AG Wave E — the V6 retrieval engine** (`f6cf406`). A re-audit of the retrieval stack, eval fixtures, lifecycle state, a deterministic typed-edge knowledge graph (`harness/skills/memory/scripts/graph.py`), Reciprocal Rank Fusion hybrid retrieval replacing the old weighted-sum merge (`recall.py`), paragraph-aware chunking + time-weighted decay scoring, and episodic-to-semantic consolidation (`consolidate.py`).

**AG Wave E — the dreaming arc** (`a7c1c2a`). A revert-log primitive, a thin `/dream` pass, a confirm/expire cycle for staged proposals, and a scheduled job wiring it onto the runner.

**AG Wave E — the experience pipeline** (`f48a8e7`). Forward-learning's approved-source pipeline (`harness/skills/memory/scripts/forward_learning.py`) — a thin v1, opt-in, one-pass deterministic screen (not the two-pass deterministic-then-semantic shape the import watchlist uses) — plus crystallization and the accumulate-loop spec.

**AG Wave E — scheduled surfaces** (`5ad9108`). The runner substrate all of the above jobs register against — schema, due-decision cycle, dry-run-until-promoted, ownership-tier write routing, throttle-pause-stop watchdog — plus the Decide-step anti-gaming guard (`scripts/goal_contract.py`) for the goal contract's runner half.

**AA5 consolidation.** Persona and opinion-registry docs reconciled to as-built (`6c0dc6c`) — all 11 roster manifests confirmed shipped, the automatic-detection half named as the sole residual. The efficiency health-axis lit and the dark-check registry cleaned (`a53ef85`). The runner budget gate fixed to fail closed, and the health scorecard fixed to never fabricate a bare-install score (`bad7e5b`). The silent-dark family of checks closed out — verification honesty plus docs/voice health (`85e9628`). And a vector-inclusive re-measurement of the V6 retrieval stack (`4c8903b`): the earlier eval sandbox couldn't load the `sqlite-vec` extension at all (a stock-Python limitation, not a `sqlite-vec` problem — it loads fine under Homebrew Python 3.13), and re-running the pinned eval with the vector stream actually live reverses the previously-shipped vector-less headline number — recorded as a genuine reversal in [agentm-memory-system.md](agentm-memory-system)'s amendment log, not a confirmation.

### Why this shape

Three different arcs land in the same release window because they were built concurrently by AG Wave E build sessions and the parallel AA5 consolidation pass, not because they're one feature. The observability ledger exists so multi-session autonomy is legible after the fact — what a run cost and what it produced — without requiring anyone to watch it live; the control plane exists so a run can be launched and supervised unattended at all. AG Wave E is the last wave of the Architecture-Governance build closing out its retrieval, dreaming, and experience pillars. AA5 is the consolidation tail making sure what shipped is actually reconciled in the docs and that the metrics claimed are honestly re-measured — the vector re-measurement in particular is worth calling out by name: it is a real regression finding against this project's own merge gate (`accuracy_regressed=True`), surfaced rather than smoothed over.

### By the numbers

27 commits (`v5.13.0..HEAD`). No paired crickets release cited in any commit message in this range. See the individual commits for per-arc test counts; the control-plane N1 demo's real spend figures are recorded in the [Autonomy design](agentm-autonomy)'s amendment log.

### Related

- [Autonomy](agentm-autonomy) — the full observability + control-plane design, with the shipped-state reconciliation and the N1 acceptance-demo amendment.
- [Memory system](agentm-memory-system) — the V6 retrieval engine detail and the vector-inclusive re-measurement amendment.
- [Runner](agentm-runner) — the scheduled-surfaces substrate the dreaming, forward-learning, and observability jobs all register against.
- [Personas](agentm-personas) · [Opinion registry](agentm-opinion-registry) — the AA5 as-built reconciliation.
- [Persist a morning report](Persist-A-Morning-Report) — the how-to for `morning_report.py --out`.
...
- [ADR 0011 — V5 unbundling](agentm-hld) — DC-1 re-audit trigger fired; baked-in orchestration call in Developer plugin can now be retired.
...
- [ADR 0013 amendment — V5-7 config-plane](memory-storage-seam) — DC-4 updated to record all 3 V5-7 tasks and the removal of implicit vault inference.
...
**Task 5 — ADR 0019 + docs.** [ADR 0019](memory-storage-seam) created, recording the three-leg arc completion, locked design calls (LC-1/4/5/6/7/8), and load-bearing assumptions with re-audit triggers. `Decisions.md` and `decisions/_Sidebar.md` updated. `device-wide-architecture.md` v1.0 lifecycle entry updated (pending → 2026-06-18 complete). `Storage-Seam.md` routing layer NOTE updated to complete. `Single-Repo-State-Mode.md` `state_mode` value updated (`"vault"` → `"backend"` with backward-compat note).
...
- [ADR 0013 amendment — capability-request matching](memory-storage-seam) — the design rationale for subset-only matching and why alternatives were rejected.
...
**Related:** [ADR 0018](memory-storage-seam) — the five-decision record for this cutover; ADR 0013 amendment — the two V5-3 negative-consequence bullets now marked resolved; ADR 0010 amendment — `personal-private` → `personal` rename noted.
...
- [ADR 0012 — Vault-write protocol](memory-storage-seam) — V5-0; the write stack the daemon composes as writer #2.
...
- [ADR 0016 — Persona tier](persona-tier) — the design decisions; DC-1 defines the inverted-dependency rule `team-coordinator` satisfies.
...
**V5-8 — the `enhances:` runtime** ([ADR 0015](agentm-hld)). The capability-discovery resolver is the runtime half of crickets' `enhances:` soft-composition — it answers "is this capability available on this host?" without importing plugin code.
...
- [ADR 0015 — Capability discovery](agentm-hld) · [ADR 0016 — Persona tier](persona-tier)
...
The realistic trigger is mundane: a Windows editor's UTF-16/BOM "Save As" (`\xff\xfe…`) on an otherwise-valid config. The fix is the minimal contract-honoring widening — catch the decode error at the same site and route it to the behavior the docstring already promises — not a broad `except Exception`. The fail-loud path stays loud (a corrupt config still refuses startup with `StorageSelectionError`, never a silent demotion to `device-local`, per [ADR 0013](memory-storage-seam)); the graceful paths stay graceful. Surfaced by an adversarial review of the V5-1 part-5 selection code immediately after the v5.0.0 tag.

### Related

- [ADR 0013 — Storage-seam fail-loud selection](memory-storage-seam) — the fail-loud contract this fix keeps intact.
- [Memory↔storage seam](Storage-Seam-Concepts) · [Choose a storage backend](Choose-A-Storage-Backend) — the V5-1 surfaces the readers back.
...
## 2026-06-13 — v5.0.0: AgentM V5 — the memory↔storage seam + the dev-loop unbundling (MAJOR)
...
**Seven-section convergence + dev-loop slim** (the two BREAKING changes). The documentation taxonomy converged to seven sections (amending [ADR 0004](agentm-foundations-hld)). And the vendored dev loop left agentm: `migrate-to-diataxis`, the phase commands, the review sub-agents, and the vendored `documenter` / `diataxis-author` copies were **retired** — all now provided by crickets plugins (`developer-workflows`, `code-review`, `wiki-maintenance`) with graceful-skip when the plugins are absent. This is the unbundling that repositions agentm as "storage-agnostic memory OS + plugin host" rather than "vendored dev loop."

### Why this shape

V5's identity, recorded in [ADR 0011](agentm-hld), is that the memory engine is the thing only agentm provides; the dev loop is a *consumer* any host with the crickets plugins can supply. Retiring the byte-duplicated dev-loop copies removes a standing parity tax and drift risk. The seam work (V5-1) is what makes the engine *storage-agnostic*: a fresh install works with zero setup on device-local markdown, and the vault becomes one backing among several reached through an interface. The fail-loud-never-demote selection guard ([ADR 0013](memory-storage-seam)) is the load-bearing safety call of that arc — a silent demotion to device-local is exactly what would mis-write or orphan an operator's live vault, so a misconfigured backend stops startup loudly rather than degrading quietly. The vault-write protocol ([ADR 0012](memory-storage-seam)) is the concurrency floor the seam's vault backend composes; the two process/storage seams (V5-1 + V5-4) are the symmetric down-facing and up-facing boundaries of the engine.
...
The five V5 roadmap arcs (V5-0 / V5-1 / V5-4 / V5-10 + the seven-section-convergence + dev-loop-slim breaking work). Three ADRs cover the wave: [ADR 0011](agentm-hld) (unbundling, 2026-06-11), [ADR 0012](memory-storage-seam) (vault-write protocol, 2026-06-12), and [ADR 0013](memory-storage-seam) (storage-seam fail-loud selection, added in this release sweep, 2026-06-13).

### Related

- [ADR 0013 — Storage-seam fail-loud selection](memory-storage-seam) · [ADR 0012 — Vault-write protocol](memory-storage-seam) · [ADR 0011 — V5 unbundling](agentm-hld)
- [Memory↔storage seam](Storage-Seam-Concepts) · [Memory↔process seam](Memory-Process-Seam) · [Named plans](Named-Plans) — the explanation pages for the three seam/plan arcs.
...
- [agentm ADR 0004 — Diátaxis Documentation Spec](agentm-foundations-hld) — canonical Diátaxis convention this skill enforces (upstream)
...
3. **`/memory discover-skills`** — periodic fetcher over operator-editable source whitelist at `<vault>/personal/skill-discovery-sources.md`. Auto-seeds 4 sources in operator's confirmed priority order: Anthropic Cookbook → awesome-claude-code → awesome-mcp-servers → awesome-llm-apps. Per-source dated snapshot + diff cache. Cadence default 7d; `--cadence-check` self-throttles for idle-hook integration (extended `memory-reflect-idle` to call it at end of each pass).
...
- [ADR 0006 — crickets split](agentm-foundations-hld)
...
**Commit range:** `v0.9.0..v1.0.0` (13 commits on `main`). Release notes: [v1.0.0](https://github.com/alexherrero/agentm/releases/tag/v1.0.0). ADR: [0005 — Drop Codex support](agentm-foundations-hld).
...
- [ADR 0005 — Drop Codex support; three-adapter scope](agentm-foundations-hld)
...
- [ADR 0004 — Diátaxis documentation spec](agentm-foundations-hld)
...
- **ProjectsV2 ownership-and-linking decision.** The dogfood run surfaced that ProjectsV2 has no repo-owned form; `/setup` now runs a two-step `gh project create` + `gh project link --repo` flow to make the project visible under the repo. Rationale in [ADR 0003](agentm-foundations-hld).
