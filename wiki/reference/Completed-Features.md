...
|---|---|---|---|
| 2026-07-09 | [v5.14.0](https://github.com/alexherrero/agentm/releases/tag/v5.14.0) — Autonomy observability + control plane; AG Wave E close-out; AA5 consolidation | **Autonomy:** device-local telemetry ledger + runner aggregator + static console + digest ladder into the vault + morning report + window-park, then a control plane on top (dispatch tier, board+handoff wiring, launch-time grade statement) closed out by a real N1 acceptance run. **AG Wave E:** V6 retrieval engine (typed-edge graph, RRF hybrid retrieval, chunking, time-weighting, consolidation), the dreaming arc (revert-log → `/dream` → confirm/expire → scheduled job), the experience pipeline (forward-learning, crystallization), and the scheduled-surfaces runner substrate. **AA5 consolidation:** persona + opinion-registry reconciliation to as-built, the efficiency health-axis lit, silent-dark verification-honesty closure, and a vector-inclusive re-measurement of the V6 retrieval stack that reverses the previously-shipped vector-less baseline. | 27 commits (`v5.13.0..v5.14.0`); paired with [crickets v3.26.0](https://github.com/alexherrero/crickets/releases/tag/v3.26.0) (the observability ledger's crickets half + the board tracking-model decision) — see the dated section below. |
...
**Commit range:** `v5.13.0..v5.14.0` (27 commits on `main`). Release notes: [v5.14.0](https://github.com/alexherrero/agentm/releases/tag/v5.14.0). Paired with [crickets v3.26.0](https://github.com/alexherrero/crickets/releases/tag/v3.26.0), which ships the crickets half of the observability ledger (#174) and the board tracking-model decision (#175).
