...
These three `plugins.autonomy.*` keys are config surface only as of this writing — the channels that read them (an on-device notifier and a digest-email sender) ship in later tasks of the same plan, and setting a key today has no observable effect until its channel lands. See [Autonomy — Delivery](agentm-autonomy#delivery--getting-it-in-front-of-you) for the full channel design.
