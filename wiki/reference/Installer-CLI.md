...
| `--enrich-enabled <true\|false>` | Set `daemon.enrich_enabled` — arm the eager enrichment trigger, so every capture fires a `claude -p` call on this machine. Absent by default, and deliberately so: this is the one config key here that **spends**, which is why arming it is an operator's decision on their own install rather than something a binary update can do. `agentmd enrich --yes` runs a single batch without it — the flag is for the standing behaviour, not for trying it once. Validates `true`/`false` (case-insensitive), rejects anything else with `rc=2` and writes nothing. Idempotent. |
