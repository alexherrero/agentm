...
`--notify-enabled` is now live: [`scripts/health/session_notify.py`](https://github.com/alexherrero/agentm/blob/main/scripts/health/session_notify.py) reads `plugins.autonomy.notify_enabled` and fires a once-daily macOS notification when it's `true`. See [Enable on-device notifications](Enable-On-Device-Notifications) for the recipe — it isn't yet wired to the runner, so it needs a manual or self-scheduled invocation until that task lands. The other two keys, `--email-to` and `--email-smtp-url`, are still config surface only — the digest-email sender that reads them ships in a later task of the same plan, and setting either today has no observable effect until it lands. See [Autonomy — Delivery](agentm-autonomy#delivery--getting-it-in-front-of-you) for the full channel design.
