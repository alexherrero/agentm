...
- **You need to undo an applied run.** Run `python3 scripts/migrate/card_backfill.py --revert <RUN_ID>`. This restores every file that run wrote. It also removes `memory/.card-backfill-complete`. It first checks that each file still holds what the run wrote. A run applied before that check existed is checked against the plan its dry run recorded, `<engine state dir>/card-backfill/plan-<RUN_ID>.json`, and without that file the revert refuses.
- **`--revert` refuses and names files that changed since the run.** Something wrote to each named file after the run, most often the night re-enriching a card. Restoring the file would lose that write, so the revert restores nothing and leaves the marker. To revert anyway, save each later write, put each named file back as the run left it (its first vault commit after the apply), run `--revert` again, and add the saved writes back by hand.
