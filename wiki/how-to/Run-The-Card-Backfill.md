...
- **You need to undo an applied run.** Run `python3 scripts/migrate/card_backfill.py --revert <RUN_ID>`. The script restores every file the run wrote. It removes the `memory/.card-backfill-complete` marker. The script first checks that each file still holds what the run wrote. A run applied before that check existed is checked against the plan its dry run recorded in `<engine state dir>/card-backfill/plan-<RUN_ID>.json`. If that plan file is missing, the revert refuses.
- **`--revert` refuses and names files that changed since the run.** Something wrote to each named file after the run. For example, the night re-enriched a card or regenerated a map, or a later migration removed a map page. Restoring the file would lose that write. The script restores nothing and leaves the marker. To revert anyway, save each later write. Put each named file back as the run left it; the first vault commit after the apply holds that state. Run `--revert` again. Add the saved writes back by hand.
