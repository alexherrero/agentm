...
- **You need to undo an applied run.** Run `python3 scripts/migrate/maps_and_root_notes.py --revert <RUN_ID>`. It restores every file the run wrote and removes `memory/.maps-and-root-notes-complete` if it exists. It first checks that each file still holds what the run wrote. A run applied before that check existed is checked against the plan its dry run recorded, `<engine state dir>/maps-and-root-notes/plan-<RUN_ID>.json`, and without that file the revert refuses.
- **`--revert` refuses and names files that changed since the run.** Something wrote to each named file after the run, such as the night appending to the moved day note or re-enriching a card whose links the run repointed. Restoring the file would lose that write, so the revert restores nothing and leaves the marker. To revert anyway, save each later write, put each named file back as the run left it (its first vault commit after the apply), run `--revert` again, and add the saved writes back by hand.
