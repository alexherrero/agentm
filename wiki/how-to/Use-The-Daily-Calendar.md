...
4. **Check the day index.** It regenerates automatically after every append or correction, so this step is for inspection, not required:
   ...
   An episodic session trace is the agent's own memory of one session: one `kind: session-trace` note under `memory/episodic/`, written at session end by the Stop hook from the transcript and the recall history, no model call. Its title is the session's first request; the body carries up to five sections, each omitted when empty — `## Asked` (the request in full), `## Outcome` (the session's own closing recap), `## Captured` and `## Recalled` as wikilinks, and `## Candidates` — every line the miner mined but did not file as a note below HIGH confidence, kept as the rule that fired, an excerpt, and a count (`Trace.render`, `harness/skills/memory/scripts/episodic_trace.py:110-154`). Frontmatter carries `touched:` (renamed from `entities:`) and, since both Stop hooks now pass `--surface`, a `surface:` field naming where the session ran (e.g. `claude-code`); both sit after the read fields the trace shares with a card, in the card's own order (agentm-vault § The card). A session that left nothing to record — nothing touched and no candidates either — leaves no trace file at all (`write_trace`, `episodic_trace.py:382-398`), so a quiet day's index links few or none.
