...
2. Size arguments: `patch` / `minor` / `major` override the auto-sized bump. A literal version like `0.8.0` pins the tag exactly (used when the commit range's auto-size undershoots — see the [v0.8.0 entry in `CHANGELOG.md`](https://github.com/alexherrero/agentic-harness/blob/main/CHANGELOG.md) for a worked example).
...
3. Confirm every `(Page-Name)` cross-link resolves — visual scan of [`wiki/_Sidebar.md`](https://github.com/alexherrero/agentic-harness/blob/main/wiki/_Sidebar.md) or run `python3 scripts/check-references.py`.
...
If any of these drift, refresh the wiki page *before* the release, not the anchor. The installer-boundary test at [`scripts/test-install.sh`](https://github.com/alexherrero/agentic-harness/blob/main/scripts/test-install.sh) proves drift never leaks into target projects (it runs `diff -r templates/wiki/ <scratch>/wiki/` byte-for-byte on every CI run); this manual check keeps drift from misinforming contributors to the harness repo.
...
See [ship-release spec](https://github.com/alexherrero/agentic-harness/blob/main/harness/skills/ship-release.md#L129-L135) for the full list. See [CI-Gates](CI-Gates) for what has to be green before invoking the skill.
