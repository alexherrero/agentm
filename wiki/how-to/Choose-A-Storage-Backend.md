...
> [!NOTE]
> **Status: implemented.** V5-1 part 5 shipped backend selection, the fail-loud guard, and the `doctor` storage preview this page describes — including V5-7 Part 2's `--requires <CAPS>` capability pre-flight (see [step 4](#step-4-optional-pre-flight-a-capability-requirement)). The engine cutover shipped separately, in V5-3 (v5.5.0); the [Memory-storage seam design § 3](memory-storage-seam) is the canonical account of what it changed.
>
...
- [The memory↔storage seam](Storage-Seam-Concepts) — why selection fails loud instead of demoting, and why the engine cutover was a step beyond V5-1 (it shipped in V5-3).
