...
2. **Read the enrichment line left to right.** The format is `N judged · N filed active · N below the floor · N sank · N calls · N tokens against the line · <model>`, with `N project records ·` inserted just before the call count on a night that merged into any (agentm-vault plan 09) — a project's charter, or a note under its `decisions/`, `designs/` or `research/`, is merged into rather than filed, so it sits beside the filing verdicts rather than inside them. Judged is how many notes the batch sent to the model, and the next three counts are what their verdicts decided. A note that sank is counted below the floor as well. `N failed` and `Stopped by <reason>` follow when a run had failures or reached one of its limits.
...
5. **Work *What needs you* from the top.** Each list gives a count and its first five items: unfiled notes the batch judged below the floor, possible twins with their similarity, shared keys, proposed facets, and what sank in the last seven days. Then three lists about what is coming or has just landed: *sinking within 30 days* and *archiving within 30 days*, each note with how long it has been silent — a threshold is easier to argue with while it is still thirty days off — and *lessons this week*, every note the weekly crystallize phase wrote, with how many sources taught each one. Read those last. A lesson never decays and nothing ages it out, so the morning it is written is the cheapest morning to disagree with it; if you would not keep one, that is the phase's own re-audit trigger firing. The next line links the day's `dreaming` facet, which lists every move the night made with the manifest behind any deletion, and the last links the needs-review map, which holds the full lists. See [Review flagged memories](Review-Flagged-Memories) for working it.
...
7. **Check *Spend* against the operator's lines.** `Last night:` gives the tokens the night added against the line, each tier as `<tier> N of <line>` (2,000,000 a night on either tier), then the tokens it processed, calls against the 250-call guard, and dollars. The line counts what a call adds: input, cache writes and output. The processed figure also counts the cached prefix every call re-reads, so at a card's measured cost it runs about three times larger. The note holds the night to these numbers even when a run by hand lowered its own. Under `Last night:` sits a line per tier-table job — *enrichment deep*, *enrichment light*, *crystallize* — with that job's own tokens, calls and dollars. That is a finer reading of the same spend, not a second accounting of it: a run recorded before the meter carried job names has no per-job lines and still counts in the totals. `The weekly phase:` appears on a night the crystallize phase ran, with the lessons it wrote, the recurrences that cleared the bar, its calls and its cost — the design re-audits that phase's cadence on this line after three runs. `Seven days:` gives both token figures and dollars across the week's paid runs, the weekly phase included. `Sessions, the last day:` appears when the observability rollup recorded session spend.
