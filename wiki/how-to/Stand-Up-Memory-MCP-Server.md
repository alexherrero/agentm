...
The daemon exposes four tools — `memory_search`, `memory_append`, `memory_capture`, `memory_forget` — to every host that speaks MCP while routing all writes through one process. See [Memory MCP tools](Memory-MCP-Tools) for full signatures and the soft-delete contract.
